...
public class SilverBox extends AbstractMaplePacketHandler {
  ...
  public static String Message =
    "You do not have to Silver Box key to open this box!";
  /* Inventario Full */
  public static String Inv_Cheio =
    "You do not have enough space in the inventory to receive the item. Please clear some space in your inventory.";
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    for (MapleInventoryType type : MapleInventoryType.values()) {
      if (c.getPlayer().getInventory(type).isFull()) {
        c
          .getPlayer()
          .dropMessage(5, Inv_Cheio + type.name().toLowerCase() + ".");
        return;
      }
    }
    int prizeid = 0;
