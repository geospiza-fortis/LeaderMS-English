...
public class Incubator {
  ...
  public static String Message =
    "You just won an item, thank you for supporting us!";
  /* Time Settings */
  ...
  public static boolean getIncubatedItem(MapleClient c) {
    ...
    try {
      FileReader fl = new FileReader(
        "Game/CashItems/Incubator/" +
        Random[(int) (chance * Random.length)] +
        ".properties"
      );
    ...
    if (itemcode == 0 || amount == 0) return false;
    if (
      getInventory(c, MapleInventoryType.EQUIP).isFull(1) ||
      getInventory(c, MapleInventoryType.USE).isFull(3) ||
      getInventory(c, MapleInventoryType.ETC).isFull(1)
    ) {
    ...
    c
      .getSession()
      .write(MaplePacketCreator.getShowItemGain(itemcode, (short) amount));
    MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
    FilePrinter.printIncubadora(
  ...
  private static MapleInventory getInventory(
    MapleClient c,
