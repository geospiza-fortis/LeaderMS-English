...
public class LoginServer implements Runnable, LoginServerMBean {
  ...
  static {
    ...
    try {
      mBeanServer.registerMBean(
        instance,
        new ObjectName("net.login:type=LoginServer,name=LoginServer")
      );
    } catch (Exception e) {
        ...
        synchronized (wli) {
          try {
            FileReader fileReader = new FileReader(
              System.getProperty("login.config")
            );
            ...
            wli =
              worldRegistry.registerLoginServer(
                initialProp.getProperty("login.key"),
                lwi
              );
            ...
            maxCharacters =
              Integer.parseInt(prop.getProperty("login.maxCharacters"));
            AutoReg =
              Boolean.parseBoolean(prop.getProperty("login.AutoRegister"));
            AutoRegLimit =
              Byte.parseByte(prop.getProperty("login.AutoRegisterLimit"));
            twoWorlds =
              Boolean.parseBoolean(
                prop.getProperty("world.twoWorlds", "false")
              );
          } catch (Exception e) {
  ...
  public void run() {
    try {
      ...
      AutoReg =
        Boolean.parseBoolean(prop.getProperty("login.AutoRegister", "false"));
      AutoRegLimit =
        Byte.parseByte(prop.getProperty("login.AutoRegisterLimit", "5"));
      twoWorlds =
        Boolean.parseBoolean(prop.getProperty("world.twoWorlds", "false"));
    } catch (Exception e) {
    ...
    rankingInterval =
      Long.parseLong(prop.getProperty("login.ranking.interval"));
    tMan.register(new RankingWorker(), rankingInterval);
    try {
      acceptor.bind(
        new InetSocketAddress(PORT),
        new MapleServerHandler(
          PacketProcessor.getProcessor(PacketProcessor.Mode.LOGINSERVER)
        ),
        cfg
      );
  ...
  public int getPossibleLogins() {
    ...
    try {
      ...
      PreparedStatement limitCheck = con.prepareStatement(
        "SELECT COUNT(*) FROM accounts WHERE loggedin > 1 AND gm=0"
      );
