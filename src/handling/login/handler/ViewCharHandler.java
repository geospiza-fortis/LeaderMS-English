  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    try {
      PreparedStatement ps = con.prepareStatement(
        "SELECT * FROM characters WHERE accountid = ?"
      );
      ...
      while (rs.next()) {
        ...
        for (int w : worlds) if (w == cworld) inside = true;

        if (!inside) worlds.add(cworld);

        MapleCharacter chr = MapleCharacter.loadCharFromDB(
          rs.getInt("id"),
          c,
          false
        );
      ...
      for (int w : worlds) {
        ...
        for (MapleCharacter chr : chars) if (
          chr.getWorld() == w
        ) chrsinworld.add(chr);
        c
          .getSession()
          .write(MaplePacketCreator.showAllCharacterInfo(w, chrsinworld));
      }
    } catch (Exception e) {
