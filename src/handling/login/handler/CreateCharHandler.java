...
public class CreateCharHandler extends AbstractMaplePacketHandler {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    IItem eq_bottom = MapleItemInformationProvider
      .getInstance()
      .getEquipById(bottom); //
    eq_bottom.setPosition((byte) -6);
    equip.addFromDB(eq_bottom);
    IItem eq_shoes = MapleItemInformationProvider
      .getInstance()
      .getEquipById(shoes); //shoes
    eq_shoes.setPosition((byte) -7);
    equip.addFromDB(eq_shoes);
    IItem eq_weapon = MapleItemInformationProvider
      .getInstance()
      .getEquipById(weapon); //weapon
    ...
    if (charok && MapleCharacterUtil.canCreateChar(name, c.getWorld())) {
      ...
      FilePrinter.printNovo(
        "" + newchar.getName() + ".rtf",
        "Name of new player: " +
        newchar.getName() +
        "\r\nAccount ID: (" +
        newchar.getAccountID() +
        ")\r\nEntered the game: " +
