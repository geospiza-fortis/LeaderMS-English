...
public final class PacketProcessor {
  ...
  private PacketProcessor() {
    int maxRecvOp = 0;
    for (RecvPacketOpcode op : RecvPacketOpcode.values()) if (
      op.getValue() > maxRecvOp
  ...
  public void registerHandler(
    ...
    try {
      ...
      System.out.println(
        "Error while registering handler - " + handler.toString()
      );
    }
  }
  ...
  public void reset(Mode mode) {
    ...
    if (mode == Mode.LOGINSERVER) {
      ...
      registerHandler(
        RecvPacketOpcode.CHANGE_CHANNEL,
      ...
      registerHandler(
        RecvPacketOpcode.PLAYER_LOGGEDIN,
      ...
      registerHandler(
        RecvPacketOpcode.TROCK_ADD_MAP,
        new VIPRockAddMapHandler()
      );
      registerHandler(RecvPacketOpcode.CANCEL_BUFF, new CancelBuffHandler());
      registerHandler(
        RecvPacketOpcode.CANCEL_ITEM_EFFECT,
        new CancelItemEffectHandler()
      );
      registerHandler(
        RecvPacketOpcode.PLAYER_INTERACTION,
      ...
      registerHandler(
        RecvPacketOpcode.DISTRIBUTE_SP,
      ...
      registerHandler(
        RecvPacketOpcode.PARTY_OPERATION,
        new PartyOperationHandler()
      );
      registerHandler(
        RecvPacketOpcode.DENY_PARTY_REQUEST,
      ...
      registerHandler(
        RecvPacketOpcode.ENTER_CASH_SHOP,
        new EnterCashShopHandler()
      );
      registerHandler(
        RecvPacketOpcode.DAMAGE_SUMMON,
      ...
      registerHandler(
        RecvPacketOpcode.BUDDYLIST_MODIFY,
        new BuddylistModifyHandler()
      );
      registerHandler(
        RecvPacketOpcode.USE_ITEMEFFECT,
      ...
      registerHandler(
        RecvPacketOpcode.GUILD_OPERATION,
      ...
      registerHandler(
        RecvPacketOpcode.BBS_OPERATION,
      ...
      registerHandler(
        RecvPacketOpcode.ENABLE_ACTION,
        new EnableActionHandler()
      );
      registerHandler(
        RecvPacketOpcode.USE_CATCH_ITEM,
        new UseCatchItemHandler()
      );
      registerHandler(
        RecvPacketOpcode.USE_MOUNT_FOOD,
        new UseMountFoodHandler()
      );
      registerHandler(
        RecvPacketOpcode.CLOSE_CHALKBOARD,
        new CloseChalkboardHandler()
      );
      registerHandler(RecvPacketOpcode.DUEY_ACTION, new DueyHandler());
      registerHandler(
        RecvPacketOpcode.MONSTER_CARNIVAL,
      ...
      registerHandler(
        RecvPacketOpcode.ALLIANCE_OPERATION,
      ...
      registerHandler(
        RecvPacketOpcode.HIRED_MERCHANT_REQUEST,
