...
public class ExternalCodeTableGetter {
  ...
  private <T extends Enum<? extends IntValueHolder> & IntValueHolder> int getValue(
    ...
    if (prop != null && prop.length() > 0) {
      ...
      if (args.length == 2) {
        ...
        if (base == def) base = getValue(args[0], values, def);
        offset = args[1];
      } else offset = args[0];
      if (offset.length() > 2 && offset.substring(0, 2).equals("0x")) return (
        Integer.parseInt(offset.substring(2), 16) + base
      ); else return Integer.parseInt(offset) + base;
    }
    return def;
  }
  ...
  public static <T extends Enum<? extends WritableIntValueHolder> & WritableIntValueHolder> String getOpcodeTable(
    ...
    for (T code : all) enumVals.append(
      code.name() +
      " = " +
      "0x" +
      HexTool.toString(code.getValue()) +
      " (" +
      code.getValue() +
      ")\n"
    );
  ...
  public static <T extends Enum<? extends WritableIntValueHolder> & WritableIntValueHolder> void populateValues(
    ...
    for (T code : values) code.setValue(exc.getValue(code.name(), values, -2));
