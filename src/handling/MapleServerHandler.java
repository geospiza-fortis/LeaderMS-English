...
public class MapleServerHandler extends IoHandlerAdapter {
  ...
  public void exceptionCaught(IoSession session, Throwable cause)
    ...
    if (mc != null && mc.getPlayer() != null) {
      FilePrinter.printError(
        ...
        cause,
        "Excecao causada por: " + mc.getPlayer()
      );
    }
  }
  ...
  public void sessionOpened(IoSession session) throws Exception {
    ...
    MapleAESOFB sendCypher = new MapleAESOFB(
      key,
      ivSend,
    ...
    session.write(
      MaplePacketCreator.getHello(MAPLE_VERSION, ivSend, ivRecv, false)
    );
  ...
  public void sessionClosed(IoSession session) throws Exception {
    synchronized (session) {
      MapleClient client = (MapleClient) session.getAttribute(
        MapleClient.CLIENT_KEY
      );
      if (client != null) {
  ...
  public void messageReceived(IoSession session, Object message)
    ...
    if (packetHandler != null && packetHandler.validateState(client)) try {
      log.trace(
        "op " + packetId + " handled by " + packetHandler.getClass().getName()
      );
