 ...
 * @author Matze
 */
public class WorldRegistryImpl
  ...
  private int getFreeChannelId() {
    for (int i = 0; i < 30; i++) if (!channelServer.containsKey(i)) return i;
    return -1;
  }
    ...
    try {
      ...
      if (rs.next()) {
        ...
        if (channelId < 1) {
          ...
          if (channelId == -1) throw new RuntimeException(
        ...
        WorldChannelInterface ret = new WorldChannelInterfaceImpl(
          cb,
          rs.getInt("channelid")
        );
        rs.close();
      ...
      log.error(
        "Encountered database error while authenticating channelserver " + ex
      );
    }
    throw new RuntimeException(
      "Couldn't find a channel with the given key (" + authKey + ")"
    );
  }

  public void deregisterChannelServer(int channel) throws RemoteException {
    channelServer.remove(channel);
    for (LoginWorldInterface wli : loginServer) wli.channelOffline(channel);
  ...
  public WorldLoginInterface registerLoginServer(
    ...
    try {
      ...
      log.error(
        "Encountered database error while authenticating loginserver " + e
      );
    }
    return ret;
  }
  ...
  public int getHighestChannelId() {
    int highest = 0;
    for (Integer channel : channelServer.keySet()) if (
      channel != null && channel.intValue() > highest
  ...
  public void clearGuilds() {
    ...
    try {
      for (ChannelWorldInterface cwi : this.getAllChannelServers()) cwi.reloadGuildCharacters();
    } catch (RemoteException re) {
      log.error(
        "RemoteException occurred while attempting to reload guilds. " + re
      );
    }
  }
