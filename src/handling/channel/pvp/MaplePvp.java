...
public class MaplePvp {
  ...
  private static boolean isMeleeAttack(
  ...
  private static boolean isRangeAttack(
    AbstractDealDamageHandler.AttackInfo attack
  ) {
    switch (attack.skill) {
  ...
  private static boolean isAoeAttack(
    AbstractDealDamageHandler.AttackInfo attack
  ) {
    switch (attack.skill) {
  ...
  private static void DamageBalancer(
    AbstractDealDamageHandler.AttackInfo attack
  ) {
    if (attack.skill == 0) {
      pvpDamage = 100;
      ...
      if (attack.skill == 4201005) {
        ...
        attack.skill == 3121004 ||
        attack.skill == 3111006 ||
        attack.skill == 3211006
      ) {
        maxDis = 450;
  ...
  private static void monsterBomb(
    ...
    attackedPlayers
      ...
      .write(
        MaplePacketCreator.serverNotice(
          5,
          player.getName() + " has hit you for " + attackedDamage + " damage!"
        )
      );
    ...
    if (attackedPlayers.getHp() <= 0 && !attackedPlayers.isAlive()) {
      ...
      if (
        player.getGuildId() != 0 &&
        player.getGuildId() != attackedPlayers.getGuildId()
      ) {
        try {
          MapleGuild guild = player
            .getClient()
            .getChannelServer()
            .getWorldInterface()
            .getGuild(player.getGuildId(), null);
      ...
      player
        ...
        .write(
          MaplePacketCreator.serverNotice(
            6,
            "You've killed " +
            attackedPlayers.getName() +
            "!! You've gained a pvp kill!"
          )
        );
      ...
      attackedPlayers
        ...
        .write(
          MaplePacketCreator.serverNotice(
            6,
            player.getName() + " has killed you!"
          )
        );
    }
  }

  public static void doPvP(
    MapleCharacter player,
    MapleMap map,
    AbstractDealDamageHandler.AttackInfo attack
  ) {
    ...
    getDirection(attack);
    for (MapleCharacter attackedPlayers : player
      .getMap()
      .getNearestPvpChar(
        player.getPosition(),
        maxDis,
      ...
      if (
        attackedPlayers.isAlive() &&
        (
          player.getParty() == null ||
          player.getParty() != attackedPlayers.getParty()
        )
      ) {
