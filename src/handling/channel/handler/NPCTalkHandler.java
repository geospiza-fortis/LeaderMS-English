...
public class NPCTalkHandler extends AbstractMaplePacketHandler {
  ...
  protected static final String pnpcText =
    "Hello? I achieved level 200. it was really hard to get level 200. Passed through trial and sufferings, finally I made it. You too can be a master if you work hard. Good luck!!";
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    MapleNPC npc;
    if (obj instanceof MapleNPC) npc = (MapleNPC) obj; else npc =
      ((PlayerNPCMerchant) obj).getNPC();
    ...
    if (npc.getId() == 2042004 || npc.getId() == 2042003) {
      for (MapleMapObject o : c.getPlayer().getMap().getAllPlayer()) {
        ((MapleCharacter) o).changeMap(
            c.getChannelServer().getMapFactory().getMap(980000000),
            c.getChannelServer().getMapFactory().getMap(980000000).getPortal(0)
          );
        ((MapleCharacter) o).getClient()
          .getSession()
          .write(
    ...
    if (
      tutMap &&
      npc.getId() >= PlayerNPCEngine.WARRIOR_ID &&
      npc.getId() <= PlayerNPCEngine.THIEF_ID + 19
    ) {
      c
        .getSession()
        .write(
    ...
    if (c.getCM() != null) {
      ...
      c
        .getSession()
        .write(MaplePacketCreator.confirmShopTransaction((byte) 20));
    }
    if (npc.hasShop()) {
      npc.sendShop(c);
    } else {
