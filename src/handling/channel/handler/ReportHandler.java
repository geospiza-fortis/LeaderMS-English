...
public class ReportHandler extends AbstractMaplePacketHandler {
  ...
  static final int GMGuildId = 198;
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    if (clogLen > 0) chatlog = slea.readAsciiString(clogLen);
    ...
    if (reported) c
      .getSession()
      .write(MaplePacketCreator.reportReply((byte) 0)); else c
      .getSession()
      .write(MaplePacketCreator.reportReply((byte) 4));
    WorldChannelInterface wci = c.getChannelServer().getWorldInterface();
    try {
      wci.broadcastGMMessage(
        null,
        MaplePacketCreator.serverNotice(5, sb.toString()).getBytes()
      );
    } catch (Exception ex) {}
  }

  public boolean addReportEntry(
    int reporterId,
    int victimId,
    byte reason,
    String chatlog
