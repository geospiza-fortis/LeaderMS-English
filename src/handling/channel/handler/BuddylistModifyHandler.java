...
public class BuddylistModifyHandler extends AbstractMaplePacketHandler {
  ...
  private static class CharacterIdNameBuddyCapacity extends CharacterNameAndId {
    ...
    public CharacterIdNameBuddyCapacity(
      int id,
      String name,
      int buddyCapacity
    ) {
  ...
  private void nextPendingRequest(MapleClient c) {
    CharacterNameAndId pendingBuddyRequest = c
      .getPlayer()
      .getBuddylist()
      .pollPendingRequest();
  ...
  private CharacterIdNameBuddyCapacity getCharacterIdAndNameFromDatabase(
    String name
  ) throws SQLException {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    WorldChannelInterface worldInterface = c
      .getChannelServer()
      .getWorldInterface();
    ...
    if (mode == 1) { // add
      ...
      if (ble != null && !ble.isVisible()) {
        ...
        try {
          ...
          if (charWithId != null) {
            ...
            if (channel != -1) {
              ChannelWorldInterface channelInterface = worldInterface.getChannelInterface(
                channel
              );
              buddyAddResult =
            ...
            if (buddyAddResult == BuddyAddResult.BUDDYLIST_FULL) {
              ...
              c
                .getSession()
                .write(MaplePacketCreator.buddylistMessage((byte) 12));
            } else {
              ...
              c
                .getSession()
                .write(
                  MaplePacketCreator.updateBuddylist(buddylist.getBuddies())
                );
            }
          } else {
            // Not Registered
            c
              .getSession()
              .write(MaplePacketCreator.buddylistMessage((byte) 15));
          }
      ...
      if (!buddylist.isFull()) {
        try {
          ...
          if (otherName != null) {
            buddylist.put(
              new BuddylistEntry(otherName, otherCid, channel, true)
            );
            c
              .getSession()
              .write(
      ...
      c
        .getSession()
        .write(
          MaplePacketCreator.updateBuddylist(player.getBuddylist().getBuddies())
        );
      nextPendingRequest(c);
    }
  }

  private void notifyRemoteChannel(
    MapleClient c,
    int remoteChannel,
    int otherCid,
