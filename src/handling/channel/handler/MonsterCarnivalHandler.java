  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
      ...
      c
        ...
        .write(
          MaplePacketCreator.serverNotice(
            5,
            "You do not have enough CP to use this."
          )
        );
    ...
    if (tab == 0) { //only spawning for now..
      ...
      Point spawnPos = c
        .getPlayer()
        .getMap()
        .getRandomSP(c.getPlayer().getTeam());
      if (spawnPos == null) {
        c
          .getSession()
          .write(
            MaplePacketCreator.serverNotice(
              5,
              "The monster cannot be summoned, as all spawn points are taken."
            )
          );
        c.getSession().write(MaplePacketCreator.enableActions());
        return;
          ...
          if (Math.random() < 0.8) { //80%
            for (
              ...
              MapleCharacter chr = c
                ...
                .getPlayerStorage()
                .getCharacterById(
                  c.getPlayer().getParty().getEnemy().getMemberByPos(i).getId()
                );
              if (chr != null) {
                chr.giveDebuff(
                  debuff,
                  MobSkillFactory.getMobSkill(skillId, level),
                  true
                );
              }
            }
            ...
            c
              .getSession()
              .write(
                MaplePacketCreator.serverNotice(5, "Casting the spell failed.")
              );
          }
        } else {
      ...
      if (status != null) {
        int success = c
          .getPlayer()
          .getMap()
          .spawnGuardian(status, c.getPlayer().getTeam());
        if (success == -1 || success == 0) {
          if (success == -1) c
            ...
            .write(
              MaplePacketCreator.serverNotice(
                5,
                "The protector cannot be summoned, as all protector spots are taken."
              )
            ); else if (success == 0) c
            .getSession()
            .write(
              MaplePacketCreator.serverNotice(
                5,
                "The protector is already summoned."
              )
            );
          c.getSession().write(MaplePacketCreator.enableActions());
          return;
        ...
        c
          ...
          .write(
            MaplePacketCreator.serverNotice(
              5,
              "The protector cannot be summoned."
            )
          );
        c.getSession().write(MaplePacketCreator.enableActions());
      }
    ...
    c
      ...
      .getMap()
      .broadcastMessage(
        MonsterCarnivalPacket.playerSummoned(c.getPlayer().getName(), tab, num)
      );
  }
