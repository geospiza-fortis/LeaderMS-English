...
public class MoveLifeHandler extends AbstractMovementPacketHandler {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    if (
      skill_1 >= 100 && skill_1 <= 200 && monster.hasSkill(skill_1, skill_2)
    ) {
    ...
    if (res != null) {
          ...
          c
            ...
            .broadcastGMMessage(
              ...
              MaplePacketCreator
                .serverNotice(
                  5,
                  c.getPlayer().getName() + " is using dupex monster vaccum."
                )
                .getBytes()
            );
      ...
      MaplePacket packet = MaplePacketCreator.moveMonster(
        skillByte,
        skill,
        skill_1,
        skill_2,
    ...
    if (
      monster.isHypnotized() &&
      monster.getHypnotizer() != null &&
      monster.canDamage()
    ) {
      MapleMonster target = monster
        .getMap()
        .findClosestMonster(monster.getPosition(), 1000);
      if (target != null) {
        ...
        monster
          .getMap()
          .broadcastMessage(MaplePacketCreator.mobDamageMob(target, dmg, 0));
      }
    }

    if (
      monster.getId() == 9300061 &&
        ...
        monster
          ...
          .broadcastMessage(
            MaplePacketCreator.serverNotice(
              0,
              "The Moon Bunny is feeling sick. Please protect him so he can make delicious rice cakes!"
            )
          );
    ...
    if (
      monster.getId() == 9300061 &&
      (monster.getShouldDrop() == true || monster.getJustSpawned() == true)
    ) {
      if (monster.getJustSpawned() == true) {
        ...
        monster
          ...
          .broadcastMessage(
            MaplePacketCreator.serverNotice(
              0,
              "The Moon Bunny made ​​1 rice cake."
            )
          );
        ...
        try {
          monster
            ...
            .spawnItemDrop(
              monster,
              monster.getEventInstance().getPlayers().get(0),
              Item,
              monster.getPosition(),
              false,
              false
        ...
        if (monster.getShouldDrop() == true) {
          ...
          monster
            ...
            .broadcastMessage(
              MaplePacketCreator.serverNotice(
                0,
                "The Moon Bunny made  " + d + " rice cakes."
              )
            );
