    ...
    if (c.getChannelServer().allowMTS()) {
      ...
      try {
        ...
        PreparedStatement ps = con.prepareStatement(
          "SELECT * FROM mts_items WHERE tab = 1 AND transfer = 0 ORDER BY id DESC LIMIT ?, 16"
        );
        ps.setInt(1, 0);
      ...
      c
        .getSession()
        .write(MTSCSPacket.NotYetSoldInv(getNotYetSold(c.getPlayer().getId())));
      c.getPlayer().saveToDB(true, true);
    } else {
      new ServernoticeMapleClientMessageCallback(5, c)
      .dropMessage("The Maple Trade Service is not available at this time.");
