    ...
    if (mode == Action.CREATE.getCode()) {
      ...
      if (createType == 3) { // Trade
        ...
        if (createType == 1 || createType == 2) {
          ...
          IPlayerInteractionManager game = new MapleMiniGame(
            c.getPlayer(),
            type,
            desc,
            pass
          ...
          if (!c.getPlayer().hasMerchant() && c.getPlayer().tempHasItems()) {
            c
              .getPlayer()
              .dropMessage(1, "Contact Fredrick to recover stored items.");
            return;
          }
          if (
            c
          ...
          if (
            c.getPlayer().haveItem(5140000) ||
            c.getPlayer().haveItem(5140001) ||
            c.getPlayer().haveItem(5140002) ||
            c.getPlayer().haveItem(5140003) &&
            c.getPlayer().haveItem(5140004) ||
            c.getPlayer().haveItem(5140006)
            /* || c.getPlayer().haveItem(5030006) || c.getPlayer().haveItem(5030002) || c.getPlayer().haveItem(5030001) || c.getPlayer().haveItem(5030000)*/
          ) {
          ...
          if (c.getPlayer().haveItem(itemId, 1, false, true)) {
            ...
            c
              .getSession()
              .write(MaplePacketCreator.getInteraction(c.getPlayer(), true));
          } else {
            AutobanManager
              .getInstance()
              .autoban(
                ...
                " | Merchant Shop: Tried opening a store without a merchant item."
              );
            return;
          }
        } else {
          System.out.println(
            "Unhandled PLAYER_INTERACTION packet: " + slea.toString()
          );
        }
      }
    } else if (mode == Action.INVITE.getCode()) {
      int otherPlayer = slea.readInt();
      MapleCharacter otherChar = c
        .getPlayer()
        .getMap()
        .getCharacterById(otherPlayer);
      ...
      if (
        ...
        MapleTrade.visitTrade(
          c.getPlayer(),
          c.getPlayer().getTrade().getPartner().getChr()
        );
      } else {
        int oid = slea.readInt();
        MapleMapObject ob = c.getPlayer().getMap().getMapObject(oid);
        if (
          ob instanceof IPlayerInteractionManager &&
          c.getPlayer().getInteraction() == null
        ) {
          ...
          if (ips.getShopType() == 1) {
            ...
            if (merchant.isOwner(c.getPlayer())) {
              ...
              merchant.broadcast(
                MaplePacketCreator.shopErrorMessage(0x0D, 1),
                false
              );
              ...
              c
                .getSession()
                .write(MaplePacketCreator.getInteraction(c.getPlayer(), false));
              return;
              ...
              c
                ...
                .dropMessage(
                  1,
                  "This shop is in maintenance, please come back later."
                );
              return;
            }
      ...
      if (c.getPlayer().getTrade() != null) {
        ...
        ips.broadcast(
          MaplePacketCreator.shopChat(
            ...
            ips.isOwner(c.getPlayer())
              ? 0
              : ips.getVisitorSlot(c.getPlayer()) + 1
          ),
          true
        );
      }
      ...
      if (shop != null && shop.isOwner(c.getPlayer())) {
        ...
        if (shop.getShopType() == 1) {
          ...
          c
            .getPlayer()
            .getMap()
            .broadcastMessage(MaplePacketCreator.spawnHiredMerchant(merchant));
          c.getPlayer().setInteraction(null);
      ...
      c
        .getPlayer()
        .getInteraction()
        .broadcast(MiniGamePacket.getMiniGameReady(), true);
    } else if (mode == Action.UN_READY.getCode()) {
      c
        .getPlayer()
        .getInteraction()
        .broadcast(MiniGamePacket.getMiniGameUnReady(), true);
      ...
      c
        ...
        .getMap()
        .broadcastMessage(
          MaplePacketCreator.sendInteractionBox(game.getOwner())
        );
      ...
      if (game.getGameType() == MiniGameType.OMOK) {
        game.broadcast(
          MiniGamePacket.getMiniGameForfeit(
            game,
      ...
      if (game.isOwner(c.getPlayer())) {
        game
          .getVisitors()[0].getClient()
          .getSession()
          .write(MiniGamePacket.getMiniGameRequestTie());
      } else {
        game
          .getOwner()
          .getClient()
      ...
      game.broadcast(
        MiniGamePacket.getMiniGameSkipTurn(game.isOwner(c.getPlayer()) ? 0 : 1),
        true
      );
      ...
      ((MapleMiniGame) c.getPlayer().getInteraction()).setPiece(
          x,
          y,
          type,
          c.getPlayer()
      ...
      if (turn == 1) {
        ...
        game.broadcast(
          MiniGamePacket.getMatchCardSelect(turn, slot, firstslot, turn),
          !game.isOwner(c.getPlayer())
        );
      } else if (game.getCardId(firstslot + 1) == game.getCardId(slot + 1)) {
        if (game.isOwner(c.getPlayer())) {
          game.broadcast(
            MiniGamePacket.getMatchCardSelect(turn, slot, firstslot, 2),
            true
          );
          game.setOwnerPoints();
        } else {
          game.broadcast(
            MiniGamePacket.getMatchCardSelect(turn, slot, firstslot, 3),
            true
          );
        ...
        game.broadcast(
          MiniGamePacket.getMatchCardSelect(
            turn,
            slot,
            firstslot,
            game.isOwner(c.getPlayer()) ? 0 : 1
      ...
      if (c.getPlayer().getTrade() != null && item != null) {
        if (checkq > 4000) {
          AutobanManager
            ...
            .autoban(
              c,
              Configuration.Server_Name + " | Some kind of cheats went on."
            );
        }
        if (
          ...
          if (
            !c.getChannelServer().allowUndroppablesDrop() &&
      ...
      mode == Action.ADD_ITEM.getCode() || mode == Action.PUT_ITEM.getCode()
    ) {
      ...
      if (shop != null && shop.isOwner(c.getPlayer())) {
        if (ivItem != null && ivItem.getQuantity() >= bundles * perBundle) {
          if (price < 0) {
            AutobanManager
              ...
              .autoban(
                c,
                Configuration.Server_Name +
                " | Tried to add an item with a negative price."
              );
            return;
          }
          if (
          ...
          MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
          if (
            ii.isThrowingStar(ivItem.getItemId()) ||
            ii.isBullet(ivItem.getItemId())
          ) {
            MapleInventoryManipulator.removeFromSlot(
              c,
              type,
      ...
      mode == Action.BUY.getCode() || mode == Action.MERCHANT_BUY.getCode()
    ) {
      ...
      mode == Action.TAKE_ITEM_BACK.getCode() ||
      mode == Action.REMOVE_ITEM.getCode()
    ) {
