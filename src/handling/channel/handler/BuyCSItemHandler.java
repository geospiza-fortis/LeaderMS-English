...
public class BuyCSItemHandler extends AbstractMaplePacketHandler {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    if (action == 3) {
        ...
        c
          ...
          .write(
            MaplePacketCreator.serverNotice(
              1,
              "This item is now blocked from the Cash Shop!"
            )
          );
        updateInformation(c);
        return;
      ...
      if (
        !MapleInventoryManipulator.checkSpace(c, itemID, item.getCount(), "")
      ) {
        c
          .getPlayer()
          .dropMessage(
            1,
            "Your inventory is full! Check to make sure you have space."
          );
        updateInformation(c);
        return;
      }
      ...
      if (itemID == 5200000) {
        ...
        AutobanManager
          .getInstance()
          .autoban(c, "Buying item that does not exist in Cash Shop!");
        return;
      }
      if (c.getPlayer().getCSPoints(useNX) >= item.getPrice()) {
        ...
        AutobanManager
          .getInstance()
          .autoban(c, "Trying to buy in Cash Shop without NX!");
        return;
      }
      if (itemID >= 5000000 && itemID <= 5000370) {
        ...
        MapleInventoryManipulator.addById(
          ...
          null,
          MaplePet.createPet(item.getId())
        );
      } else {
        MapleInventoryManipulator.addFromDrop(
          c,
          getItemFromCashInfo(item),
          null
        );
        FilePrinter.printCashShop(
          c.getPlayer().getName() + ".txt",
          "Purchased Item: " +
          item.getId() +
          "\r\nValue: " +
          item.getPrice() +
          "\r\nRemaining Cash: (PayPal) " +
          c.getPlayer().getCSPoints(1) +
          "/ (MaplePoints) " +
          c.getPlayer().getCSPoints(2) +
          "/ (CardCash) " +
          c.getPlayer().getCSPoints(4) +
          "\r\nOn day: " +
      ...
      if (checkBirthday(c, slea.readInt())) {
        ...
        if (victim != null) {
          if (
            item == null ||
            c.getPlayer().getCSPoints(4) < item.getPrice() ||
            text.length() > 73 ||
            text.length() < 1
          ...
          if (item.getId() == 5220000) {
            c
              .getPlayer()
              .dropMessage(1, "This item is not available for gifting.");
            updateInformation(c);
            return;
          }
          if (
      ...
      try {
        ...
        PreparedStatement ps = con.prepareStatement(
          "DELETE FROM wishlist WHERE charid = ?"
        );
          ...
          if (sn != 0) {
            ps =
              con.prepareStatement(
                "INSERT INTO wishlist(charid, sn) VALUES(?, ?) "
              );
            ps.setInt(1, c.getPlayer().getId());
            ps.setInt(2, sn);
            ps.executeUpdate();
      ...
      c
        .getSession()
        .write(MTSCSPacket.sendWishList(c.getPlayer().getId(), true));
    } else if (action == 7) {
      slea.skip(1);
      byte toCharge = slea.readByte();
      int toIncrease = slea.readInt();
      if (
        c.getPlayer().getCSPoints(toCharge) >= 4000 &&
        c.getPlayer().getStorage().getSlots() < 48
      ) { // 48 is max.
        c.getPlayer().modifyCSPoints(toCharge, -4000);
        if (toIncrease == 0) c.getPlayer().getStorage().gainSlots((byte) 4);
        c.getSession().write(MTSCSPacket.showNXMapleTokens(c.getPlayer()));
        c.getSession().write(MTSCSPacket.enableCSUse0());
        c.getSession().write(MTSCSPacket.enableCSUse1());
        c.getSession().write(MTSCSPacket.enableCSUse2());
      ...
      if (partnerChar == null) {
        c
          ...
          .write(
            MaplePacketCreator.serverNotice(
              1,
              "The partner specified could not be found. \\r\\nPlease make sure your partner is online and in the same channel."
            )
          );
      } else if (
        c
        ...
        c
          .getPlayer()
          .dropMessage(1, "You already have a ring with another person!");
        return;
      } else if (
        c
          .getPlayer()
        ...
        c
          ...
          .write(
            MaplePacketCreator.serverNotice(
              1,
              "Successfully created a ring for you and your partner!\\r\\nIf you can not see the effect, try re-logging in."
            )
          );
      }
    } else if (action == 7) {
