...
public class BBSOperationHandler extends AbstractMaplePacketHandler {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    if (c.getPlayer().getGuildId() <= 0) return; // expelled while viewing bbs or hax
    ...
    switch (mode) {
      case 0:
        ...
        if (icon >= 0x64 && icon <= 0x6a) {
          if (
            !c.getPlayer().haveItem(5290000 + icon - 0x64, 1, false, true)
          ) return; // hax, using an nx icon that s/he doesn't have
        } else if (!(icon >= 0 && icon <= 2)) return; // hax, using an invalid icon

        if (!bEdit) newBBSThread(
          c,
          title,
          text,
          icon,
          bNotice
        ); else editBBSThread(c, title, text, icon, localthreadid);
        break;
      case 1:
        // delete a thread
  ...
  public static void newBBSReply(
    MapleClient client,
    int localthreadid,
    String text
  ) {
    ...
    try {
      ...
      if (!threadRS.next()) return; // thread no longer exists, deleted?
  ...
  public static void editBBSThread(
    ...
    if (c.getGuildId() <= 0) return; // expelled while viewing?
  ...
  public static void deleteBBSThread(MapleClient client, int localthreadid) {
    ...
    try {
      ...
      if (
        mc.getId() != threadRS.getInt("postercid") && mc.getGuildRank() > 2
      ) return; // but let all master/jrs delete // [hax] deleting a thread that he didn't make
      int threadid = threadRS.getInt("threadid");
  ...
  public static void deleteBBSReply(MapleClient client, int replyid) {
    ...
    try {
      ...
      ps.setInt(1, replyid);
      ResultSet rs = ps.executeQuery();
      if (!rs.next()) return; // reply no longer exists, deleted already?

      if (mc.getId() != rs.getInt("postercid") && mc.getGuildRank() > 2) return; // [hax] deleting a reply that he didn't make
  ...
  public static void displayThread(
    ...
    try {
      ...
      if (threadRS.getInt("replycount") > 0) {
        ...
        ps2.setInt(
          1,
          !bIsThreadIdLocal ? threadid : threadRS.getInt("threadid")
        );
