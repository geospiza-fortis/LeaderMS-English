  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    if (c.getPlayer().getMap().getDisableDamage() && !c.getPlayer().isGM()) {
      c
        .getSession()
        .write(
          MaplePacketCreator.serverNotice(5, "Attacking here is disabled.")
        );
    ...
    if (
      skillId != 0 &&
      skill.getEffect(player.getSkillLevel(skill)).getCooldown() > 0
    ) {
    ...
    if (type == MapleWeaponType.NOT_A_WEAPON) throw new RuntimeException(
      "Player " +
      player.getName() +
      " is attacking with something that is not a weapon."
    );
    ...
    boolean hasShadowPartner =
      player.getBuffedValue(MapleBuffStat.SHADOWPARTNER) != null;
    ...
    boolean shadowClaw =
      player.getBuffedValue(MapleBuffStat.SHADOW_CLAW) != null;
    ...
    if (projectile != 0 || soulArrow || attack.skill == 5121002) {
      ...
      if (mii.isThrowingStar(projectile)) {
        ...
        for (int i = 0; i < 255; i++) { // impose order...
          IItem item = player
            ...
            .getItem((byte) i);
          if (item != null) {
            if (item.getItemId() / 1000 == 5021) visProjectile =
              item.getItemId();
            break;
          }
        }
      } else if (
        soulArrow || skillId == 3111004 || skillId == 3211004
      ...
      if (skillId != 4001344) { // not lucky 7
        basedamage =
          (projectileWatk != 0)
            ? player.calculateMaxBaseDamage(totalWatk + projectileWatk)
            : player.getCurrentMaxBaseDamage();
      } else basedamage =
        (int) (
          ((player.getTotalLuk() * 5.0) / 100.0) * (totalWatk + projectileWatk)
        ); // l7 has a different formula :>
      ...
      if (player.getJob().isA(MapleJob.ASSASSIN)) {
        ...
        if (player.getSkillLevel(criticalthrow) > 0) critdamagerate =
          (
            criticalthrow
              .getEffect(player.getSkillLevel(criticalthrow))
              .getDamage() /
            100.0
          );
        ...
        if (critlevel > 0) critdamagerate =
          (criticalshot.getEffect(critlevel).getDamage() / 100.0) - 1.0;
      }
      ...
      if (hasShadowPartner) {
        ...
        MapleStatEffect shadowPartnerEffect = shadowPartner.getEffect(
          player.getSkillLevel(shadowPartner)
        );
        maxdamage *=
          (skillId != 0)
