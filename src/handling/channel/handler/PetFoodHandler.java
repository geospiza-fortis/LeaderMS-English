    ...
    if (c.getPlayer().haveItem(itemId, 1, false, true)) {
      ...
      if (pet.getFullness() < 100) {
        ...
        if (gainCloseness && pet.getCloseness() < 30000) {
          int newCloseness =
            pet.getCloseness() + (1 * c.getChannelServer().getPetExpRate());
          ...
          if (
            ...
            c
              .getSession()
              .write(
                PetPacket.showOwnPetLevelUp(c.getPlayer().getPetIndex(pet))
              );
            c
              .getPlayer()
              .getMap()
        ...
        if (gainCloseness) {
          ...
          pet.setCloseness(newCloseness);
          if (
            newCloseness < ExpTable.getClosenessNeededForLevel(pet.getLevel())
          ) {
            pet.setLevel(pet.getLevel() - 1);
          }
        }
