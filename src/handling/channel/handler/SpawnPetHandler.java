...
public class SpawnPetHandler extends AbstractMaplePacketHandler {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
      ...
      for (int i = 0; i < pet.length && !done; i++) {
        ...
        if (!player.haveItem(pet[petno], 1, true, true)) {
          ...
          MapleInventoryManipulator.addById(
            ...
            pet[petno],
            (short) 1,
            "Apperantly, Your Dragon egg hatched!.",
            null,
            MaplePet.createPet(pet[petno])
          );
          done = true;
        }
      }
    ...
    if (player.getPetIndex(pet) != -1) {
      ...
      if (
        player.getSkillLevel(SkillFactory.getSkill(8)) == 0 &&
        player.getPet(0) != null
      ) {
      ...
      player
        .getMap()
        .broadcastMessage(player, PetPacket.showPet(player, pet, false), true);
      int uniqueid = pet.getUniqueId();
      List<Pair<MapleStat, Integer>> stats = new ArrayList<Pair<MapleStat, Integer>>();
      stats.add(
        new Pair<MapleStat, Integer>(MapleStat.PET, Integer.valueOf(uniqueid))
      );
