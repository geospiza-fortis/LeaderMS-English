    ...
    if (!CommandProcessor.getInstance().processCommand(c, text)) {
      ...
      if (player != null) {
        player
          .getClient()
          .getSession()
          .write(MaplePacketCreator.toSpouse(c.getPlayer().getName(), text, 5));
        c
          .getSession()
          .write(MaplePacketCreator.toSpouse(c.getPlayer().getName(), text, 4));
      } else { // not found
        try {
          if (
            ChannelServer
            ...
            ChannelServer
              .getInstance(c.getChannel())
              .getWorldInterface()
              .spouse(c.getPlayer().getName(), recipient, c.getChannel(), text);
            c
              .getSession()
