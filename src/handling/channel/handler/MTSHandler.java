...
public class MTSHandler extends AbstractMaplePacketHandler {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    if (slea.available() > 0) {
      ...
      if (op == 2) { //put item up for sale
        ...
        if (itemtype == 1) slea.readInt(); else slea.readShort();
        byte slot = 1;
        short quantity = 1;
        if (itemtype != 1) slot = (byte) slea.readInt(); else quantity =
          (short) slea.readInt();

        if (itemtype == 1) slea.readShort();

        if (itemtype != 1) quantity = (short) slea.readInt(); else slot =
          (byte) slea.readInt();

        if (itemtype == 1) slea.readInt();

        int price = slea.readInt();

        if (itemtype == 1) quantity = 1;
        ...
        if (i != null && c.getPlayer().getMeso() >= 5000) {
          ...
          try {
            ...
            if (i.getType() == 2) {
              ...
              ps =
                con.prepareStatement(
                  "INSERT INTO mts_items (tab, type, itemid, quantity, seller, price, upgradeslots, level, str, dex, `int`, luk, hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, locked, owner, sellername, sell_ends) VALUES " +
                  "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                );
              ps.setInt(1, 1);
              ps.setInt(2, (int) type.getType());
          ...
          c
            .getSession()
            .write(
              MTSCSPacket.TransferInventory(getTransfer(c.getPlayer().getId()))
            );
          c
            .getSession()
            .write(
              MTSCSPacket.NotYetSoldInv(getNotYetSold(c.getPlayer().getId()))
            );
        }
      } else if (op == 3) {} else if (op == 4) { //send offer for wanted item //list wanted item
        int itemid = slea.readInt();
        int price = slea.readInt();
        ...
        if (tab == 4 && type == 0) c
          .getSession()
          .write(getCart(c.getPlayer().getId())); else c
          .getSession()
          .write(getMTS(tab, type, page));
        c.getSession().write(MTSCSPacket.enableMTS());
        c
          .getSession()
          .write(
        ...
        try {
          PreparedStatement ps = con.prepareStatement(
            "UPDATE mts_items SET transfer = 1 WHERE id = ? AND seller = ?"
          );
          ps.setInt(1, id);
        ...
        try {
          ps =
            con.prepareStatement(
              "SELECT * FROM mts_items WHERE seller = ? AND transfer = 1 ORDER BY id DESC"
            );
          ...
          if (rs.next()) {
            ...
            c
              .getSession()
              .write(
                MTSCSPacket.MTSConfirmTransfer(i.getQuantity(), i.getPosition())
              );
            c
              .getSession()
              .write(
                MTSCSPacket.TransferInventory(
        ...
        try {
          PreparedStatement ps = con.prepareStatement(
            "SELECT id FROM mts_cart WHERE cid = ? AND itemid = ?"
          );
          ps.setInt(1, id);
          ps.setInt(2, c.getPlayer().getId());
          ...
          if (!rs.next()) {
            PreparedStatement pse = con.prepareStatement(
              "INSERT INTO mts_cart (cid, itemid) VALUES (?, ?)"
            );
        ...
        try {
          PreparedStatement ps = con.prepareStatement(
            "DELETE FROM mts_cart WHERE itemid = ? AND cid = ?"
          );
          ps.setInt(1, id);
          ps.setInt(2, c.getPlayer().getId());
          ps.executeUpdate();
        ...
        int itemid = slea.readInt();
      } else if (op == 12) {} else if (op == 14) {} else if (op == 16) { //cancel wanted cart thing //buy auction item now //buy
        ...
        try {
          ps =
            con.prepareStatement(
              "SELECT * FROM mts_items WHERE id = ? ORDER BY id DESC"
            );
          ps.setInt(1, id);
          ...
          if (rs.next()) {
            int price =
              rs.getInt("price") + 110 + (int) (rs.getInt("price") * 0.1);
            if (c.getPlayer().getCSPoints(4) >= price) {
              boolean alwaysnull = true;
              for (ChannelServer cserv : ChannelServer.getAllInstances()) {
                MapleCharacter victim = cserv
                  .getPlayerStorage()
                  .getCharacterById(rs.getInt("seller"));
                if (victim != null) {
              ...
              if (alwaysnull) {
                ...
                if (rse.next()) {
                  PreparedStatement psee = con.prepareStatement(
                    "UPDATE accounts SET cardNX = cardNX + ? WHERE id = ?"
                  );
              ...
              c
                .getSession()
                .write(
                  MTSCSPacket.NotYetSoldInv(
                    getNotYetSold(c.getPlayer().getId())
                  )
                );
            } else {
  ...
  public MaplePacket getCart(int cid) {
    ...
    try {
      ps =
        con.prepareStatement(
          "SELECT * FROM mts_cart WHERE cid = ? ORDER BY id DESC"
        );
      ps.setInt(1, cid);
      ...
      while (rs.next()) {
        PreparedStatement pse = con.prepareStatement(
          "SELECT * FROM mts_items WHERE id = ?"
        );
          ...
          if (rse.getInt("type") != 1) {
            ...
            items.add(
              new MTSItemInfo(
                ...
                rse.getInt("id"),
                rse.getInt("seller"),
                rse.getString("sellername"),
                rse.getString("sell_ends")
              )
            );
          } else {
            Equip equip = new Equip(
              rse.getInt("itemid"),
              (byte) rse.getInt("position"),
              -1
            );
  ...
  public MaplePacket getMTS(int tab, int type, int page) {
    ...
    try {
      ...
      if (type != 0) ps =
        con.prepareStatement(
          "SELECT COUNT(*) FROM mts_items WHERE tab = ? AND type = ? AND transfer = 0"
        ); else ps =
        con.prepareStatement(
          "SELECT COUNT(*) FROM mts_items WHERE tab = ? AND transfer = 0"
        );
      ps.setInt(1, tab);
