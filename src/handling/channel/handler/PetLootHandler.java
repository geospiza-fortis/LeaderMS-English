...
public class PetLootHandler extends AbstractMaplePacketHandler {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    MaplePet pet = c
      .getPlayer()
      .getPet(c.getPlayer().getPetIndex(slea.readInt()));
    slea.skip(13);
    ...
    if (ob instanceof MapleMapItem) {
      ...
      synchronized (mapitem) {
        ...
        if (mapitem.getMeso() > 0 && mapitem.getDropper() != c.getPlayer()) {
          if (
            c
              ...
              .findById(1812000) !=
            null
          ) { //Evil hax until I find the right packet - Ramon
          ...
          if (ii.isPet(mapitem.getItem().getItemId())) {
            ...
            if (
              MapleInventoryManipulator.addFromDrop(
                c,
                mapitem.getItem(),
                "Picked up by " + c.getPlayer().getName(),
                true
              )
            ) {
              c
        ...
        if (mapitem.getDropper() != c.getPlayer()) mapitem.setPickedUp(true);
      }
    }
    c.getSession().write(MaplePacketCreator.enableActions());
  }
