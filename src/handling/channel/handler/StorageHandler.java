  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    if (mode == 4) { // Take out.
      ...
      if (item != null) {
        if (
          ...
          MapleInventoryManipulator.addFromDrop(
            c,
            item,
            "Taken out from storage by " + c.getPlayer().getName(),
            false
          );
        } else {
          storage.store(item);
          c
            .getSession()
            .write(
              MaplePacketCreator.serverNotice(1, "Your inventory is full")
            );
        }
        ...
        AutobanManager
          .getInstance()
          .autoban(c, "Trying to take storage item that does not exist.");
        return;
      }
      ...
      if (quantity < 1) {
        AutobanManager
          .getInstance()
          .autoban(c, "Trying to store " + quantity + " of " + itemId);
        return;
      }
      if (storage.isFull()) {
        ...
        c
          ...
          .write(
            MaplePacketCreator.serverNotice(
              1,
              "You don't have enough mesos to store this item."
            )
          );
      } else {
      ...
      c
        .getPlayer()
        .dropMessage(1, "Sorry, this storage is currently unavailable.");
    } else if (mode == 7) { // Meso.
      int meso = slea.readInt();
      int storageMesos = storage.getMeso();
      int playerMesos = c.getPlayer().getMeso();
      if (
        (meso > 0 && storageMesos >= meso) || (meso < 0 && playerMesos >= -meso)
      ) {
        ...
        AutobanManager
          ...
          .addPoints(
            ...
            1000,
            0,
            "Trying to store or withdraw more mesos than available (" +
            meso +
            "/" +
            storage.getMeso() +
            "/" +
