...
public class SummonDamageHandler extends AbstractMaplePacketHandler {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    for (MapleSummon sum : summons) if (sum.getObjectId() == oid) summon = sum;
    if (summon == null) {
      System.out.println(
        MapleClient.getLogMessage(c, "Using summon attack without a summon")
      );
      return;
    }
    ...
    for (SummonAttackEntry attackEntry : allDamage) {
      ...
      MapleMonster target = player
        .getMap()
        .getMonsterByOid(attackEntry.getMonsterOid());
      if (target != null) {
        if (damage > 0 && summonEffect.getMonsterStati().size() > 0) if (
