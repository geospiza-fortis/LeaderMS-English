...
public class GuildOperationHandler extends AbstractMaplePacketHandler {
  ...
  private void respawnPlayer(MapleCharacter mc) {
    mc
      ...
      .broadcastMessage(
        mc,
        MaplePacketCreator.removePlayerFromMap(mc.getId()),
        false
      );
    mc
      .getMap()
      .broadcastMessage(mc, MaplePacketCreator.spawnPlayerMapobject(mc), false);
    if (mc.hasBattleShip()) {
      mc
        .getMap()
        .broadcastMessage(
          mc,
          MaplePacketCreator.showBuffeffect(mc.getId(), 52210006, 1, (byte) 3),
          false
        );
    }
    if (mc.getNoPets() > 0) {
      for (MaplePet pet : mc.getPets()) {
        mc
          ...
          .broadcastMessage(
            mc,
            PetPacket.showPet(mc, pet, false, false),
            false
          );
      }
    }
  ...
  private org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(
    this.getClass()
  );
    ...
    switch (type) {
      case 0x02:
        ...
        if (mc.getMeso() < MapleGuild.CREATE_GUILD_COST) {
          c
            ...
            .write(
              MaplePacketCreator.serverNotice(
                1,
                "You do not have enough mesos to create a Guild."
              )
            );
          return;
        }
        ...
        try {
          gid =
            c
              .getChannelServer()
              .getWorldInterface()
              .createGuild(mc.getId(), guildName);
        ...
        if (gid == 0) {
          c
            .getSession()
            .write(MaplePacketCreator.genericGuildMessage((byte) 0x1c));
          return;
        }
        ...
        c
          ...
          .write(
            MaplePacketCreator.serverNotice(
              1,
              "You have successfully created a Guild."
            )
          );
        respawnPlayer(mc);
        break;
      case 0x05:
        ...
        if (mc.getGuildId() <= 0 || mc.getGuildRank() > 2) { // 1 = guild master, 2 = jr.
          System.out.println(
            "[hax] " +
            mc.getName() +
            " used guild invitation when s/he isn't allowed."
          );
          return;
        }
        String name = slea.readMapleAsciiString();
        MapleGuildResponse mgr = MapleGuild.sendInvite(c, name);
        if (mgr != null) c.getSession().write(mgr.getPacket()); else {
      ...
      case 0x06:
        ...
        if (mc.getGuildId() > 0) {
          System.out.println(
            "[hax] " +
            mc.getName() +
            " attempted to join a guild when s/he is already in one."
          );
          return;
        }
        gid = slea.readInt();
        ...
        if (cid != mc.getId()) {
          System.out.println(
            "[hax] " +
            mc.getName() +
            " attempted to join a guild with a different character id."
          );
          return;
        }
        ...
        try {
          s =
            c
              .getChannelServer()
              .getWorldInterface()
              .addGuildMember(mc.getMGC());
        } catch (java.rmi.RemoteException e) {
          System.out.println(
            "RemoteException occurred while attempting to add character to guild" +
            e
          );
          c
            .getSession()
      ...
      case 0x07:
        ...
        if (
          cid != mc.getId() ||
          !name.equals(mc.getName()) ||
          mc.getGuildId() <= 0
        ) {
          System.out.println(
            "[hax] " +
            mc.getName() +
            " tried to quit guild under the name \"" +
            name +
            "\" and current guild id of " +
            mc.getGuildId() +
            "."
          );
          return;
        ...
        try {
          ...
          System.out.println(
            "RemoteException occurred while attempting to leave guild" + re
          );
          c
            .getSession()
            .write(
      ...
      case 0x08:
        ...
        if (mc.getGuildRank() > 2 || mc.getGuildId() <= 0) {
          System.out.println(
            "[hax] " + mc.getName() + " is trying to expel without rank 1 or 2."
          );
          return;
        }
        try {
          c
            .getChannelServer()
            .getWorldInterface()
            .expelMember(mc.getMGC(), name, cid);
        } catch (java.rmi.RemoteException re) {
          System.out.println(
            "RemoteException occurred while attempting to change rank" + re
          );
          c
            .getSession()
            .write(
      ...
      case 0x0d:
        ...
        for (int i = 0; i < 5; i++) ranks[i] = slea.readMapleAsciiString();
        try {
          c
            .getChannelServer()
            .getWorldInterface()
            .changeRankTitle(mc.getGuildId(), ranks);
        } catch (java.rmi.RemoteException re) {
          System.out.println("RemoteException occurred" + re);
          c
            .getSession()
      ...
      case 0x0e:
        ...
        if (
          mc.getGuildRank() > 2 ||
          (newRank <= 2 && mc.getGuildRank() != 1) ||
          mc.getGuildId() <= 0
        ) {
          System.out.println(
            "[hax] " +
            mc.getName() +
            " is trying to change rank outside of his/her permissions."
          );
          return;
        }
        ...
        try {
          c
            .getChannelServer()
            .getWorldInterface()
            .changeRank(mc.getGuildId(), cid, newRank);
        } catch (java.rmi.RemoteException re) {
          System.out.println(
            "RemoteException occurred while attempting to change rank" + re
          );
      ...
      case 0x0f:
        ...
        if (
          mc.getGuildId() <= 0 ||
          mc.getGuildRank() != 1 ||
          mc.getMapId() != 200000301
        ) {
          System.out.println(
            "[hax] " +
            mc.getName() +
            " tried to change guild emblem without being the guild leader."
          );
          return;
        }
        ...
        try {
          c
            .getChannelServer()
            .getWorldInterface()
            .setGuildEmblem(mc.getGuildId(), bg, bgcolor, logo, logocolor);
        } catch (java.rmi.RemoteException re) {
          System.out.println("RemoteException occurred" + re);
          c
            .getSession()
      ...
      case 0x10:
        ...
        if (mc.getGuildId() <= 0 || mc.getGuildRank() > 2) {
          System.out.println(
            "[hax] " +
            mc.getName() +
            " tried to change guild notice while not in a guild."
          );
          return;
        }
        ...
        try {
          c
            .getChannelServer()
            .getWorldInterface()
            .setGuildNotice(mc.getGuildId(), notice);
        } catch (java.rmi.RemoteException re) {
          System.out.println("RemoteException occurred" + re);
          c
            .getSession()
