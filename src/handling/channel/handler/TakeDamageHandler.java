...
public class TakeDamageHandler extends AbstractMaplePacketHandler {
  ...
  public final void handlePacket(
    ...
    if (damagefrom != -2) {
      ...
      if (
        (
          player.getMap().getMonsterById(monsteridfrom) == null ||
          attacker == null
        ) &&
            ...
            for (int i = 0; i < amount; i++) {
              ...
              c
                ...
                .spawnItemDrop(
                  ...
                  c.getPlayer(),
                  new Item(4031868, (byte) 0, (short) 1),
                  position,
                  true,
                  true
                );
    ...
    if (damagefrom != -1 && damagefrom != -2 && attacker != null) {
      MobAttackInfo attackInfo = MobAttackInfoFactory.getMobAttackInfo(
        attacker,
        damagefrom
      );
          ...
          if (jobid == 212 || jobid == 222 || jobid == 232) {
            ...
            if (
              ...
              player
                .getClient()
                .getSession()
                .write(MaplePacketCreator.showOwnBuffEffect(id, 5));
              player
                .getMap()
                .broadcastMessage(
                  player,
                  MaplePacketCreator.showBuffeffect(player.getId(), id, 5),
                  false
                );
            }
          }
    ...
    if (damage > 0 && !player.isHidden()) {
      if (attacker != null && !attacker.isBoss()) {
        if (
          damagefrom == -1 &&
          player.getBuffedValue(MapleBuffStat.POWERGUARD) != null
        ) {
          int bouncedamage = (int) (
            damage *
            (
              player.getBuffedValue(MapleBuffStat.POWERGUARD).doubleValue() /
              100
            )
          );
      ...
      if (damagefrom != -2) {
        ...
        if (jobid < 200 && jobid % 10 == 2) {
          achilles1 =
            SkillFactory.getSkill(jobid * 10000 + jobid == 112 ? 4 : 5);
          achilles = player.getSkillLevel(achilles);
        }
        if (achilles != 0 && achilles1 != null) {
          damage *=
            (int) (achilles1.getEffect(achilles).getX() / 1000.0 * damage);
        }
      }
      ...
      if (
        ...
        int mploss = (int) (
          damage *
          (
            player.getBuffedValue(MapleBuffStat.MAGIC_GUARD).doubleValue() /
            100.0
          )
        );
