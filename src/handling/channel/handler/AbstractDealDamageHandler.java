...
public abstract class AbstractDealDamageHandler
  ...
  public class AttackInfo {
    ...
    private MapleStatEffect getAttackEffect(
      MapleCharacter chr,
      ISkill theSkill
    ) {
  ...
  protected synchronized void applyAttack(
    AttackInfo attack,
    ...
    if (attack.skill != 0) {
      ...
      if (attack.skill != 2301002 || attack.skill != 9101000) {
        ...
        SkillFactory.getSkill(attack.skill).isGMSkill() && !player.isGM()
      ) {
    ...
    if (
      ...
      attack.skill == 2121007 &&
      attack.skill == 1311006 &&
      attack.skill == 2221007 &&
      attack.skill == 2321008 &&
      player.getMapId() == 107000200
    ) {
      player
        .getClient()
        .getSession()
        .write(
          MaplePacketCreator.serverNotice(
            5,
            "[CashPQ] This skill is disabled on this map!"
          )
        );
      return;
    }
    ...
    if (attack.skill == 4211006) { // meso explosion
      for (Pair<Integer, List<Integer>> oned : attack.allDamage) {
        ...
        if (
          mapobject != null && mapobject.getType() == MapleMapObjectType.ITEM
        ) {
          ...
          mapobject != null && mapobject.getType() != MapleMapObjectType.MONSTER
        ) {
          player
            .getCheatTracker()
    ...
    for (Pair<Integer, List<Integer>> oned : attack.allDamage) {
      ...
      if (monster != null) {
          ...
          case 1221011: //sanctuary
            if (attack.isHH) {
              ...
              int HHDmg = (int) (
                ...
                (
                  theSkill
                    .getEffect(player.getSkillLevel(theSkill))
                    .getDamage() /
                  100
                )
              );
              HHDmg =
                (int) (
                  Math.floor(
                    Math.random() * (HHDmg - HHDmg * .80) + HHDmg * .80
                  )
                );
          ...
          case 5111004: // energy drain.
            int gainhp = (int) (
              ...
              (double) SkillFactory
                ...
                .getEffect(
                  player.getSkillLevel(SkillFactory.getSkill(attack.skill))
                )
                .getX() /
              100.0
            );
            gainhp =
              Math.min(
                monster.getMaxHp(),
                Math.min(gainhp, player.getMaxHp() / 2)
              );
            player.addHP(gainhp);
            break;
          default:
              ...
              if (player.getBuffedValue(MapleBuffStat.BLIND) != null) {
                if (
                  SkillFactory
                    .getSkill(3221006)
                    .getEffect(
                      player.getSkillLevel(SkillFactory.getSkill(3221006))
                    )
                    .makeChanceResult()
                ) {
              ...
              if (player.getBuffedValue(MapleBuffStat.HAMSTRING) != null) {
                if (
                  SkillFactory
                    .getSkill(3121007)
                    .getEffect(
                      player.getSkillLevel(SkillFactory.getSkill(3121007))
                    )
                    .makeChanceResult()
                ) {
                  MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(
                    ...
                    if (
                      ...
                      monster.applyStatus(
                        ...
                        SkillFactory
                          .getSkill(charge)
                          .getEffect(
                            player.getSkillLevel(SkillFactory.getSkill(charge))
                          )
                          .getY() *
                        2000
                      );
        ...
        if (player.getSkillLevel(SkillFactory.getSkill(4120005)) > 0) {
          MapleStatEffect venomEffect = SkillFactory
            .getSkill(4120005)
            .getEffect(player.getSkillLevel(SkillFactory.getSkill(4120005)));
          ...
          MapleStatEffect venomEffect = SkillFactory
            .getSkill(4220005)
            .getEffect(player.getSkillLevel(SkillFactory.getSkill(4220005)));
          for (int i = 0; i < attackCount; i++) {
            if (venomEffect.makeChanceResult()) {
              if (monster.getVenomMulti() < 3) {
                monster.setVenomMulti((monster.getVenomMulti() + 1));
        ...
        if (
          ...
          attackEffect != null &&
          attackEffect.getMonsterStati().size() > 0
        ) {
          if (attackEffect.makeChanceResult()) {
            MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(
              attackEffect.getMonsterStati(),
              theSkill,
              false
            );
            monster.applyStatus(
    ...
    if (totDamage > 1) {
      player
        ...
        .setAttacksWithoutHit(
          player.getCheatTracker().getAttacksWithoutHit() + 1
        );
    ...
    if (!player.isGM()) {
      ...
      if (player.getBuffedValue(MapleBuffStat.WK_CHARGE) != null) {
        ...
        maximumDamageToMonster *=
          chargeSkill.getEffect(player.getSkillLevel(chargeSkill)).getDamage() /
          100.0;
      }
        ...
        switch (monster.getEffectiveness(element)) {
          ...
          case WEAK:
            elementalMaxDamagePerMonster =
              (int) (maximumDamageToMonster * (1.0 + elementalEffect));
            break;
          case STRONG:
            elementalMaxDamagePerMonster =
              (int) (maximumDamageToMonster * (1.0 - elementalEffect));
  ...
  private void handlePickPocket(
    MapleCharacter player,
    MapleMonster monster,
    Pair<Integer, List<Integer>> oned
  ) {
    int delay = 0;
    ...
    for (Integer eachd : oned.getRight()) {
      if (
        SkillFactory
          .getSkill(4211003)
          .getEffect(player.getSkillLevel(SkillFactory.getSkill(4211003)))
          .makeChanceResult()
      ) {
        double perc = (double) eachd / (double) reqdamage;

        final int todrop = Math.min(
          (int) Math.max(perc * (double) maxmeso, (double) 1),
          maxmeso
        );
  ...
  public AttackInfo parseDamage(LittleEndianAccessor lea, boolean ranged) {
    ...
    for (int i = 0; i < ret.numAttacked; i++) {
      ...
      ret.allDamage.add(
        new Pair<Integer, List<Integer>>(Integer.valueOf(oid), allDamageNumbers)
      );
    }
    return ret;
  }

  public AttackInfo parseMesoExplosion(
    ...
    if (ret.numAttackedAndDamage == 0) {
      ...
      for (int j = 0; j < bullets; j++) {
        ...
        lea.skip(1);
        ret.allDamage.add(
          new Pair<Integer, List<Integer>>(Integer.valueOf(mesoid), null)
        );
      }
      return ret;
    } else {
