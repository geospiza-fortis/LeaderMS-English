...
public class CloseRangeDamageHandler extends AbstractDealDamageHandler {
  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    ...
    if (
      skillId != 0 &&
      skill.getEffect(c.getPlayer().getSkillLevel(skill)).getCooldown() > 0
    ) {
    ...
    if (
      energyChargeSkillLevel > 0 &&
      player.getJob().equals(MapleJob.BUCCANEER) ||
      player.getJob().equals(MapleJob.MARAUDER)
    ) {
    ...
    if (attack.numAttacked > 0 && attack.skill == 1311005) {
      int totDamageToOneMonster = attack.allDamage
        .get(0)
        .getRight()
        .get(0)
        .intValue(); // sacrifice attacks only 1 mob with 1 attack
      int remainingHP =
        player.getHp() -
        totDamageToOneMonster *
        attack.getAttackEffect(player).getX() /
        100;
      if (remainingHP > 1) {
    ...
    if (attack.numAttacked > 0 && attack.skill == 1211002) {
      ...
      int advcharge_level = player.getSkillLevel(
        SkillFactory.getSkill(1220010)
      );
      if (advcharge_level > 0) {
        MapleStatEffect advcharge_effect = SkillFactory
          .getSkill(1220010)
    ...
    if (skillId == 4211006) {
      ...
      maxdamage *=
        (double) 1.0 +
        (
          combo.getEffect(player.getSkillLevel(combo)).getDamage() / 100.0 - 1.0
        ) *
