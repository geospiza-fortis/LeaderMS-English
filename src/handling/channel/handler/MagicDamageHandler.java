  ...
  public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
    if (c.getPlayer().getMap().getDisableDamage() && !c.getPlayer().isGM()) {
      c
        .getSession()
        .write(
          MaplePacketCreator.serverNotice(5, "Attacking is currently disabled.")
        );
    ...
    int maxdamage = (int) (
      ((player.getTotalMagic() * 0.8) + (player.getLuk() / 4) / 18) *
      skill.getEffect(player.getSkillLevel(skill)).getDamage() *
      0.8 *
      (player.getMasterLevel(skill) * 10 / 100)
    );
    ...
    applyAttack(
      ...
      player,
      maxdamage,
      attack.getAttackEffect(player).getAttackCount()
    );
      ...
      if (eaterLevel > 0) {
        for (Pair<Integer, List<Integer>> singleDamage : attack.allDamage) {
          eaterSkill
            ...
            .applyPassive(
              player,
              player.getMap().getMapObject(singleDamage.getLeft()),
              0
            );
        }
        break;
