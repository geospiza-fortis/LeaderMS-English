...
public class ChannelWorldInterfaceImpl
  extends UnicastRemoteObject
  implements ChannelWorldInterface {
  ...
  public ChannelWorldInterfaceImpl(ChannelServer server)
  ...
  public void spouse(String sender, String target, int channel, String message)
    throws RemoteException {
    if (isConnected(target)) {
      server
        .getPlayerStorage()
        ...
        .getClient()
        .getSession()
        .write(MaplePacketCreator.toSpouse(sender, message, 5));
    }
  }

  public boolean isConnected(String charName) throws RemoteException {
    MapleCharacter check = server
      .getPlayerStorage()
  ...
  public void shutdown(int time) throws RemoteException {
    server.broadcastPacket(
      MaplePacketCreator.serverNotice(
        0,
        "O servidor vai ser desligado em " +
        (time / 60000) +
        " minuto(s), por favor, fazer logoff com seguranca."
      )
    );
    TimerManager
      .getInstance()
      .schedule(new ShutdownServer(server.getChannel()), time);
  }
  ...
  public void partyChat(MapleParty party, String chattext, String namefrom)
    ...
    for (MaplePartyCharacter partychar : party.getMembers()) {
      if (
        partychar.getChannel() == server.getChannel() &&
        ...
        if (chr != null) {
          chr
            .getClient()
            .getSession()
            .write(MaplePacketCreator.multiChat(namefrom, chattext, 1));
        }
      }
    }
  }
  ...
  public List<CheaterData> getCheaters() throws RemoteException {
    ...
    List<MapleCharacter> allplayers = new ArrayList<MapleCharacter>(
      server.getPlayerStorage().getAllCharacters()
    );
  ...
  @Override
  public BuddyAddResult requestBuddyAdd(
    String addName,
    int channelFrom,
    int cidFrom,
    String nameFrom
  ) {
    MapleCharacter addChar = server
      .getPlayerStorage()
  ...
  public void sendPacket(
    ...
    for (int i : targetIds) {
      ...
      if (c != null) c.getClient().getSession().write(packet);
    }
  }

  @Override
  public void setGuildAndRank(
    List<Integer> cids,
    int guildid,
    int rank,
    int exception
  ) throws RemoteException {
    for (int cid : cids) if (cid != exception) setGuildAndRank(
      cid,
      guildid,
      rank
    );
  ...
  public void setGuildAndRank(int cid, int guildid, int rank)
    ...
    if (guildid == -1 && rank == -1) bDifferentGuild = true; else { //just need a respawn
    ...
    if (bDifferentGuild) {
      mc
        ...
        .broadcastMessage(
          mc,
          MaplePacketCreator.removePlayerFromMap(cid),
          false
        );
      mc
        .getMap()
        .broadcastMessage(
          mc,
          MaplePacketCreator.spawnPlayerMapobject(mc),
          false
        );
      ...
      for (int i = 0; i < 3; i++) {
        if (pets[i] != null) {
          mc
            ...
            .broadcastMessage(
              mc,
              PetPacket.showPet(mc, pets[i], false, false),
              false
            );
        }
      }
  ...
  public void reloadGuildCharacters() throws RemoteException {
    for (MapleCharacter mc : server.getPlayerStorage().getAllCharacters()) {
      if (mc.getGuildId() > 0) {
        ...
        server
          .getWorldInterface()
          .setGuildMemberOnline(mc.getMGC(), true, server.getChannel());
  ...
  public void messengerInvite(
    ...
    if (isConnected(target)) {
      MapleMessenger messenger = server
        .getPlayerStorage()
        .getCharacterByName(target)
        .getMessenger();
      if (messenger == null) {
        ...
        MapleCharacter from = ChannelServer
  ...
  public void addMessengerPlayer(
      ...
      if (
        ...
        if (chr != null) {
          chr
            ...
            .getSession()
            .write(
              MaplePacketCreator.joinMessenger(messengerchar.getPosition())
            );
        }
      }
    }
  ...
  public void broadcastSMega(String sender, byte[] message)
    throws RemoteException {
    MaplePacket packet = new ByteArrayMaplePacket(message);
