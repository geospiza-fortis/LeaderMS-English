...
public class ChannelServer implements Runnable, ChannelServerMBean {
        ...
        synchronized (wci) {
          try {
            ...
            FileReader fr = new FileReader(
              System.getProperty("channel.config")
            );
            initialProp.load(fr);
            fr.close();
            Registry registry = LocateRegistry.getRegistry(
              initialProp.getProperty("world.host"),
            ...
            QuestExpRate =
              Integer.parseInt(props.getProperty("world.questExp"));
            mesoRate = Integer.parseInt(props.getProperty("world.meso"));
            dropRate = Integer.parseInt(props.getProperty("world.drop"));
            bossdropRate =
              Integer.parseInt(props.getProperty("world.bossdrop"));
            petExpRate = Integer.parseInt(props.getProperty("world.petExp"));
            mountExpRate =
              Integer.parseInt(props.getProperty("world.mountExp"));
            serverMessage = props.getProperty("world.serverMessage");
            dropUndroppables =
              Boolean.parseBoolean(props.getProperty("world.alldrop", "false"));
            moreThanOne =
              Boolean.parseBoolean(
                props.getProperty("world.morethanone", "false")
              );
            gmWhiteText =
              Boolean.parseBoolean(
                props.getProperty("world.gmWhiteText", "true")
              );
            cashshop =
              Boolean.parseBoolean(
                props.getProperty("world.cashshop", "false")
              );
  ...
  public void run() {
    try {
      ...
      dropUndroppables =
        Boolean.parseBoolean(props.getProperty("world.alldrop", "false"));
      moreThanOne =
        Boolean.parseBoolean(props.getProperty("world.morethanone", "false"));
      eventSM =
        new EventScriptManager(
          this,
          props.getProperty("channel.events").split(",")
        );
      gmWhiteText =
        Boolean.parseBoolean(props.getProperty("world.gmWhiteText", "false"));
      cashshop =
        Boolean.parseBoolean(props.getProperty("world.cashshop", "false"));
      mts = Boolean.parseBoolean(props.getProperty("world.mts", "false"));
    ...
    cfg
      .getFilterChain()
      .addLast("codec", new ProtocolCodecFilter(new MapleCodecFactory()));
    ...
    try {
      final MapleServerHandler serverHandler = new MapleServerHandler(
      ...
      tMan.schedule(
        new Runnable() {
          public void run() {
            serv.broadcastPacket(
              MaplePacketCreator.serverNotice(
                6,
                "[System Message] " +
                Configuration.botMessages[(int) (
                    Math.random() * Configuration.botMessages.length
                  )]
              )
            );
            tMan.schedule(
      ...
      log.error(
        "Connection at port " + port + " failed (ch: " + getChannel() + ")" + e
      );
    }
  ...
  public List<MapleCharacter> getPartyMembers(MapleParty party) {
    ...
    for (MaplePartyCharacter partychar : party.getMembers()) {
      if (partychar.getChannel() == getId()) {
        MapleCharacter chr = getPlayerStorage()
  ...
  private final class ShutDown implements Runnable {
    ...
    public void run() {
      ...
      MapleCharacter chrs[] = allchars.toArray(
        new MapleCharacter[allchars.size()]
      );
  ...
  public void broadcastGMPacket(MaplePacket data) {
    for (MapleCharacter chr : players.getAllCharacters()) {
      if (chr.isGM()) chr.getClient().getSession().write(data);
    }
  }
  ...
  private static ChannelServer newInstance(String key)
    ...
    mBeanServer.registerMBean(
      instance,
      new ObjectName(
        "net.channel:type=ChannelServer,name=ChannelServer" + uniqueID++
      )
    );
  ...
  public final void addPlayer(final MapleCharacter chr) {
    ...
    chr
      .getClient()
      .getSession()
      .write(MaplePacketCreator.serverMessage(serverMessage));
  }
  ...
  public void setChannel(int channel) {
    if (pendingInstances.containsKey(key)) pendingInstances.remove(key);
    if (instances.containsKey(channel)) instances.remove(channel);
  ...
  public void shutdown(int time) {
    broadcastPacket(
      MaplePacketCreator.serverNotice(
        0,
        "The server will shut down in under " +
        (time / 60000) +
  ...
  public MapleGuild getGuild(MapleGuildCharacter mgc) {
    ...
    if (gsStore.get(gid) == null) gsStore.put(gid, new MapleGuildSummary(g));

    return g;
  }

  public MapleGuildSummary getGuildSummary(int gid) {
    if (gsStore.containsKey(gid)) return gsStore.get(gid); else { //this shouldn't happen much, if ever, but if we're caught
      //without the summary, we'll have to do a worldop
      try {
        MapleGuild g = this.getWorldInterface().getGuild(gid, null);
        if (g != null) gsStore.put(gid, new MapleGuildSummary(g));
  ...
  public void reloadGuildSummary() {
    try {
      ...
      for (int i : gsStore.keySet()) {
        ...
        if (g != null) gsStore.put(
          i,
          new MapleGuildSummary(g)
        ); else gsStore.remove(i);
      }
    } catch (RemoteException re) {
  ...
  public static void main(String args[])
    throws FileNotFoundException, IOException, NotBoundException, InstanceAlreadyExistsException, MBeanRegistrationException, NotCompliantMBeanException, MalformedObjectNameException {
    initialProp = new Properties();
    ...
    for (
      int i = 0;
      i < Integer.parseInt(initialProp.getProperty("channel.count", "0"));
      i++
    ) {
            ...
            for (ChannelServer channel : getAllInstances()) {
              for (int i = 910000001; i <= 910000022; i++) {
                if (channel.getMapFactory().isMapLoaded(i)) {
                  ...
                  for (MapleMapObject obj : m.getMapObjectsOfType(
