...
public class NPCConversationManager extends AbstractPlayerInteraction {
  ...
  public void sendNext(String text) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalk(npc, (byte) 0, text, "00 01"));
  }

  public void sendPrev(String text) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalk(npc, (byte) 0, text, "01 00"));
  }

  public void sendNextPrev(String text) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalk(npc, (byte) 0, text, "01 01"));
  }

  public void sendOk(String text) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalk(npc, (byte) 0, text, "00 00"));
  }

  public void sendYesNo(String text) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalk(npc, (byte) 1, text, ""));
  }

  public void sendAcceptDecline(String text) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalk(npc, (byte) 0x0C, text, ""));
  }

  public void sendSimple(String text) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalk(npc, (byte) 4, text, ""));
  }

  public void sendStyle(String text, int styles[]) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalkStyle(npc, text, styles));
  }

  public void sendGetNumber(String text, int def, int min, int max) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalkNum(npc, text, def, min, max));
  }

  public void sendGetText(String text) {
    getClient()
      .getSession()
      .write(MaplePacketCreator.getNPCTalkText(npc, text));
  }
  ...
  public void unequipEverything() {
    MapleInventory equipped = getPlayer()
  ...
  public void warpRandom(int mapid) {
    ...
    MaplePortal portal = target.getPortal(
  ...
  public void gainExpCard() {
    ...
    IItem ret = MapleItemInformationProvider
      .getInstance()
      .getEquipById(5211048);
    ...
    c
      ...
      .write(
        MaplePacketCreator.sendDuey(
          (byte) 8,
          DueyHandler.loadItems(this.getPlayer())
        )
      );
  }
  ...
  public void addItemToMarket(int itemid, int quantity, int price) {
    this.c.getChannelServer()
      .getMarket()
      .addItem(itemid, quantity, price, c.getPlayer().getId());
  }

  public void editEquipById(
    int input,
    byte stat,
    short value,
    boolean supergacha
    ...
    if (supergacha) {
      MapleInventoryManipulator.editEquipById(
        getPlayer(),
        input,
        stat,
        value,
  ...
  public void reloadChar() {
    getPlayer()
      .getClient()
      .getSession()
      .write(MaplePacketCreator.getCharInfo(getPlayer()));
  ...
  public byte gainItemRetPos(int itemid) {
    ...
    c
      .getSession()
      .write(MaplePacketCreator.addInventorySlot(MapleInventoryType.EQUIP, it));
    c
      .getSession()
      .write(MaplePacketCreator.getShowItemGain(itemid, (short) 1, true));
  ...
  public void changeOccupationById(int occ) {
    ...
    getPlayer()
      .sendServerNotice(
        "<Job Leader> Congratulations " +
        getPlayer().getName() +
        ", you are now a " +
        getPlayer().getOccupation() +
        "."
      );
  }
  ...
  public void removeItemFromMarket(int itemid, int quantity) {
    this.c.getChannelServer()
      .getMarket()
      .removeItem(itemid, quantity, c.getPlayer().getId());
  }
  ...
  public void showInventory(int type) {
    String send = "";
    MapleInventory invy = c
      .getPlayer()
      .getInventory(MapleInventoryType.getByType((byte) type));
  ...
  public String getMarket() {
    ...
    for (ItemEntry ie : me.getItems()) {
      if (ie.getOwner() == c.getPlayer().getId()) continue; //Don't let him see their own items
      ret +=
        "#L" +
        count +
        "##v" +
  ...
  public String getMarketRetrival() {
    ...
    int count = 0;
    for (ItemEntry ie : me.getItems()) {
      if (ie.getOwner() != c.getPlayer().getId()) continue; //Only own items
      ret +=
        "#L" +
        count +
        "##v" +
  ...
  public List<ItemEntry> getMarketItems() {
    ...
    synchronized (c.getChannelServer().getMarket().getItems()) {
      for (ItemEntry ie : c.getChannelServer().getMarket().getItems()) {
        if (ie.getOwner() != c.getPlayer().getId()) ret.add(ie);
      }
    }
    return ret;
  }

  public void warpSquadMembersClock(
    MapleSquadType type,
    int mapId,
    int clock,
    int mapExit
  ...
  public void gainPQPoints(int gain) {
    ...
    getPlayer()
      .dropMessage(
        ...
        " Quest] Quest complete successfully. You now have (" +
        getPlayer().getpqPoints() +
        ") Q.Points!"
      );
  }
  ...
  public void startCPQ(final MapleCharacter challenger, int field) {
    try {
      if (challenger != null) {
        if (challenger.getParty() == null) throw new RuntimeException(
          "Challenger's party was null!"
        );
        for (MaplePartyCharacter mpc : challenger.getParty().getMembers()) {
          ...
          mc =
            c
              .getChannelServer()
              .getPlayerStorage()
              .getCharacterByName(mpc.getName());
          if (mc != null) {
            mc.changeMap(
              c.getPlayer().getMap(),
      ...
      for (MaplePartyCharacter mpc : c.getPlayer().getParty().getMembers()) {
        ...
        if (mc != null) {
          ...
          mc
            .getClient()
            .getSession()
            .write(MaplePacketCreator.getClock(3 * 60));
        }
      }
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  public void challengeParty(int field) {
    ...
    if (leader != null) {
      if (!leader.isChallenged()) {
        ...
        for (MaplePartyCharacter player : c
          .getPlayer()
          .getParty()
          .getMembers()) {
          party.add(player);
        ...
        NPCScriptManager
          .getInstance()
          .start("cpqchallenge", leader.getClient(), npc, party);
      } else {
  ...
  public List<MapleCharacter> getPartyMembers() {
    ...
    for (ChannelServer channel : ChannelServer.getAllInstances()) {
      for (MaplePartyCharacter a : getPlayer().getParty().getMembers()) {
        MapleCharacter ch = channel
          .getPlayerStorage()
          .getCharacterByName(a.getName());
  ...
  public int getHiredMerchantMesos() {
    ...
    try {
      PreparedStatement ps = con.prepareStatement(
        "SELECT MerchantMesos FROM characters WHERE id = ?"
      );
  ...
  public boolean getDonationItems(boolean tempTable) {
    ...
    try {
      PreparedStatement ps = con.prepareStatement(
        "SELECT * FROM donationitems" + Table + " WHERE ownerid = ?"
      );
      ps.setInt(1, getPlayer().getId());
      ResultSet rs = ps.executeQuery();
      while (rs.next()) {
        ...
        if (!getPlayer().getInventory(MapleInventoryType.EQUIP).isFull()) {
          MapleInventoryManipulator.addFromDrop(
            c,
            ii.randomizeStats((Equip) spItem),
            "Donation Item"
          );
  ...
  public boolean createPlayerNPC() {
    try {
      ...
      try {
        ...
        Logger
          .getLogger(NPCConversationManager.class.getName())
