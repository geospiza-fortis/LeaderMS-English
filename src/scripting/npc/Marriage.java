...
public class Marriage {
  ...
  public static void createMarriage(
    ...
    try {
      ...
      log.warn(
        "Problem marrying " + player.getName() + " and " + partner.getName(),
        ex
      );
    }
  }
  ...
  public static void divorceEngagement(
    MapleCharacter player,
    MapleCharacter partner
  ) {
    try {
      ...
      PreparedStatement get = con.prepareStatement(
        "SELECT FROM engagements WHERE husbandid = ?"
      );
      get.setInt(1, pid);
      ResultSet rs = get.executeQuery();

      PreparedStatement ps = con.prepareStatement(
        "DELETE FROM engagements WHERE husbandid = ?"
      );
      if (rs.next()) {
        ps.setInt(1, pid);
      } else {
      ...
      log.warn(
        "Problem breaking engagement with " +
        player.getName() +
        " and " +
        partner.getName(),
        ex
  ...
  public static void divorceMarriage(
    ...
    try {
      ...
      PreparedStatement get = con.prepareStatement(
        "SELECT partnerid FROM characters WHERE id = ?"
      );
      get.setInt(1, player.getId());
      ResultSet rs = get.executeQuery();
      if (rs.next()) {
        PreparedStatement ps = con.prepareStatement(
          "DELETE FROM marriages WHERE husbandid = ?"
        );
        ps.setInt(1, pid);
        ps.executeUpdate();
        PreparedStatement ps1 = con.prepareStatement(
          "UPDATE characters SET married = 0 WHERE id = ?, and WHERE id = ?"
        );
        ps1.setInt(1, player.getId());
        ps1.setInt(2, partner.getId());
        ps1.executeUpdate();
      ...
      log.warn(
        "Problem divorcing" + player.getName() + " and " + partner.getName(),
        ex
      );
    }
  }
