...
public class ReactorActionManager extends AbstractPlayerInteraction {
  ...
  public void dropItems(
    ...
    List<ReactorDropEntry> chances = ReactorScriptManager
      .getInstance()
      .getDrops(this.reactor.getReactorId());
    ...
    while (iter.hasNext()) {
      ...
      if (
        Math.random() < (1.0D / d.chance) &&
        (
          (d.questid <= 0) ||
          (
            getPlayer()
              .getQuest(MapleQuest.getInstance(d.questid))
              .getStatus() ==
    ...
    for (ReactorDropEntry d : items) {
      if (d.itemId == 0) {
        ...
        int mesoDrop = (int) (
          displayDrop *
          ChannelServer.getInstance(getClient().getChannel()).getMesoRate()
        );
        reactor
          .getMap()
          .spawnMesoDrop(
        ...
        reactor
          .getMap()
          .spawnItemDrop(reactor, getPlayer(), drop, dropPos, false, true);
      }
      dropPos.x += 25;
    }
  }
