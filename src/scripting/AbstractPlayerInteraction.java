...
public class AbstractPlayerInteraction {
  ...
  private MapleMap getWarpMap(int map) {
    ...
    if (getPlayer().getEventInstance() == null) {
      target =
        ChannelServer.getInstance(c.getChannel()).getMapFactory().getMap(map);
    } else {
  ...
  public MapleGuild getGuild() {
    try {
      return c
        .getChannelServer()
        .getWorldInterface()
        .getGuild(getPlayer().getGuildId(), null);
    } catch (RemoteException ex) {
      Logger
        .getLogger(AbstractPlayerInteraction.class.getName())
  ...
  public boolean isLeader() {
    return (
      getParty().getLeader().equals(new MaplePartyCharacter(c.getPlayer()))
    );
  }

  //PQ methods: give items/exp to all party members

  public void givePartyItems(
    int id,
    short quantity,
  ...
  public void givePartyQPoints(int amount, List<MapleCharacter> party) {
    for (MapleCharacter chr : party) {
      ...
      chr.dropMessage(
        ...
        " Quest] Completed quest successfully. You now have (" +
        chr.getpqPoints() +
        ") Q.Points!"
      );
    }
  ...
  public void removeFromParty(int id, List<MapleCharacter> party) {
    for (MapleCharacter chr : party) {
      ...
      if (possesed > 0) {
        ...
        cl.announce(
          MaplePacketCreator.getShowItemGain(id, (short) -possesed, true)
        );
      }
    }
  }
  ...
  public int countReactor(byte st) {
    MapleMap map = c.getPlayer().getMap();
    double range = Double.POSITIVE_INFINITY;
    List<MapleMapObject> reactorz = map.getMapObjectsInRange(
      c.getPlayer().getPosition(),
      range,
      Arrays.asList(MapleMapObjectType.REACTOR)
    );
  ...
  public void worldMessage(int type, String message) {
    handling.MaplePacket packet = MaplePacketCreator.serverNotice(
      type,
      message
    );
  ...
  public void gainEquip(int itemId, int quantity) {
    if (MapleItemInformationProvider.getInstance().isEquip(itemId)) {
      Equip eq = (Equip) MapleItemInformationProvider
        .getInstance()
        .getEquipById(itemId);
      ...
      this.getClient()
        .getSession()
        .write(
          MaplePacketCreator.getShowItemGain(itemId, (short) quantity, true)
        );
    }
  }
