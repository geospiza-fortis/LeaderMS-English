...
public class EventInstanceManager {
  ...
  public void monsterKilled(final MapleCharacter chr, final MapleMonster mob) {
    ...
    try {
      int inc =
        (
          (Double) em.getIv().invokeFunction("monsterValue", this, mob.getId())
        ).intValue();
      if (disposed || chr == null) {
        return;
      }
      ...
      .schedule(
        new Runnable() {
          public void run() {
            ...
            } catch (NullPointerException npe) {} catch (ScriptException ex) {
  ...
  public void dispose_NoLock() {
    ...
    try {
      ...
      System.out.println(
        "Caused by : " + emN + " instance name: " + name + " method: dispose:"
      );
      e.printStackTrace();
