...
public class MapleInventoryManipulator {
  ...
  public static boolean addSuperFromNPC(
    MapleClient c,
    int itemid,
    String logInfo,
    int max
    ...
    if (quantity == 1) {
      ...
      c
        .getSession()
        .write(MaplePacketCreator.addInventorySlot(type, idrop, true));
    } else {
  ...
  public static boolean addRing(MapleCharacter chr, int itemId, int ringId) {
    ...
    chr
      .getClient()
      .getSession()
      .write(MaplePacketCreator.addInventorySlot(type, nEquip));
    return true;
  }
  ...
  public static boolean addFromDrop(MapleClient c, IItem item, String logInfo) {
    ...
    MapleInventoryType type = ii.getInventoryType(item.getItemId());
    if (
      !c.getChannelServer().allowMoreThanOne() &&
    ...
    if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (i.hasNext()) {
              ...
              if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner())) {
                ...
                c
                  .getSession()
                  .write(
                    MaplePacketCreator.updateInventorySlot(type, eItem, true)
                  );
              }
            } else {
              break;
      ...
      while (
        ...
        c
          .getSession()
          .write(MaplePacketCreator.addInventorySlot(type, nItem, true));
        if (
          (
            ii.isThrowingStar(item.getItemId()) || ii.isBullet(item.getItemId())
          ) &&
      ...
      if (quantity == 1) {
        ...
        c
          .getSession()
          .write(MaplePacketCreator.addInventorySlot(type, item, true));
      } else {
        throw new RuntimeException(
          "Trying to create equip with non-one quantity"
        );
    ...
    c
      .getSession()
      .write(
        MaplePacketCreator.getShowItemGain(item.getItemId(), item.getQuantity())
      );
    return true;
  }
      ...
      if (!ii.isThrowingStar(itemId) && !ii.isBullet(itemId)) {
          ...
          while (quantity > 0) {
            if (i.hasNext()) {
              ...
              if (
                ...
                c
                  .getSession()
                  .write(MaplePacketCreator.updateInventorySlot(type, eItem));
              }
            } else {
              break;
            }
        ...
        while (
          ...
          if (newQ != 0) {
            ...
            if (petid != -1) {
              System.out.println(
                "[PET INFO] Created a pet with id <" + petid + ">"
              );
            }
            byte newSlot = c.getPlayer().getInventory(type).addItem(nItem);
            if (newSlot == -1) {
            ...
            c
              .getSession()
              .write(MaplePacketCreator.addInventorySlot(type, nItem));
            if (
              (ii.isThrowingStar(itemId) || ii.isBullet(itemId)) &&
              quantity == 0
            ) {
              break;
            }
  ...
  public static void removeFromSlot(
    ...
    if (item.getQuantity() == 0 && !allowZero) {
      ...
      c
        .getSession()
        .write(
          MaplePacketCreator.updateInventorySlot(type, (Item) item, fromDrop)
        );
    }
  }
  ...
  public static void move(
    ...
    if (
        ...
        c
          ...
          .write(
            MaplePacketCreator.moveAndMergeInventoryItem(
              ...
              dst,
              (
                (Item) c.getPlayer().getInventory(type).getItem(dst)
              ).getQuantity()
            )
          );
      }
      ...
      c
        .getSession()
        .write(MaplePacketCreator.moveInventoryItem(type, src, dst));
    }
  }
  ...
  public static void equip(MapleClient c, byte src, byte dst) {
    ...
    source =
      (Equip) c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(src);
    target =
      (Equip) c
        .getPlayer()
        .getInventory(MapleInventoryType.EQUIPPED)
    ...
    if (
      c.getPlayer().getBuffedValue(MapleBuffStat.BOOSTER) != null &&
  ...
  public static void drop(
      ...
      c
        ...
        .write(
          MaplePacketCreator.serverNotice(
            5,
            "Jr. GameMasters cannot drop items."
          )
        );
      return;
    }
    ...
    if (
      c.getPlayer().getItemQuantity(itemId, true) < quantity ||
      quantity < 0 ||
      source == null ||
      (
        quantity == 0 &&
    ...
    if (
      quantity < source.getQuantity() && !ii.isThrowingStar(source.getItemId())
    ) {
      ...
      c
        .getSession()
        .write(MaplePacketCreator.dropInventoryItemUpdate(type, source));
  ...
  public static void editEquipById(
    MapleCharacter chr,
    int itemid,
    byte stat,
    short value
