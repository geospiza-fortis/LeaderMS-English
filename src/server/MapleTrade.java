...
public class MapleTrade {
  ...
  public void lock() {
    ...
    partner
      ...
      .getClient()
      .getSession()
      .write(MaplePacketCreator.getTradeConfirmation());
  }
  ...
  public void complete2() {
    ...
    if (exchangeMeso > 0) chr.gainMeso(
      exchangeMeso - getFee(exchangeMeso),
      false,
      true,
      false
    );
    ...
    chr
      .getClient()
      .getSession()
      .write(MaplePacketCreator.getTradeCompletion(number));
  }
  ...
  public void cancel() {
    ...
    if (meso > 0) chr.gainMeso(meso, false, true, false);
    // just to be on the safe side...
    meso = 0;
    ...
    chr
      .getClient()
      .getSession()
      .write(MaplePacketCreator.getTradeCancel(number));
  }
    ...
    if (chr.getMeso() >= meso) {
      ...
      AutobanManager
        ...
        .addPoints(
          ...
          1000,
          0,
          "Trying to trade more mesos than you have."
        );
    }
  }
  ...
  public void chat(String message) {
    chr
      .getClient()
      .getSession()
      .write(MaplePacketCreator.getPlayerShopChat(chr, message, true));
    if (partner != null) {
      partner
        .getChr()
        .getClient()
        .getSession()
        .write(MaplePacketCreator.getPlayerShopChat(chr, message, false));
    }
  }
  ...
  public boolean fitsInInventory() {
    ...
    for (Map.Entry<MapleInventoryType, Integer> entry : neededSlots.entrySet()) {
      if (
        chr.getInventory(entry.getKey()).isFull(entry.getValue() - 1)
      ) return false;
    }
    return true;
  }
  ...
  public static void startTrade(MapleCharacter c) {
    if (c.getTrade() == null) {
      ...
      c
        ...
        .getSession()
        .write(
          MaplePacketCreator.serverNotice(5, "You are already in a trade")
        );
    }
  }
    ...
    if (c1.gmLevel() == 3 && c2.gmLevel() == 3) {
      c1
        ...
        .write(
          MaplePacketCreator.serverNotice(
            5,
            "GameMasters JR's nao podem realizar trocar."
          )
        );
      return;
    }
  ...
  public static void visitTrade(MapleCharacter c1, MapleCharacter c2) {
    if (
      ...
      c2
        .getClient()
        .getSession()
        .write(MaplePacketCreator.getTradePartnerAdd(c1));
      c1
        .getClient()
        .getSession()
        .write(
