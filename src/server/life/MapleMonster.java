...
public class MapleMonster extends AbstractLoadedMapleLife {
  ...
  private int hp;
  private int mp;
  private WeakReference<MapleCharacter> controller = new WeakReference<MapleCharacter>(
    null
  );
          ...
          new Runnable() {
            public void run() {
              if (MapleMonster.this.isAlive() && isHypnotized()) {
                ...
                int damage =
                  hypnotizer.getLevel() * 10 + (int) (Math.random() * 300);
                damage(hypnotizer, damage, true);
                getMap()
                  .broadcastMessage(
                    MaplePacketCreator.damageMonster(getObjectId(), damage)
                  );
                if (isAlive()) {
                  hypnotizeDamage();
                } else {
      ...
      target
        ...
        .write(
          MaplePacketCreator.serverNotice(
            5,
            "[Random Event] Monster got bored and left."
          )
        );
    } else {
  ...
  public int getDrop() {
    ...
    for (DropEntry d : dl) {
      ...
      d.assignedRangeLength =
        (int) Math.ceil(((double) 1 / (double) d.chance) * minChance);
    ...
    for (DropEntry d : dl) {
      if (
        c >= d.assignedRangeStart &&
        c < (d.assignedRangeStart + d.assignedRangeLength)
      ) {
        return d.itemId;
      }
    }
  ...
  public int getQuestDrop(MapleCharacter chr) {
    ...
    for (QuestDropEntry d : dl) {
      if (
        c >= d.assignedRangeStart &&
        c < (d.assignedRangeStart + d.assignedRangeLength) &&
        chr.getQuest(MapleQuest.getInstance(d.questid)).getStatus() ==
      ...
      public void run() {
        synchronized (this) {
          ...
          mob
            ...
            .spawnItemDrop(
              mob,
              mob.getEventInstance().getPlayers().get(0),
              t,
              mob.getPosition(),
              mob.isBoss(),
              true
          ...
          if (mob.getId() == 9300061) {
            ...
            mob
              ...
              .broadcastMessage(
                MaplePacketCreator.serverNotice(
                  6,
                  "The Easter Moon made " + d + " rice cakes."
                )
              );
          }
  ...
  public void damage(
    MapleCharacter from,
    int damage,
    boolean updateAttackTime
  ) {
    AttackerEntry attacker = null;
    if (from.getParty() != null) {
      attacker =
        new PartyAttackerEntry(
          from.getParty().getId(),
          from.getClient().getChannelServer()
        );
    } else {
      attacker =
        new SingleAttackerEntry(from, from.getClient().getChannelServer());
    }
    ...
    if (this.hp < 1 && this.getId() == 9300061) {
      this.getMap()
        .broadcastMessage(
          MaplePacketCreator.serverNotice(5, "[Notice] You failed the mission.")
        );
  ...
  public void heal(int hp, int mp) {
    ...
    setMp(mp2Heal);
    getMap()
      .broadcastMessage(MaplePacketCreator.healMonster(getObjectId(), hp));
  }
  ...
  public MapleCharacter killBy(final MapleCharacter killer) {
    if (killer.getMapId() == 980000000) {
      for (int i = 0; i < mobs.length; i++) if (this.getId() == mobs[i]) {
        for (MapleCharacter currmap : killer.getMap().getCharacters()) {
          currmap.addBossPoints(
            this.getId() == mobs[0]
              ? 1
              : this.getId() == mobs[1]
                ? 2
          ...
          currmap.dropMessage(
            ...
            " BPQ] Congratulations, you killed " +
            getMobName(false) +
            "."
          );
          currmap.dropMessage(
            ...
            Configuration.Server_Name +
            " BPQ] You now have (" +
            currmap.getBossPoints() +
            ") Boss Points."
          );
          currmap.dropMessage(
            "[" +
            Configuration.Server_Name +
            " BPQ] The next monster is: " +
            getMobName(true) +
            "."
          );
        }
                ...
                if (MapleMonster.this.getId() == 9400300) {
                  for (MapleCharacter party : killer.getPartyMembers()) {
                    party.dropMessage(
                      "[" +
                      Configuration.Server_Name +
                      " BPQ] You managed to complete the BPQ. "
                    );
                    party.dropMessage(
                      "[" +
                      Configuration.Server_Name +
                      " BPQ] You currently have (" +
                      party.getBossPoints() +
                      ") Boss Points!"
                    );
                  ...
                  killer.spawnMonster(
                    mobs[x + 1],
                    MapleMonster.this.getId() == mobs[0]
                      ? 200000000
                      : MapleMonster.this.getId() == mobs[1]
                        ? 300000000
                        : MapleMonster.this.getId() == mobs[2]
                          ? 400000000
                          : MapleMonster.this.getId() == mobs[3]
                            ? 500000000
                            : MapleMonster.this.getId() == mobs[4]
                              ? 600000000
                              : MapleMonster.this.getId() == mobs[5]
                                ? 700000000
                                : MapleMonster.this.getId() == mobs[6]
                                  ? 800000000
                                  : MapleMonster.this.getId() == mobs[7]
                                    ? 900000000
                                    : MapleMonster.this.getId() == mobs[8]
                                      ? 1000000000
                                      : MapleMonster.this.getId() == mobs[9]
                                        ? 1100000000
                                        : MapleMonster.this.getId() == mobs[10]
                                          ? 1200000000
                                          : MapleMonster.this.getId() ==
                                            mobs[11]
                                            ? 1300000000
                                            : MapleMonster.this.getId() ==
                                              mobs[12]
    ...
    for (AttackerEntry attackEntry : attackers) {
      attackEntry.killedMob(
        killer.getMap(),
        (int) Math.ceil(
          totalBaseExp * ((double) attackEntry.getDamage() / getMaxHp())
        ),
        attackEntry == highest
      );
    }
    if (this.getController() != null) { // this can/should only happen when a hidden gm attacks the monster
      getController()
        .getClient()
        .getSession()
        .write(MaplePacketCreator.stopControllingMonster(this.getObjectId()));
    ...
    if (toSpawn != null) {
      ...
      if (
        toSpawn.contains(9300216) &&
        reviveMap.getId() > 925000000 &&
        reviveMap.getId() < 926000000
      ) {
        reviveMap.broadcastMessage(
          MaplePacketCreator.playSound("Dojang/clear")
        );
        reviveMap.broadcastMessage(
          MaplePacketCreator.showEffect("dojang/end/clear")
        );
      }
      TimerManager
        .getInstance()
  ...
  public void switchController(
    ...
    if (controllers != null) {
      ...
      controllers
        .getClient()
        .getSession()
        .write(MaplePacketCreator.stopControllingMonster(getObjectId()));
    }
  ...
  public void sendSpawnData(MapleClient c) {
    ...
    if (hasBossHPBar()) {
      if (
        this.getMap().countMonster(8810026) > 2 &&
        this.getMap().getId() == 240060200
      ) {
  ...
  public void sendDestroyData(MapleClient client) {
    client
      .getSession()
      .write(MaplePacketCreator.killMonster(getObjectId(), false));
  }
  ...
  public boolean applyStatus(
    ...
    if (poison) {
      ...
      if (
        from.getJob() == MapleJob.NIGHTLORD ||
        ...
        if (from.getJob() == MapleJob.NIGHTLORD) {
          ...
          matk =
            SkillFactory.getSkill(4120005).getEffect(poisonLevel).getMatk();
          ...
          matk =
            SkillFactory.getSkill(4220005).getEffect(poisonLevel).getMatk();
        } else {
          return false;
        }
        Random r = new Random();
        int luk = from.getLuk();
        int maxDmg = (int) Math.ceil(
          Math.min(Short.MAX_VALUE, 0.2 * luk * matk)
        );
        int minDmg = (int) Math.ceil(
    ...
    if (isAlive()) {
      ...
      for (int i = 0; i < remove.length; i++) {
        if (monsterBuffs.contains(remove[i])) {
          ...
          MaplePacket packet = MaplePacketCreator.cancelMonsterStatus(
            getObjectId(),
            Collections.singletonMap(remove[i], Integer.valueOf(1))
          );
          map.broadcastMessage(packet, getPosition());
          if (
            getController() != null &&
  ...
  public void usedSkill(final int skillId, final int level, long cooltime) {
    ...
    if (
      this.skillsUsed.containsKey(new Pair<Integer, Integer>(skillId, level))
    ) {
      int times =
        this.skillsUsed.get(new Pair<Integer, Integer>(skillId, level)) + 1;
  ...
  private class AttackingMapleCharacter {
    ...
    public AttackingMapleCharacter(
      MapleCharacter attacker,
      long lastAttackTime
    ) {
      super();
    ...
    public List<AttackingMapleCharacter> getAttackers() {
      ...
      if (chr != null) {
        return Collections.singletonList(
          new AttackingMapleCharacter(chr, lastAttackTime)
        );
      } else {
  ...
  private class PartyAttackerEntry implements AttackerEntry {
    ...
    public List<AttackingMapleCharacter> getAttackers() {
      List<AttackingMapleCharacter> ret = new ArrayList<AttackingMapleCharacter>(
        attackers.size()
      );
      for (Entry<Integer, OnePartyAttacker> entry : attackers.entrySet()) {
        MapleCharacter chr = cserv
          .getPlayerStorage()
          .getCharacterById(entry.getKey());
        if (chr != null) {
          ret.add(
            new AttackingMapleCharacter(chr, entry.getValue().lastAttackTime)
          );
        }
      }
      return ret;
    ...
    private Map<MapleCharacter, OnePartyAttacker> resolveAttackers() {
      Map<MapleCharacter, OnePartyAttacker> ret = new HashMap<MapleCharacter, OnePartyAttacker>(
        attackers.size()
      );
      for (Entry<Integer, OnePartyAttacker> aentry : attackers.entrySet()) {
        MapleCharacter chr = cserv
          .getPlayerStorage()
          .getCharacterById(aentry.getKey());
        if (chr != null) {
    ...
    public void addDamage(
      ...
      if (oldPartyAttacker != null) {
        ...
        // future but not now
        OnePartyAttacker onePartyAttacker = new OnePartyAttacker(
          from.getParty(),
          damage
        );
    ...
    public void killedMob(MapleMap map, int baseExp, boolean mostDamage) {
      ...
      Map<MapleCharacter, Integer> expMap = new ArrayMap<MapleCharacter, Integer>(
        6
      );
      for (Entry<MapleCharacter, OnePartyAttacker> attacker : attackers_.entrySet()) {
        ...
        double expFraction =
          (innerBaseExp * expBonus) / (expApplicable.size() + 1);
        ...
        for (MapleCharacter expReceiver : expApplicable) {
          ...
          if (
            levelMod > 1.0 || this.attackers.containsKey(expReceiver.getId())
          ) {
            levelMod = 1.0;
          }
