...
public class MobSkill {
  ...
  public void applyEffect(
    ...
    switch (skillId) {
      ...
      case 140:
        if (
          makeChanceResult() && !monster.isBuffed(MonsterStatus.MAGIC_IMMUNITY)
        ) {
      ...
      case 141:
        if (
          makeChanceResult() && !monster.isBuffed(MonsterStatus.WEAPON_IMMUNITY)
        ) {
    ...
    if (monStat != null || heal) {
      if (lt != null && rb != null && skill) {
        List<MapleMapObject> objects = getObjectsInRange(
          monster,
  ...
  private List<MapleCharacter> getPlayersInRange(
    MapleMonster monster,
    MapleCharacter player
  ) {
