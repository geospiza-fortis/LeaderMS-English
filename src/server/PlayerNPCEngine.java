...
public class PlayerNPCEngine {
  ...
  public static boolean createGeneralNPC(int ID, MapleCharacter chr)
    throws Throwable {
    ...
    ps =
      con.prepareStatement(
        "INSERT INTO playernpcs(name, hair, face, skin, dir, map, npcid)" +
        "VALUES (?, ?, ?, ?, ?, ?, ?)"
      );
    ps.setString(1, chr.getName());
    ...
    if (nid != -1) chr
      ...
      .getMapFactory()
      .getMap(mapId)
      .broadcastMessage(MaplePacketCreator.getPlayerNPC(nid));
    return true;
  }
