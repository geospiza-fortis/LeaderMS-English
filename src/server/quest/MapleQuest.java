...
public class MapleQuest {
  ...
  private static MapleDataProvider questData = MapleDataProviderFactory.getDataProvider(
  ...
  private MapleQuest(int id) {
    try {
      ...
      relevantMobs = new LinkedList<Integer>();
      // read reqs
      MapleData startReqData = requirements
        .getChildByPath(String.valueOf(id))
        .getChildByPath("0");
      ...
      MapleData completeReqData = requirements
        .getChildByPath(String.valueOf(id))
        .getChildByPath("1");
      ...
      // read acts
      MapleData startActData = actions
        .getChildByPath(String.valueOf(id))
        .getChildByPath("0");
      ...
      MapleData completeActData = actions
        .getChildByPath(String.valueOf(id))
        .getChildByPath("1");
  ...
  public void complete(MapleCharacter c, int npc, Integer selection) {
    if ((autoPreComplete || checkNPCOnMap(c, npc)) && canComplete(c, npc)) {
      ...
      c
        .getClient()
        .getSession()
        .write(MaplePacketCreator.showForeignEffect(12));
      c
        .getMap()
        .broadcastMessage(
          c,
          MaplePacketCreator.showForeignEffect(c.getId(), 12),
          false
        );
    }
  }
