    ...
    switch (getType()) {
      case JOB:
        for (MapleData jobEntry : getData().getChildren()) {
          if (
            c
              .getJob()
              .equals(MapleJob.getById(MapleDataTool.getInt(jobEntry))) ||
            c.isGM()
          ) {
            return true;
          }
        ...
        for (MapleData questEntry : getData().getChildren()) {
          ...
          if (
            ...
            MapleQuestStatus.Status
              .getById(MapleDataTool.getInt(questEntry.getChildByPath("state")))
      ...
      case ITEM:
        ...
        for (MapleData itemEntry : getData().getChildren()) {
          ...
          if (
            quantity <
            MapleDataTool.getInt(itemEntry.getChildByPath("count"), 1) ||
            MapleDataTool.getInt(itemEntry.getChildByPath("count"), 1) <= 0 &&
            quantity > 0
          ) {
            return false;
          }
        ...
        for (MapleData mobEntry : getData().getChildren()) {
          ...
          try {
            ...
            FilePrinter.printError(
              FilePrinter.QUEST + quest.getId() + ".txt",
              t
            );
          }
