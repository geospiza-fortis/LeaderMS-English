...
public enum MapleQuestActionType {
  ...
  public static MapleQuestActionType getByWZName(String name) {
    if (name.equals("exp")) return EXP; else if (
      name.equals("money")
    ) return MESO; else if (name.equals("item")) return ITEM; else if (
      name.equals("skill")
    ) return SKILL; else if (
      name.equals("nextQuest")
    ) return NEXTQUEST; else if (name.equals("pop")) return FAME; else if (
      name.equals("buffItemID")
    ) return BUFF; else return UNDEFINED;
  }
}
