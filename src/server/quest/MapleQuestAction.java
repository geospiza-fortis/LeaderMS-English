...
public class MapleQuestAction {
  ...
  private boolean canGetItem(MapleData item, MapleCharacter c) {
    if (item.getChildByPath("gender") != null) {
      ...
      if (gender != 2 && gender != c.getGender()) return false;
    }
    if (item.getChildByPath("job") != null) {
      ...
      if (job < 100) {
        // koreans suck.
        if (
          MapleJob.getBy5ByteEncoding(job).getId() /
          100 !=
          c.getJob().getId() /
          100
        ) return false;
  ...
  public void run(MapleCharacter c, Integer extSelection) {
    ...
    ServernoticeMapleClientMessageCallback snmcmc = new ServernoticeMapleClientMessageCallback(
      5,
      c.getClient()
    );
    switch (type) {
      case EXP:
        ...
        c.gainExp(
          ...
          ChannelServer.getInstance(c.getClient().getChannel()).getQuestRate(),
          true,
          true
        );
        break;
        ...
        for (MapleData iEntry : data.getChildren()) {
          ...
          if (iEntry.getChildByPath("prop") != null) {
            if (MapleDataTool.getInt(iEntry.getChildByPath("prop")) == -1) {
              ...
              MapleDataTool.getInt(iEntry.getChildByPath("id")) != selection
            ) continue;
          }
          if (MapleDataTool.getInt(iEntry.getChildByPath("count")) < 0) { // remove items
            ...
            int itemId = MapleDataTool.getInt(iEntry.getChildByPath("id"));
            short quantity = (short) MapleDataTool.getInt(
              iEntry.getChildByPath("count")
            );
            ...
            c
              ...
              .getSession()
              .write(
                MaplePacketCreator.getShowItemGain(itemId, quantity, true)
              );
          }
        }
        break;
      ...
      case SKILL:
        ...
        for (MapleData sEntry : data) {
          ...
          int skillLevel = MapleDataTool.getInt(
            sEntry.getChildByPath("skillLevel")
          );
      ...
      case FAME:
        ...
        c
          .getClient()
          .getSession()
          .write(MaplePacketCreator.getShowFameGain(fameGain));
        break;
      case BUFF:
        status = c.getQuest(quest);
        if (
