...
public class PublicChatHandler {
  ...
  private static void addPlayer(MapleCharacter chr) {
    ...
    for (int chrIds : playerHolder.keySet()) {
      MapleCharacter chrs = ChannelServer
        .getInstance(playerHolder.get(chrIds))
        .getPlayerStorage()
      ...
      chrs
        ...
        .write(
          MaplePacketCreator.multiChat(
            "",
            chr.getName() + " has been added to the chat.",
            2
          )
        );
    }
    ...
    if (playerHolder.containsKey(chr.getId())) {
      for (int chrIds : playerHolder.keySet()) {
        ...
        chrs
          .getClient()
          .getSession()
          .write(MaplePacketCreator.multiChat(chr.getName(), message, 3));
      }
    }
  }
  ...
  public static boolean doChat(MapleClient c, String text) {
    ...
    WhisperMapleClientMessageCallback mc = new WhisperMapleClientMessageCallback(
      "ChatBot",
      c
    );
    if (text.charAt(0) == '`') { // ` is much easier than typing ~
      ...
      if (splitted[0].equalsIgnoreCase("connect")) {
        ...
        c
          .getSession()
          .write(
            MaplePacketCreator.multiChat("", "Current people in the chat", 3)
          );
        ...
        if (playerHolder.size() == 0) {
          ...
          for (int chrIds : playerHolder.keySet()) {
            ...
            if (sb.length() > 70) {
              c
                .getSession()
                .write(MaplePacketCreator.multiChat("", sb.toString(), 3));
          ...
          c
            .getSession()
            .write(
              MaplePacketCreator.multiChat("", i + " total connected.", 3)
            );
        }
