      ...
      synchronized (this) {
          ...
          if (npcs || !type.equals("n")) {
            ...
            if (myLife instanceof MapleMonster) {
              ...
              if (mobTime == -1 && respawns) { //does not respawn, force spawn once
                ...
                if (team == 1 || team == 0) map.addMonsterSpawn(
                  monster,
                  mobTime,
                  team
                ); else map.addMonsterSpawn(monster, mobTime);
              }
        ...
        if (
          ...
          Boolean.parseBoolean(
            ChannelServer.getInstance(channel).getProperty("world.faekchar")
          )
        ) {
          MapleClient faek = new MapleClient(null, null, new MockIOSession());
          try {
            MapleCharacter faekchar = MapleCharacter.loadCharFromDB(
              30000,
              faek,
              true
            );
