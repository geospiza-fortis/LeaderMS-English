...
public class MapleDoor extends AbstractMapleMapObject {
    ...
    for (MapleMapObject obj : town.getMapObjects()) {
      if (obj instanceof MapleDoor) {
        ...
        if (
          ...
          owner
            .getParty()
            .containsMembers(new MaplePartyCharacter(door.getOwner()))
        ) {
    ...
    if (
      ...
      if (
        ...
        (
          ...
          owner
            .getParty()
            .containsMembers(new MaplePartyCharacter(client.getPlayer()))
        )
      ) {
        client
          .getSession()
  ...
  public void sendDestroyData(MapleClient client) {
    if (
      ...
      client
        .getSession()
        .write(MaplePacketCreator.removeDoor(owner.getId(), false));
      client
        .getSession()
        .write(MaplePacketCreator.removeDoor(owner.getId(), true));
    }
  }
  ...
  public void warp(MapleCharacter chr, boolean toTown) {
    if (
      ...
      if (!toTown) chr.changeMap(target, targetPosition); else chr.changeMap(
        town,
        townPortal
      );
    } else {
