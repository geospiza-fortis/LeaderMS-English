...
public class MapleFootholdTree {
  ...
  public void insert(MapleFoothold f) {
    ...
    if (
      ...
      if (f.getX2() <= center.x && f.getY2() <= center.y) nw.insert(
        f
      ); else if (f.getX1() > center.x && f.getY2() <= center.y) ne.insert(
        f
      ); else if (f.getX2() <= center.x && f.getY1() > center.y) sw.insert(
        f
      ); else se.insert(f);
    }
  }
  ...
  private List<MapleFoothold> getRelevants(Point p, List<MapleFoothold> list) {
    ...
    if (nw != null) {
      if (p.x <= center.x && p.y <= center.y) nw.getRelevants(
        p,
        list
      ); else if (p.x > center.x && p.y <= center.y) ne.getRelevants(
        p,
        list
      ); else if (p.x <= center.x && p.y > center.y) sw.getRelevants(
        p,
        list
