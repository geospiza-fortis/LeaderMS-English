...
public class MonsterCarnival {
  ...
  public MonsterCarnival(MapleParty p1, MapleParty p2, int mapid) {
    ...
    if (chnl != chnl1) throw new RuntimeException(
      "CPQ leaders are on different channels.."
    );
    ...
    if (leader1.getId() == charid || leader2.getId() == charid) {
      ...
      map.broadcastMessage(
        MaplePacketCreator.serverNotice(
          5,
          "Maple " + teamS + " has quitted the Monster Carnival."
        )
      );
      return;
    } else {
  ...
  public void extendTime() {
    map.broadcastMessage(
      MaplePacketCreator.serverNotice(5, "The time has been extended.")
    );
      ...
      if (mc != null) {
        if (redWin) {
          mc
            .getClient()
            .getSession()
            .write(MaplePacketCreator.showEffect("quest/carnival/win"));
          mc
            .getClient()
            .getSession()
            .write(MaplePacketCreator.playSound("MobCarnival/Win"));
        } else {
          mc
            .getClient()
            .getSession()
            .write(MaplePacketCreator.showEffect("quest/carnival/lose"));
          mc
            .getClient()
            .getSession()
            .write(MaplePacketCreator.playSound("MobCarnival/Lose"));
        }
      }
    }
    for (MaplePartyCharacter mpc : leader2.getParty().getMembers()) {
  ...
  public int getTotalCP(int team) {
    if (team == 0) return redTotalCP; else if (
      team == 1
    ) return blueTotalCP; else throw new RuntimeException("Unknown team");
  }

  public void setTotalCP(int totalCP, int team) {
    if (team == 0) this.redTotalCP = totalCP; else if (
      team == 1
    ) this.blueTotalCP = totalCP;
  }

  public int getCP(int team) {
    if (team == 0) return redCP; else if (
      team == 1
    ) return blueCP; else throw new RuntimeException("Unknown team");
  }

  public void setCP(int CP, int team) {
    if (team == 0) this.redCP = CP; else if (team == 1) this.blueCP = CP;
  }
}
