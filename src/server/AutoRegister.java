...
public class AutoRegister {
  ...
  public static boolean getAccountExists(String login) {
    ...
    try {
      ...
      System.out.println(
        "XSource: Error acquiring the account of (" + login + ")."
      );
    }
  ...
  public static void createAccount(String login, String pwd, String eip) {
    try {
      PreparedStatement ipq = DatabaseConnection
        .getConnection()
        .prepareStatement(
      ...
      if (
        !rs.first() ||
        rs.last() &&
        rs.getRow() < LoginServer.getInstance().AutoRegLimit()
      ) {
        try {
          PreparedStatement ps = DatabaseConnection
            .getConnection()
            .prepareStatement(
              "INSERT INTO accounts (name, password, email, birthday, macs, lastknownip) VALUES (?, ?, ?, ?, ?, ?)"
            );
          ps.setString(1, login);
          ...
          System.out.println(
            "XSource: Error creating the account of (" +
            login +
            " | " +
            pwd +
            " | " +
