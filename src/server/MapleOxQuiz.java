...
public class MapleOxQuiz {
  ...
  public void checkAnswers() {
    for (MapleCharacter chr : map.getCharacters()) {
      ...
      if (correct) {
        chr.gainExp(
          expGain * chr.getClient().getChannelServer().getExpRate(),
          true,
          false
        );
      } else {
          ...
          public void run() {
            map.broadcastMessage(
              MaplePacketCreator.serverNotice(
                6,
                MapleOxQuizFactory.getOXQuestion(round, question)
              )
            );
            TimerManager
              .getInstance()
        ...
        new Runnable() {
          public void run() {
            map.broadcastMessage(
              MaplePacketCreator.serverNotice(
                6,
                MapleOxQuizFactory.getOXExplain(round, question)
              )
            );
            if (map.getOx() != null) { // Set next one if Ox Quiz is still active.
              ...
              map.broadcastMessage(
                MaplePacketCreator.serverNotice(6, "Ox Quiz Deactivated")
              );
            }
          }
        },
