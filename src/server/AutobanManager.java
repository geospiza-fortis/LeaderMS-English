...
public class AutobanManager implements Runnable {
  ...
  public void autoban(MapleClient c, String reason) {
    if (c.getPlayer().isGM()) {
      System.out.println(
        "[h4x] Trying to a/b gm - something fishy going on: {}" + reason
      );
    }
  ...
  public synchronized void addPoints(
    ...
    if (this.points.get(acc) >= AUTOBAN_POINTS) {
      ...
      if (c.getPlayer().isGM()) {
        System.out.println(
          "[h4x] Trying to a/b gm - something fishy going on: {}" +
          banReason.toString()
        );
      } else {
            ...
            .broadcastMessage(
              ...
              MaplePacketCreator
                .serverNotice(
                  ...
                  "[Autoban] " +
                  name +
                  " banido pelo sistema. (Ultima razao: " +
                  reason +
                  ")"
                )
                .getBytes()
        ...
        System.out.println(
          "[h4x] Autobanned player " + name + " (accountid " + acc + ")"
        );
      }
      return;
    }
  ...
  public void run() {
    ...
    for (ExpirationEntry e : expirations) {
      if (e.time <= now) this.points.put(
          e.acc,
          this.points.get(e.acc) - e.points
        ); else return;
    }
  }
}
