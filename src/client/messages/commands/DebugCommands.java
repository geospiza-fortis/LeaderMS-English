...
public class DebugCommands implements Command {
  ...
  public void execute(MapleClient c, MessageCallback mc, String[] splitted)
    ...
    if (splitted[0].equals("!resetquest")) {
      MapleQuest
        .getInstance(Integer.parseInt(splitted[1]))
        .forfeit(c.getPlayer());
    } else if (splitted[0].equals("!nearestPortal")) {
      final MaplePortal portal = player
        .getMap()
        .findClosestSpawnpoint(player.getPosition());
      mc.dropMessage(
        portal.getName() +
        " id: " +
        portal.getId() +
      ...
      mplew.write(
        HexTool.getByteArrayFromHexString("B9 00 00 47 1E 00 00 0A 04 76 FF")
      );
      c.getSession().write(mplew.getPacket());
      mplew = new MaplePacketLittleEndianWriter();
      mplew.write(
        HexTool.getByteArrayFromHexString(
          "36 00 00 EF 1C 0D 4C 3E 1D 0D 0A 04 76 FF"
        )
      );
      c.getSession().write(mplew.getPacket());
      ...
      c
        .getSession()
        .write(
          MaplePacketCreator.applyMonsterStatusTest(
      ...
      c
        .getSession()
        .write(
          MaplePacketCreator.applyMonsterStatusTest2(
            Integer.valueOf(splitted[2]),
            mask,
            1000,
            Integer.valueOf(splitted[3])
      ...
      MapleReactorStats reactorSt = MapleReactorFactory.getReactor(
      ...
      c
        .getSession()
        .write(
          MonsterCarnivalPacket.startMonsterCarnival(c.getPlayer().getTeam())
        );
      ...
      mc.dropMessage(
        "CP: " + c.getPlayer().getCP() + "/" + c.getPlayer().getTotalCP()
      );
      mc.dropMessage(
        "Party CP: " +
        c.getPlayer().getMonsterCarnival().getCP(c.getPlayer().getTeam()) +
        "/" +
        c.getPlayer().getMonsterCarnival().getTotalCP(c.getPlayer().getTeam())
      );
      mc.dropMessage("Team: " + c.getPlayer().getTeam());
      mc.dropMessage(
        "Has Instance: " + (c.getPlayer().getMonsterCarnival() != null)
      );
      mc.dropMessage(
        "Has enemy: " + (c.getPlayer().getParty().getEnemy() != null)
      );
      mc.dropMessage(
        "Enemy ID: " + c.getPlayer().getParty().getEnemy().getId()
      );
      mc.dropMessage(
        "Red team: " + (c.getPlayer().getMonsterCarnival().getRed() != null)
      );
      mc.dropMessage(
        "Red team ID: " + (c.getPlayer().getMonsterCarnival().getRed().getId())
      );
      mc.dropMessage(
        "Blue team: " + (c.getPlayer().getMonsterCarnival().getBlue() != null)
      );
      mc.dropMessage(
        "Blue team ID: " +
        (c.getPlayer().getMonsterCarnival().getBlue().getId())
      );
      mc.dropMessage(
        "Time left: " + (c.getPlayer().getMonsterCarnival().getTimeLeft())
      );
      mc.dropMessage(
        "Time left seconds: " +
        c.getPlayer().getMonsterCarnival().getTimeLeftSeconds()
      );
      ...
      mc.dropMessage(
        "[Event Instance] warped to map " + Integer.parseInt(splitted[1])
      );
    } else if (splitted[0].equalsIgnoreCase("!cpqbuffmap")) {
      c
        .getPlayer()
        .getMap()
        .buffMonsters(c.getPlayer().getTeam(), MonsterStatus.WEAPON_IMMUNITY);
      ...
      c
        .getPlayer()
        .getMap()
        .debuffMonsters(c.getPlayer().getTeam(), MonsterStatus.WEAPON_IMMUNITY);
      ...
      c
        ...
        .getMap()
        .broadcastMessage(
          MaplePacketCreator.spawnNPC(MapleLifeFactory.getNPC(9901000))
        );
      c
        .getPlayer()
        .getMap()
        .broadcastMessage(MaplePacketCreator.getPlayerNPC(c.getPlayer()));
    } else if (splitted[0].equalsIgnoreCase("!pmerchant")) {
      PlayerNPCMerchant merchant = new PlayerNPCMerchant(
        c.getPlayer(),
        9901000
      );
      ...
      mc.dropMessage(
        c
          .getPlayer()
          .getMap()
          .setPortalDisable(!c.getPlayer().getMap().getPortalDisable()) +
        ": portal disabled"
      );
