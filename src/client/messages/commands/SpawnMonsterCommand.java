...
public class SpawnMonsterCommand implements Command {
  ...
  public void execute(MapleClient c, MessageCallback mc, String[] splitted)
    ...
    if (
      c.getPlayer().getGMLevel() <= 3 && c.getChannelServer().eventOn != true
    ) {
      c
        .getSession()
        .write(
          MaplePacketCreator.serverNotice(
            5,
            "This command is only available during events."
          )
        );
      return;
    }
    ...
    if (newExpRatio < oldExpRatio && newexp > 0) {
      mc.dropMessage(
        "The new hp/exp ratio is better than the old one. (" +
        newExpRatio +
        " < " +
        oldExpRatio +
        ")"
      ...
      mc.dropMessage(
        "Please consider that you're spawning a better monster than the original."
      );
  ...
  public CommandDefinition[] getDefinition() {
    return new CommandDefinition[] {
      new CommandDefinition(
        ...
        "Spawns the monster with the given id",
        2
      ),
    };
  }
