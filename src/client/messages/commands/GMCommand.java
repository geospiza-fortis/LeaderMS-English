...
public class GMCommand implements Command {
  ...
  public void execute(MapleClient c, MessageCallback mc, String[] splitted)
    ...
    if (splitted[0].equals("!notice")) {
      ...
      if (outputMessage.equalsIgnoreCase("!array")) outputMessage =
        c.getChannelServer().getArrayString();
      ...
      c
        .getChannelServer()
        .broadcastPacket(MaplePacketCreator.serverNotice(6, message));
      ...
      String reason =
        ...
        ": " +
        originalReason;
      MapleCharacter target = cserv
        .getPlayerStorage()
        .getCharacterByName(splitted[1]);
      if (target != null) {
        String readableTargetName = MapleCharacterUtil.makeMapleReadable(
          target.getName()
        );
        String ip = target
          .getClient()
      ...
      if (splitted.length < 4 || originalReason == null) {
        mc.dropMessage(
          "Syntax helper: !tempban <name> [i / m / w / d / h] <amount> [r [reason id] : Text Reason <Make sure you put :>"
        );
        return;
      ...
      if (gReason > 14) {
        mc.dropMessage(
          "You have entered an incorrect ban reason ID, please try again."
        );
        return;
      }
      ...
      tempB.set(
        tempB.get(Calendar.YEAR) + yChange,
      ...
      if (victim == null) {
        ...
        if (
          accId >= 0 && MapleCharacter.tempban(reason, tempB, gReason, accId)
        ) {
          String readableTargetName = MapleCharacterUtil.makeMapleReadable(
            victim.getName()
          );
          ...
          mc.dropMessage(
            "There was a problem offline banning character " + splitted[1] + "."
          );
        }
      } else {
      ...
      c
        .getSession()
        .write(MaplePacketCreator.serverNotice(6, builder.toString()));
    } else if (splitted[0].equals("!cheaters")) {
      try {
        List<CheaterData> cheaters = c
          .getChannelServer()
          .getWorldInterface()
          .getCheaters();
      ...
      if (victim != null) {
        ...
        try {
          ...
          WorldLocation loc = c
            .getChannelServer()
            .getWorldInterface()
            .getLocation(splitted[1]);
          if (loc != null) {
            mc.dropMessage(
              "You will be cross-channel warped. This may take a few seconds."
            );
            ...
            try {
              WorldChannelInterface wci = ChannelServer
                .getInstance(c.getChannel())
                .getWorldInterface();
              wci.addBuffsToStorage(
                c.getPlayer().getId(),
                c.getPlayer().getAllBuffs()
              );
              wci.addCooldownsToStorage(
                c.getPlayer().getId(),
                c.getPlayer().getAllCooldowns()
              );
            } catch (RemoteException e) {
              c.getChannelServer().reconnectWorld();
            }
            c.getPlayer().saveToDB(true, true);
            if (c.getPlayer().getCheatTracker() != null) c
              .getPlayer()
              .getCheatTracker()
              .dispose();
            ChannelServer
              .getInstance(c.getChannel())
              .removePlayer(c.getPlayer());
      ...
      if (splitted.length > 1) {
        ...
        MaplePacket packet = MaplePacketCreator.serverNotice(
          6,
          "Drop rate was changed to " + drop + "x."
        );
        ChannelServer.getInstance(c.getChannel()).broadcastPacket(packet);
      } else {
      ...
      if (splitted.length > 1) {
        ...
        MaplePacket packet = MaplePacketCreator.serverNotice(
          6,
          "Boss drop rate was changed to " + bossdrop + "x."
        );
        ChannelServer.getInstance(c.getChannel()).broadcastPacket(packet);
      } else {
      ...
      if (splitted.length > 1) {
        ...
        MaplePacket packet = MaplePacketCreator.serverNotice(
          6,
          "Expeerience rate was changed to " + exp + "x."
        );
        ChannelServer.getInstance(c.getChannel()).broadcastPacket(packet);
      } else {
        mc.dropMessage("Syntax: !bossdroprate <number>");
      ...
      mc.dropMessage(
        "You have destroyed " + items.size() + " items on the ground."
      );
  ...
  public CommandDefinition[] getDefinition() {
    return new CommandDefinition[] {
      ...
      new CommandDefinition(
        "me",
        "message",
        "send a message with your name as the prefix",
        2
      ),
      new CommandDefinition(
        "ban",
        "charname reason",
        "Permanently ip, mac and accountbans the given character",
        2
      ),
      new CommandDefinition(
        "tempban",
        "<name> [i / m / w / d / h] <amount> [r  [reason id] : Text Reason",
        "Tempbans the given account",
        2
      ),
      new CommandDefinition(
        "dc",
        "[-f] name",
        "Disconnects player matching name provided. Use -f only if player is persistant!",
        2
      ),
      ...
      new CommandDefinition(
        "warp",
        "playername [targetid]",
        "Warps yourself to the player with the given name. When targetid is specified warps the player to the given mapid",
        2
      ),
      new CommandDefinition(
        "warphere",
        "playername",
        "Warps the player with the given name to yourself",
        2
      ),
      new CommandDefinition(
        "jail",
        "[2] playername",
        "Warps the player to a map that he can't leave",
        2
      ),
      new CommandDefinition(
        "map",
        "mapid",
        "Warps you to the given mapid (use /m instead)",
        2
      ),
      new CommandDefinition(
        "say",
        "message",
        "Talks to the whole world in the format: [Name] message",
        2
      ),
