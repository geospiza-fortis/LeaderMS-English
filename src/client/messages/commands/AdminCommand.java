...
public class AdminCommand implements Command {
  ...
  public void execute(MapleClient c, MessageCallback mc, String[] splitted)
    ...
    if (splitted[0].equals("!shutdown")) {
      ...
      List<MapleMapObject> npcs = player
        .getMap()
      ...
      List<MapleMapObject> npcs = map.getMapObjectsInRange(
        c.getPlayer().getPosition(),
        Double.POSITIVE_INFINITY,
        Arrays.asList(MapleMapObjectType.NPC)
      );
      ...
      mc.dropMessage(
        "Saving all characters in channel " + cserv.getChannel() + "..."
      );
      Collection<MapleCharacter> chrs = cserv
        .getPlayerStorage()
        .getAllCharacters();
        ...
        c
          ...
          .write(
            MaplePacketCreator.getPacketFromHexString(
              StringUtil.joinStringFrom(splitted, 0)
            )
          );
      } else {
      ...
      try {
        mc.dropMessage(
          "Attempting to reload all guilds... this may take a while..."
        );
        cserv.getWorldInterface().clearGuilds();
        mc.dropMessage("Completed.");
      } catch (RemoteException re) {
        mc.dropMessage(
          "RemoteException occurred while attempting to reload guilds."
        );
        log.error(
          "RemoteException occurred while attempting to reload guilds.",
          re
      ...
      for (MapleCharacter chr : people) {
        if (chr.getClient().getChannel() != c.getChannel()) {
          ...
          ChannelServer
            .getInstance(chr.getClient().getChannel())
            .removePlayer(chr);
      ...
      final int w = 32;
      int dStart =
        c.getPlayer().getPosition().x - (splitString.length() / 2 * w);
      ...
      for (MapleCharacter mchr : cs.getPlayerStorage().getAllCharacters()) {
        MapleInventory equipped = mchr.getInventory(
      ...
      if (victim != null) {
        if (curse.equalsIgnoreCase("seal")) {
          victim.giveDebuff(
            MapleDisease.getType(120),
            MobSkillFactory.getMobSkill(120, 1),
            true
          );
        } else if (curse.equalsIgnoreCase("stun")) {
          victim.giveDebuff(
            MapleDisease.getType(123),
            MobSkillFactory.getMobSkill(123, 1),
            true
          );
        } else if (curse.equalsIgnoreCase("seduce")) {
          victim.giveDebuff(
            MapleDisease.getType(128),
            MobSkillFactory.getMobSkill(128, 1),
            true
          );
        } else if (curse.equalsIgnoreCase("poison")) {
          victim.giveDebuff(
            MapleDisease.getType(125),
            MobSkillFactory.getMobSkill(125, 1),
            true
          );
        } else if (curse.equalsIgnoreCase("darkness")) {
          victim.giveDebuff(
            MapleDisease.getType(121),
            MobSkillFactory.getMobSkill(121, 1),
            true
          );
        } else if (curse.equalsIgnoreCase("weaken")) {
          victim.giveDebuff(
            MapleDisease.getType(122),
            MobSkillFactory.getMobSkill(122, 1),
            true
          );
      ...
      if (force) {
        if (
          map.getForcedReturnId() == map.getId() ||
          map.getReturnMapId() == map.getId()
        ) {
          map.warpAllToCashShop(
            map.getStreetName() +
            " : " +
            map.getMapName() +
            "was disposed by " +
            c.getPlayer().getName() +
            ". You will be warped to the cash shop."
          );
        } else map.warpAllToNearestTown(
          map.getStreetName() +
          " : " +
          map.getMapName() +
          "was disposed by " +
          c.getPlayer().getName() +
          ". You will be warped to the nearest town."
        );
      }
      ...
      c
        .getChannelServer()
        .getWorldInterface()
        .broadcastMessage("", MaplePacketCreator.yellowChat(text).getBytes());
        ...
        ChannelServer
          ...
          .broadcastMessage(
            ...
            MaplePacketCreator
              .serverNotice(
                0,
                "MapleStory Physical Fitness Test (or rather, tedious jump quest event) has started."
              )
              .getBytes()
          );
      ...
      Rectangle area = splitted[1].equals("pq")
        ? null
        : map.getArea(Integer.parseInt(splitted[2]));
      List<MapleCharacter> players = splitted[1].equals("pq")
        ? null
        : map.getPlayersInRect(area);
      if (splitted[1].equals("give")) {
        ...
        if (splitted[3].equals("exp")) {
          ...
          short quantity = (short) CommandProcessor.getOptionalIntArg(
            splitted,
            5,
            1
          );
          ...
          for (MapleCharacter x : players) {
            MapleInventoryManipulator.addById(
              ...
              "Area item.",
              x.getName(),
              pet ? MaplePet.createPet(amount) : -1
            );
          }
        }
        ...
        Point br = new Point(
          tl.x + (int) area.getWidth(),
          tl.y + (int) area.getHeight()
        );
        ...
        MaplePacket wrong1 = MaplePacketCreator.showEffect(
      ...
      mc.dropMessage(
        c
          .getPlayer()
          .getMap()
          .setDisableInvincibilitySkills(
            !c.getPlayer().getMap().getDisableInvincibilitySkills()
          ) +
        ": invincibility skills disabled"
      );
      ...
      Collection<MapleCharacter> victims = c
        .getPlayer()
        .getMap()
        .getCharacters();
      ...
      c
        .getPlayer()
        .getMap()
        .startMapEffect(StringUtil.joinStringFrom(splitted, 1), 5120002);
    } else if (splitted[0].equals("!toggledamage")) {
      mc.dropMessage(
        c
          .getPlayer()
          .getMap()
          .setDisableDamage(!c.getPlayer().getMap().getDisableDamage()) +
        ": Damage disabled"
      );
      ...
      mc.dropMessage(
        c
          .getPlayer()
          .getMap()
          .setDisableChat(!c.getPlayer().getMap().getDisableChat()) +
        ": Chat disabled"
      );
  ...
  public CommandDefinition[] getDefinition() {
    return new CommandDefinition[] {
      ...
      new CommandDefinition(
        "shutdownnow",
        "",
        "Shuts down the current channel now",
        4
      ),
      new CommandDefinition(
        "shutdownworld",
      ...
      new CommandDefinition(
        "removenpcs",
        "",
        "Removes all custom spawned npcs from the map - requires reentering the map",
        4
      ),
      ...
      new CommandDefinition(
        "mynpcpos",
        "",
        "Gets the info for making an npc",
        4
      ),
      new CommandDefinition(
        "npc",
        "npcid",
        "Spawns the npc with the given id at the player position",
        4
      ),
      new CommandDefinition(
        "packet",
        "hex data",
        "Shows a clock to everyone in the map",
        4
      ),
      ...
      new CommandDefinition(
        "startProfiling",
        "",
        "Starts the CPU Sampling based profiler",
        4
      ),
      new CommandDefinition(
        "stopProfiling",
        "[fileName]",
        "Stops the Profiler and saves the results to the given fileName",
        4
      ),
      ...
      new CommandDefinition(
        "clock",
        "<duration> [mapid to warp to] [min level to warp] [max level to warp]",
        "",
        4
      ),
      ...
      new CommandDefinition(
        "mspfc",
        "",
        "ASK ANGELSL FOR TRAINING ON COMMAND!",
        4
      ),
      new CommandDefinition(
        "area",
