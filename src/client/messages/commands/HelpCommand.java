...
public class HelpCommand implements Command {
  ...
  public void execute(
    ...
    if (splittedLineLineLine[0].equals("!startev")) {
      ...
      String originalReason = StringUtil.joinStringFrom(
        splittedLineLineLine,
        2
      );
      String reason =
        c.getPlayer().getName() +
        " banned " +
        splittedLineLineLine[1] +
        ": " +
        originalReason;
      MapleCharacter target = cserv
        .getPlayerStorage()
        .getCharacterByName(splittedLineLineLine[1]);
      if (target != null) {
      ...
      if (victim != null) {
        ...
        try {
          ...
          WorldLocation loc = c
            .getChannelServer()
            .getWorldInterface()
            .getLocation(splittedLineLineLine[1]);
          if (loc != null) {
            mc.dropMessage(
      ...
      if (splittedLineLineLine.length > 2) {
        try {
          targetPortal =
            target.getPortal(Integer.parseInt(splittedLineLineLine[2]));
      ...
      if (splittedLineLineLine.length > 1) {
        MaplePacket packet = MaplePacketCreator.serverNotice(
          ...
          c.getPlayer().getName() +
          "] " +
          StringUtil.joinStringFrom(splittedLineLineLine, 1)
        );
        try {
          ChannelServer
            .getInstance(c.getChannel())
      ...
      mc.dropMessage(
        "(GM-3) Command " + splittedLineLineLine[0] + " does not exist!"
      );
    }
  }
