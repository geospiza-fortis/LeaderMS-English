...
public class OtherCommands implements Command {
  ...
  public void execute(MapleClient c, MessageCallback mc, String[] splitted)
    ...
    if (splitted[0].equals("!listreports")) {
      ...
      try {
        while (rs.next()) {
          mc.dropMessage(
            ...
            rs.getInt("id") +
            " | time reported: " +
            rs.getTimestamp("reporttime").toString() +
            " | reason: " +
            reasons[rs.getByte("reason")]
          );
        }
      } catch (Exception ex) {}
      ...
      if (c.getPlayer().getMapId() != 109020001) {
        mc.dropMessage(
          "You are not allowed to start the OX Quiz here, please go to map 109020001"
        );
      } else {
        ...
        c
          ...
          .broadcastMessage(
            MaplePacketCreator.serverNotice(
              6,
              "[OX Quiz] " + OXHandler.getOXQuestion(bla1, bla2)
            )
          );
      }
      ...
      if (splitted.length != 3) {
        ...
        c
          ...
          .broadcastMessage(
            MaplePacketCreator.serverNotice(
              5,
              "[Explanation] " + OXHandler.getOXExplain(bla1, bla2)
            )
          );
      }
      ...
      c
        .getChannelServer()
        .getWorldInterface()
        .broadcastMessage(null, MaplePacketCreator.sendHint(hint).getBytes());
    } else if (splitted[0].equals("!sendnote")) {
      if (splitted.length >= 2) {
        try {
          String text = StringUtil.joinStringFrom(splitted, 2);
          MaplePacketCreator.sendUnkwnNote(
            splitted[1],
            text,
            c.getPlayer().getName()
          );
      ...
      if (splitted.length != 4) {
        ...
        c
          .getSession()
          .write(
            MonsterCarnivalPacket.obtainCP(
        ...
        c
          .getSession()
          .write(
            MonsterCarnivalPacket.updateCP(
  ...
  private static void deleteReport(int id) {
    try {
      Connection dcon = DatabaseConnection.getConnection();
      PreparedStatement ps = dcon.prepareStatement(
