...
public class CharCommands implements Command {
  ...
  public void execute(MapleClient c, MessageCallback mc, String[] splitted)
    ...
    if (splitted[0].equals("!lowhp")) {
      ...
      for (
        Iterator<Entry<Integer, MapleMap>> it = c
          .getChannelServer()
          .getMapFactory()
          .getMaps()
          .entrySet()
      ...
      .dropMessage("You are on map " + player.getMap().getId());
            ...
            .broadcastMessage(
              ...
              MaplePacketCreator
                .serverNotice(
                  6,
                  c.getChannel(),
                  "[Event] An event has started in Channel " +
                  c.getChannel() +
                  ". Use @event to participate."
                )
                .getBytes()
            );
            ...
            .broadcastMessage(
              ...
              MaplePacketCreator
                .serverNotice(
                  6,
                  c.getChannel(),
                  "[Event] The event ended. Thank you to all who participated."
                )
                .getBytes()
            );
        } catch (RemoteException e) {
      ...
      MapleCharacter victim = cserva
        .getPlayerStorage()
        .getCharacterByName(splitted[1]);
      cserva.broadcastPacket(
        MaplePacketCreator.serverNotice(
          6,
          "Character Status <" + splitted[1] + ">"
        )
      );
      ...
      if (
        Integer.parseInt(splitted[1]) >= 5000000 &&
        ...
        MapleInventoryManipulator.addById(
          ...
          quantity,
          player.getName() + "used !item with quantity " + quantity,
          player.getName(),
          petId
        );
        return;
      ...
      for (MapleCharacter mch : player.getMap().getCharacters()) mch
        .getMap()
        .broadcastMessage(
          MaplePacketCreator.getChatText(mch.getId(), text, false, 0)
        );
      ...
      victim.giveDebuff(
        MapleDisease.SEDUCE,
        MobSkillFactory.getMobSkill(128, Integer.parseInt(splitted[2]))
      );
      ...
      for (MapleCharacter map : player.getMap().getCharacters()) {
        ...
        map.giveDebuff(
          MapleDisease.SEDUCE,
          MobSkillFactory.getMobSkill(128, Integer.parseInt(splitted[1]))
        );
      }
    } else if (splitted[0].equals("!bomb")) {
      for (MapleCharacter chr : player.getMap().getCharacters()) player
        .getMap()
        .spawnMonsterOnGroudBelow(
          MapleLifeFactory.getMonster(9300166),
          chr.getPosition()
        );
      for (int i = 0; i < 250; i += 50) {
        player
          ...
          .spawnMonsterOnGroudBelow(
            MapleLifeFactory.getMonster(9300166),
            new Point(player.getPosition().x - i, player.getPosition().y)
          );
        player
          .getMap()
          .spawnMonsterOnGroudBelow(
      ...
      for (ChannelServer channel : ChannelServer.getAllInstances()) {
        for (MapleCharacter players : channel
          .getPlayerStorage()
          .getAllCharacters()) {
      ...
      Collection<MapleCharacter> chrs = c
        .getChannelServer()
      ...
      mc.dropMessage(
        "Total characters on channel " + c.getChannel() + ": " + chrs.size()
      );
      ...
      if (splitted.length < 2) {
        ...
        c
          .getSession()
          .write(
            MaplePacketCreator.updateAriantPQRanking(splitted[1], 5, false)
          );
      }
      ...
      cserva
        .getPlayerStorage()
        .getCharacterByName(splitted[1])
        .gainvotePoints(Integer.parseInt(splitted[2]));
      player.dropMessage(
        6,
        "Done. Sent " + splitted[2] + " points to " + splitted[1] + "."
      );
    } else if (splitted[0].equals("!leaderpoints")) {
      cserva
        .getPlayerStorage()
        .getCharacterByName(splitted[1])
        .gainLeaderPoints(Integer.parseInt(splitted[2]));
      player.dropMessage(
        6,
        "Done. Sent " + splitted[2] + " points to " + splitted[1] + "."
      );
      ...
      cserva
        .getPlayerStorage()
        .getCharacterByName(splitted[1])
        .modifyCSPoints(5, Integer.parseInt(splitted[2]));
      player.dropMessage(
        6,
        "Done. Sent " + splitted[2] + " points to " + splitted[1] + "."
      );
      ...
      cserva
        .getPlayerStorage()
        .getCharacterByName(splitted[1])
        .addjqpoints(Integer.parseInt(splitted[2]));
      player.dropMessage(
        6,
        "Done. Sent " + splitted[2] + " points to " + splitted[1] + "."
      );
      ...
      cserva
        .getPlayerStorage()
        .getCharacterByName(splitted[1])
        .gainpqPoints(Integer.parseInt(splitted[2]));
      player.dropMessage(
        6,
        "Done. Sent " + splitted[2] + " points to " + splitted[1] + "."
      );
      ...
      for (int i = 1; i < 5; i *= 2) {
        cserva
          .getPlayerStorage()
          .getCharacterByName(splitted[1])
          .modifyCSPoints(i, Integer.parseInt(splitted[2]));
      }
      player.dropMessage(
        6,
        "Done. Sent " + splitted[2] + " NX Points to " + splitted[1] + "."
      );
    } else if (splitted[0].equals("!conectados")) {
      try {
        Map<Integer, Integer> connected = cserva
          .getWorldInterface()
          .getConnected();
      ...
      for (int i : array) SkillFactory
        .getSkill(i)
        .getEffect(SkillFactory.getSkill(i).getMaxLevel())
        .applyTo(player);
  ...
  public CommandDefinition[] getDefinition() {
    return new CommandDefinition[] {
      ...
      new CommandDefinition(
        "saveall",
        "",
        "Saves all chars. Please use it wisely, quite expensive command.",
        4
      ),
