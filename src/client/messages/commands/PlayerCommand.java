...
public class PlayerCommand implements Command {
  ...
  public void execute(MapleClient c, MessageCallback mc, String[] splitted)
    ...
    if (
      splitted[0].equalsIgnoreCase("@gm") || splitted[0].equalsIgnoreCase("!gm")
    ) {
      ...
      if (gmUsages.get(c.getPlayer().getId()) != null) {
        ...
        if (System.currentTimeMillis() - lastUse < 60 * 1000 * 2) {
          mc.dropMessage(
            "You can only send messages to GMs twice in 2 minutes."
          );
          return;
        } else {
    ...
    if (splitted[0].equalsIgnoreCase("@bug")) {
      ...
      splitted[0].equalsIgnoreCase("@commands") ||
      splitted[0].equalsIgnoreCase("!commands")
    ) {
      mc.dropMessage(
        " - " +
      ...
      if (c.getChannelServer().eventOn == true) {
        ...
        c
          .getPlayer()
          .dropMessage(
            "There are no active events at this time. Please try again later!"
          );
        return;
      }
      ...
      try {
        ...
        Logger
          .getLogger(PlayerCommand.class.getName())
        ...
        String text =
          (
            !player.getSmegaEnabled()
              ? "[Disabled] Smegas have been disabled."
              : "[Activated] Smegas have been activated."
          );
        mc.dropMessage(text);
      ...
      if (!gm) {
        ps =
          (PreparedStatement) con.prepareStatement(
            "SELECT jqrank , level, name FROM characters WHERE gm < 3 ORDER BY jqrank desc LIMIT 3"
          );
      } else {
        ps =
          (PreparedStatement) con.prepareStatement(
            "SELECT jqrank , name, gm FROM characters WHERE gm >= 3"
          );
      }
  ...
  public CommandDefinition[] getDefinition() {
    return new CommandDefinition[] {
      ...
      new CommandDefinition(
        "dispose",
        "",
        "Not able to speak to an NPC? Use this command.",
        0
      ),
      new CommandDefinition(
        "partyfix",
        "",
        "Fixes a bug in the creation of parties.",
        0
      ),
      new CommandDefinition(
        "event",
        "",
        "Takes you to the event in progress.",
        0
      ),
      new CommandDefinition(
        "exitevent",
        "",
        "Takes you back to the map where you were. (Leave event)",
        0
      ),
      new CommandDefinition(
        "smega",
        "",
        "This command Enables/Disables super megaphones (Rate: 10,000 Mesos).",
        0
      ),
