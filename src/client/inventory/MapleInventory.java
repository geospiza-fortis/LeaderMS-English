...
public class MapleInventory implements Iterable<IItem>, InventoryContainer {
  ...
  public void addFromDB(IItem item) {
    if (
      item.getPosition() < 0 && !type.equals(MapleInventoryType.EQUIPPED)
    ) throw new RuntimeException(
      "Item with negative position in non-equipped IV wtf?"
    );
  ...
  public void move(byte sSlot, byte dSlot, short slotMax) {
    ...
    if (source == null) throw new InventoryException(
      "Trying to move empty slot"
    );
    if (target == null) {
      ...
      if (type.getType() == MapleInventoryType.EQUIP.getType()) swap(
        target,
        source
      );
  ...
  public void removeItem(byte slot, short quantity, boolean allowZero) {
    ...
    if (item == null) return; // TODO is it ok not to throw an exception here?
