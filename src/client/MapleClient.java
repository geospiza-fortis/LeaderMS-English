...
public class MapleClient {
  ...
  private void loadMacsIfNescessary() throws SQLException {
    if (macs.isEmpty()) {
      Connection con = DatabaseConnection.getConnection();
      PreparedStatement ps = con.prepareStatement(
        "SELECT macs FROM accounts WHERE id = ?"
      );
      ps.setInt(1, accId);
      ResultSet rs = ps.executeQuery();
      if (rs.next()) {
  ...
  public int finishLogin(boolean success) {
    if (success) {
      synchronized (MapleClient.class) {
        if (
          getLoginState() > LOGIN_NOTLOGGEDIN &&
          getLoginState() != LOGIN_WAITING
        ) {
          loggedIn = false;
          return 7;
        }
      ...
      try {
        Connection con = DatabaseConnection.getConnection();
        PreparedStatement ps = con.prepareStatement(
          "UPDATE accounts SET LastLoginInMilliseconds = ? WHERE id = ?"
        );
  ...
  public int login(String login, String pwd, boolean ipMacBanned) {
    ...
    try {
      PreparedStatement ps = con.prepareStatement(
        "SELECT id, password, salt, tempban, banned, gm, macs, lastknownip, greason FROM accounts WHERE name = ?"
      );
      ...
      if (rs.next()) {
        ...
        if (
          !rs
            .getString("lastknownip")
            .equals(session.getRemoteAddress().toString())
        ) {
          PreparedStatement lkip = con.prepareStatement(
            "UPDATE accounts SET lastknownip = ? WHERE id = ?"
          );
          String sockAddr = session.getRemoteAddress().toString();
        ...
        if (LoginServer.getInstance().isServerCheck() && !gm) {
          ...
          if (passhash.equals(pwd)) {
            ...
            if (
              LoginCrypto.isLegacyPassword(passhash) &&
              LoginCrypto.checkPassword(pwd, passhash)
            ) {
              loginok = 0;
            ...
            if (updatePasswordHash) {
              ...
              try {
                ...
                pss.setString(
                  1,
                  LoginCrypto.makeSaltedSha512Hash(pwd, newSalt)
                );
   ...
   */
  public static String getChannelServerIPFromSubnet(
    ...
    if (subnetInfo.contains("net.login.subnetcount")) {
      int subnetCount = Integer.parseInt(
        subnetInfo.getProperty("net.login.subnetcount")
      );
      for (int i = 0; i < subnetCount; i++) {
        String[] connectionInfo = subnetInfo
          .getProperty("net.login.subnet." + i)
          .split(":");
        ...
        if (
          ((ipAddress & subnet) == (channelIP & subnet)) &&
  ...
  private void unban() {
    ...
    try {
      ...
      ps.close();
      ps =
        con.prepareStatement("DELETE FROM ipbans WHERE ip LIKE CONCAT(?, '%')");
  ...
  public int fix(String login, String pwd) {
    ...
    try {
      ...
      if (rs.next()) {
        ...
        if (
          LoginCryptoLegacy.isLegacyPassword(passhash) &&
      ...
      if (chr != null && isLoggedIn()) {
        ...
        try {
          if (chr.getMessenger() != null) {
            ...
            getChannelServer()
              .getWorldInterface()
              .leaveMessenger(chr.getMessenger().getId(), messengerplayer);
        ...
        try {
          ...
          if (chr.getParty() != null) {
            try {
              ...
              log.warn(
                "Failed removing party character. Player already removed.",
                e
              );
            }
          }
  ...
  public void deleteAllCharacters() {
    ...
    try {
      ...
      ps =
        con.prepareStatement("SELECT id FROM characters WHERE accountid = ?");
  ...
  public static String getLogMessage(
    ...
    if (cfor != null) {
      if (cfor.getPlayer() != null) {
        builder.append("<");
        builder.append(
          MapleCharacterUtil.makeMapleReadable(cfor.getPlayer().getName())
        );
        builder.append(" (ID: ");
      ...
      if (cfor.getAccountName() != null) {
        ...
        builder.append(
          MapleCharacterUtil.makeMapleReadable(cfor.getAccountName())
        );
        builder.append(") ");
      }
    }
  ...
  public int getTimesTalked(int n) {
    if (
      timesTalked.get(new Pair<MapleCharacter, Integer>(getPlayer(), n)) == null
    ) {
      setTimesTalked(n, 0);
    }
  ...
  public boolean hasPinTurnedOn() {
    ...
    try {
      ps =
        DatabaseConnection
          .getConnection()
          .prepareStatement("SELECT pintoggle FROM accounts WHERE id = ?");
  ...
  public void setPinToggle(int x) {
    try {
      PreparedStatement ps = DatabaseConnection
        .getConnection()
        .prepareStatement("UPDATE accounts SET pintoggle = ? WHERE id = ?");
      ps.setInt(1, x);
      ps.setInt(2, getAccID());
      ps.executeUpdate();
      ps.close();
