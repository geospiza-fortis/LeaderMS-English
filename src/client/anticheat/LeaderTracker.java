...
public class LeaderTracker {
  ...
  public void Tracker() {
    if (
      SPAMMING_MEGA >= 100 ||
      TOO_MUCH_DAMAGE >= 100 ||
      CHEATING_MERCHANT >= 15 ||
      TRYING_TO_HACK >= 10 ||
      SPAMMING_SUMMON_BAGS > 15
    ) {
    ...
    try {
      ...
      ps =
        (PreparedStatement) con.prepareStatement(
          "SELECT name, banned, banreason, macs FROM accounts WHERE name = ?"
        );
      ps.setString(1, name);
      rs = ps.executeQuery();
      if (rs.next()) {
        if (rs.getInt("banned") > 0) {
          ...
          ps.close();
          return (
            "Username: " + user + " | BanReason: " + reason + " | Macs: " + mac
          );
        } else {
          rs.close();
          ps.close();
      ...
      ps =
        (PreparedStatement) con.prepareStatement(
          "SELECT name, banned, banreason, macs FROM accounts WHERE id = ?"
        );
      ps.setInt(1, accid);
      rs = ps.executeQuery();
      if (rs.getInt("banned") > 0) {
