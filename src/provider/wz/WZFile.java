...
public class WZFile implements MapleDataProvider {
  ...
  public WZFile(File wzfile, boolean provideImages) throws IOException {
    ...
    lea =
      new GenericLittleEndianAccessor(
        new InputStreamByteStream(
  ...
  private void parseDirectory(WZDirectoryEntry dir) {
    ...
    for (int i = 0; i < entries; i++) {
      ...
      switch (marker) {
        case 0x02:
          name =
            WZTool.readDecodedStringAtOffset(
              slea,
              lea.readInt() + this.headerSize + 1,
              true
            );
  ...
  public WZIMGFile getImgFile(String path) throws IOException {
    ...
    WZFileEntry entry = (WZFileEntry) dir.getEntry(
    ...
    String fullPath =
      wzfile
        .getName()
        .substring(0, wzfile.getName().length() - 3)
        .toLowerCase() +
      "/" +
      path;
    return new WZIMGFile(
      this.wzfile,
      entry,
      provideImages,
