...
public class WZIMGFile {
  ...
  protected void dumpImg(OutputStream out, SeekableLittleEndianAccessor slea)
    throws IOException {
  ...
  private void parse(WZIMGEntry entry, SeekableLittleEndianAccessor slea) {
    ...
    switch (marker) {
      ...
      case 1:
        {
          String name = WZTool.readDecodedStringAtOffsetAndReset(
            slea,
            file.getOffset() + slea.readInt()
          );
          entry.setName(name);
          break;
        }
      default:
        log.error(
          "Unknown Image identifier: {} at offset {}",
          marker,
    ...
    switch (marker) {
      ...
      case 8:
        ...
        if (iMarker == 0) {
          ...
          entry.setData(
            WZTool.readDecodedStringAtOffsetAndReset(
              slea,
              slea.readInt() + file.getOffset()
            )
          );
        } else {
  ...
  private void parseExtended(
    ...
    switch (marker) {
      ...
      default:
        throw new RuntimeException(
          "Unknown extended image identifier: " +
          marker +
          " at offset " +
          (slea.getPosition() - file.getOffset())
        );
    ...
    if (type.equals("Property")) {
      ...
      if (provideImages) {
        ...
        entry.setData(
          new PNGMapleCanvas(width, height, dataLength, format + format2, null)
        );
