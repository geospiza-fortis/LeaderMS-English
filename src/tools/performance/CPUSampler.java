...
public class CPUSampler {
  ...
  public void save(Writer writer, int minInvocations, int topMethods)
    throws IOException {
    ...
    for (
      int i = 0;
      i < topMethods && i < topConsumers.getTopConsumers().size();
      i++
    ) {
      builder.append(
        topConsumers
          .getTopConsumers()
          .get(i)
          .toString(topConsumers.getTotalInvocations(), 1)
      );
    }
    builder.append("\nStack Traces:\n");
  ...
  private int findRelevantElement(StackTraceElement[] trace) {
    ...
    if (
      firstIncluded >= 0 &&
      trace[firstIncluded].getClassName()
        .equals("tools.performance.CPUSampler$SamplerThread")
    ) { // don't sample us
      return -1;
    }
