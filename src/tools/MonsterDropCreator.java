...
public class MonsterDropCreator {
    ...
    for (Pair<Integer, String> Pair : itemNameCache) {
      if (Pair.getLeft() >= 2380000 && Pair.getLeft() <= 2388054) {
        ...
        for (Pair<Integer, MobInfo> Pair_ : mobCache) {
          if (
            Pair_.getRight().getName().equalsIgnoreCase(bookName.toString())
          ) {
            int rate = 7000;
  ...
  private static void getAllItems() {
    MapleDataProvider data = MapleDataProviderFactory.getDataProvider(
  ...
  public static void getAllMobs() {
    ...
    MapleDataProvider mobData = MapleDataProviderFactory.getDataProvider(
    ...
    for (MapleData itemFolder : mob.getChildren()) { // Get the list of mobs from String.wz
      ...
      try {
        MapleData monsterData = mobData.getData(
          StringUtil.getLeftPaddedStr(Integer.toString(id) + ".img", '0', 11)
        );
        MobInfo mobInfo = new MobInfo(
          id == 8810018
            ...
            : MapleDataTool.getIntConvert(
              "boss",
              monsterData.getChildByPath("info"),
              0
            ), // fix for HT
          MapleDataTool.getIntConvert(
            "rareItemDropLevel",
            monsterData.getChildByPath("info"),
            0
          ), // New info string added since v.71 MSEA
