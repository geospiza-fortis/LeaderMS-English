...
function getDefinition() {
  ...
  ret[0] = new CommandDefinition(
    "gmshop",
    "shopname",
    "Opens a GM shop. Just type !gmshop for a more thorough explanation.",
    "100"
  );
  return ret;
}
...
function execute(c, mc, splitted) {
  if (splitted.length != 2) {
    mc.dropMessage(
      "Syntax: !shop <shopid>, where shopid can be numerical or the following shops:"
    );
    mc.dropMessage(
      "mapleitems, glimmer, miscitems, scrolling, summonbags, warrior, magician, thief, bowman, nxrings, nxeffects, nxpackages, nxemotes, itembuffs, smega, gms, pets"
    );
