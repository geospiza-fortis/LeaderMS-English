    ...
    if (status == 0) {
      ...
      if (selection == 0) {
        ...
        if (!hasParty()) {
          ...
          //PARTY SIZE WRONG
          cm.sendOk(
            "Your party must consist of at least " + minPlayers + " members!"
          );
        } else if (!checkPartyLevels()) {
          //WRONG LEVELS
          cm.sendOk(
            "A member of your party does not fulfill the level requirements of " +
              minlvl +
              "-" +
              maxlvl +
              "."
...
function checkPartyLevels() {
  ...
  if (cm.getParty() == null) {
    ...
    for (var i = 0; i < party.size() && pass; i++) {
      if (
        party.get(i).getLevel() < 51 ||
        party.get(i).getLevel() > 100 ||
        party.get(i).getMapid() != cm.getMapId()
      ) {
        pass = false;
      }
    }
