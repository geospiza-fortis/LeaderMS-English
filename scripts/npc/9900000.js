  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      if (cm.getPlayer().isGM() == false) {
        ...
        cm.sendOk(
          "I'm sorry, I only do Males. You should try my sister NimaKIM."
        );
        cm.dispose();
      } else {
        cm.sendSimple(
      ...
      if (selection == 0) {
        ...
        for (var i = 0; i < hair.length; i++) {
          ...
          if (hair[i] == 30100 || hair[i] == 30010) {
            // default to black(well.. 0 anyhow) for these to prevent crashes.
            hairnew.push(hair[i]);
          } else {
            hairnew.push(hair[i] + parseInt(cm.getPlayer().getHair() % 10));
          }
        ...
        var current = parseInt(cm.getPlayer().getHair() / 10) * 10;
        if (current == 30100) {
          // check for bad combos. several for fantasy
          haircolor = Array(
            current,
            current + 1,
            current + 2,
        ...
        for (var i = 0; i < face.length; i++) {
          ...
          if (face[i] == 20022 || face[i] == 20021) {
            // default to black for these since sapphire crashes
            facenew.push(face[i]);
          } else {
            facenew.push(
              face[i] +
        ...
        if (current == 20022 || current == 20021) {
          // check for bad combos. this one disables sapphire (5)
          colors = Array(
            current,
            current + 100,
            current + 200,
