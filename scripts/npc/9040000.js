    ...
    if (status == 0)
      ...
      if (selection == 0) {
        if (
          ...
          cm.getPlayer().getGuildRank() >= 3
        ) {
          //no guild or not guild master/jr. master
          cm.sendNext(
            "Only a Master or Jr. Master of the guild can start an instance."
          );
          cm.dispose();
        } else {
          ...
          if (em == null)
            ...
            if (getEimForGuild(em, cm.getPlayer().getGuildId()) != null)
              cm.sendOk(
                "Your guild already has an active instance. Please try again later."
              );
            else {
              //start GQ
              ...
              cm.guildMessage(
                "The guild has been entered into the Guild Quest. Please report to Shuang at the Excavation Camp on channel " +
                  cm.getPlayer().getClient().getChannel() +
                  "."
              );

              //remove all GQ items from player entering
              for (var i = 0; i < GQItems.length; i++) cm.removeAll(GQItems[i]);
            }
          }
          cm.dispose();
        }
        ...
        if (cm.getPlayer().getGuildId() == 0) {
          ...
          if (em == null)
            ...
            if (eim == null)
              cm.sendOk(
                "Your guild is currently not registered for an instance."
              );
            else {
              if ("true".equals(eim.getProperty("canEnter"))) {
                ...
                cm.sendOk(
                  "I'm sorry, but the guild has gone on without you. Try again later."
                );
            }
          }
          cm.dispose();
