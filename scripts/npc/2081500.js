    ...
    if (status == 0) {
      ...
      if (cm.getJobId() % 10 > 1) {
        if (
          cm
            .getPlayer()
            .getMasterLevel(cm.getJobId == 512 ? 5121000 : 5221000) < 30
        ) {
          if (cm.getJobId() == 512) {
          ...
          cm.sendOk(
            "It seems you were not properly advanced and were missing some skills. That has been fixed now."
          );
        } else {
          cm.sendOk(
            "Your abilities are now at their maximum. Train hard and one day you may become one of the Greats."
          );
        }
        cm.dispose();
      } else if (
      ...
      cm.sendAcceptDecline(
        "Are you ready to take the final step into your destiny?"
      );
    } else if (status == 2) {
