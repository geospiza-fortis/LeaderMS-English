...
function action(mode, type, selection) {
  if (mode == -1) {
    ...
    if (cm.getPlayer().getMapId() == 240050000) {
      ...
      cm.getPlayer().getMapId() == 240050300 ||
      cm.getPlayer().getMapId() == 240050310
    ) {
      var eim = cm.getPlayer().getEventInstance();
      ...
      if (status == 0) {
        if (cm.isLeader()) {
          if (cm.haveItem(4001093, 6)) {
            //&& cm.countMonster == 0) {
            cm.sendNext(
              "Good job on finishing the Quest! You will now be warped to #bEntrance to Horntail's Cave#k."
            );
          } else {
            cm.sendNext(
              "Please kill all the monsters on the map and get 6 Blue Keys before talking to me..."
            );
            cm.dispose();
          }
        } else {
