  ...
  if (mode == -1) {
    ...
    if (curMap == 1) {
      ...
      if (playerStatus) {
        ...
        if (premable == null) {
          cm.sendNext(
            "Here is the back of the Pirate Ship. You will work your way to the front and save Wu-Yang for me. See the Pirates here? Kill them and give me 20 of each Marks. There are three kinds of Marks - Rookie, Rising, and Veteran. These pirates aren't the same as the ones off the ship, so please be careful ~"
          );
          ...
          if (complete != null) {
            ...
            cm.haveItem(4001120, 20) &&
            cm.haveItem(4001121, 20) &&
            cm.haveItem(4001122, 20)
          ) {
            cm.sendNext(
              "Thank you for bringing me 20 of each Mark. You may proceed now!"
            );
            ...
            cm.sendNext(
              "You have not got 20 of each #bMark of the Pirate.#k yet."
            );
            cm.dispose();
          }
        }
      ...
      var party = cm.getChar().getEventInstance().getPlayers();
      cm.sendOk(
        "This is the Storage of the Pirate Ship. Break the Boxes, and kill all the monsters, then proceed through the portal."
      );
      cm.dispose();
    } else if (curMap == 4) {
      //2nd to last stage! w00t!!!
      if (playerStatus) {
        //Is leader
        var eim = cm.getChar().getEventInstance();
        var mf = eim.getMapFactory();
        ...
        if (premable == null) {
          cm.sendNext(
            "Over here, is the place where the high - ranked pirates are trying to escape. They have the keys with them. Kill them, and occasionally they may drop a Key. Drop the key on to the door you wish to seal, then they can't run away. Once you seal all the doors, talk to me and you can proceed. Again, these Pirates aren't the same as the ones off the ship, so please be careful ~"
          );
          ...
          if (complete != null) {
            ...
            thisMap.getReactorByName("sMob1").getState() != 0 &&
            ...
            cm.sendNext(
              "Thank you for stopping the Pirates from running away. Please proceed, Lord Pirate must be taken down as soon as possible, and Wu Yang must be saved."
            );
            // cm.gainItem(4001120, -20);
            // cm.gainItem(4001121, -20);
            // cm.gainItem(4001122, -20);
      ...
      if (playerStatus) {
        ...
        if (premable == null) {
          cm.sendNext(
            "Here is Lord Pirate's Room. He will appear any minute... Be careful ~"
          );
