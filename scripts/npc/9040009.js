  ...
  if (mode == -1) {
    ...
    if (eim == null) {
      ...
      if (eim.getProperty("leader").equals(cm.getPlayer().getName())) {
        if (
          cm.getPlayer().getMap().getReactorByName("statuegate").getState() > 0
        ) {
          cm.sendOk("Proceed.");
          cm.dispose();
        } else {
          if (status == 0) {
              ...
              if (
                ...
                if (stage == 3) {
                  cm.getPlayer()
                    ...
                    .hitReactor(cm.getC());
                  cm.sendOk(
                    "Excellent work. Please proceed to the next stage."
                  );
            ...
            for (var i = 0; i < combo.length; i++) {
              cm.getPlayer()
                ...
                .delayedHitReactor(cm.getC(), delay + 3500 * i);
            }
...
function getReactors() {
  ...
  while (iter.hasNext()) {
    var mo = iter.next();
    if (
      mo.getType() == MapleMapObjectType.REACTOR &&
      !mo.getName().equals("statuegate")
    ) {
