...
function action(mode, type, selection) {
  if (mode == -1) {
    ...
    cm.sendOk(
      "Wise choice. Who wouldn't want free mesos from the #bBonus Stage#k."
    );
    cm.dispose(); //No
  } else {
    if (mode == 1) status++;
    ...
    if (status == 0) cm.sendYesNo("Would you like to leave the bonus?");
    else if (status == 1) {
      if (isLeader()) cm.sendOk("Ok, Your loss");
      else {
