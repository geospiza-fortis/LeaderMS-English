    ...
    if (status == 0)
      ...
      if (selection == 0) {
        if (cm.getPlayer().getGuildId() > 0) {
          ...
          cm.sendYesNo(
            "Creating a Guild costs #b" +
              cm.getPlayer().guildCost() +
        ...
        if (
          ...
          cm.sendOk(
            "You can only disband a Guild if you are the leader of that Guild."
          );
          cm.dispose();
        } else
          cm.sendYesNo(
            "Are you sure you want to disband your Guild? You will not be able to recover it afterward and all your GP will be gone."
          );
      } else if (selection == 2) {
        if (
          cm.getPlayer().getGuildId() <= 0 ||
          ...
          cm.sendOk(
            "You can only increase your Guild's capacity if you are the leader."
          );
          cm.dispose();
        } else
          cm.sendYesNo(
            "Increasing your Guild capacity by #b5#k costs #b" +
              cm.getPlayer().capacityCost() +
              " mesos#k, are you sure you want to continue?"
          );
      }
    } else if (status == 2) {
