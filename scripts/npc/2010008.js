      ...
      if (selection == 0) {
        if (cm.getChar().getGuildId() > 0) {
          if (cm.getPlayer().getGuildRank() == 1)
            cm.sendYesNo(
              "Creating or changing Guild Emblem costs #b" +
                cm.getPlayer().emblemCost() +
          ...
          cm.sendOk(
            "You must be a Guild Leader to change the Emblem. Please speak with Heracle to create a Guild, then talk to me."
          );
          cm.dispose();
        }
      }
