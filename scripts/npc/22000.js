...
function start() {
  cm.sendYesNo(
    "Take this ship and you'll head off to a bigger continent. For #e150 mesos#n, I'll take you to #bVictoria Island#k. The thing is, once you leave this place, you can't ever come back. What do you think? Do you want to go to Victoria Island?"
  );
}
  ...
  if (mode == -1) cm.dispose();
    ...
    if (status == 1) {
      if (cm.haveItem(4031801))
        ...
        cm.sendNext(
          "Bored of this place? Here... Give me #e150 mesos#n first..."
        );
    } else if (status == 2) {
      if (cm.haveItem(4031801))
        cm.sendNextPrev(
        ...
        if (cm.getLevel() > 6) {
          if (cm.getPlayer().getMeso() < 150) {
            cm.sendOk(
              "What? You're telling me you wanted to go without any money? You're one weirdo..."
            );
            cm.dispose();
          } else
            cm.sendNext(
              "Awesome! #e150#n mesos accepted! Alright, off to Victoria Island!"
            );
        } else {
          cm.sendOk(
            "Let's see... I don't think you are strong enough. You'll have to be at least Level 7 to go to Victoria Island."
          );
          cm.dispose();
        }
      }
