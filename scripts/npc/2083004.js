  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      if (cm.getSquadState(MapleSquadType.HORNTAIL) == 0) {
        cm.sendSimple(
          "Would you like to assemble a team to take on the mighty #rHorntail#k?\r\n#b#L1#Lets get this going!#l\r\n#L2#No, I think I'll wait a bit...#l"
        );
        ...
        if (cm.checkSquadLeader(MapleSquadType.HORNTAIL)) {
          ...
          cm.sendSimple(
            "Would you like to join the Horntail Squad?\r\n#b#L1#Yes, I'm ready to take him on!#l\r\n#L2#No, I think I'll wait a bit...#l"
          );
          status = 9;
        }
      } else {
      ...
      if (selection == 1) {
        if (cm.createMapleSquad(MapleSquadType.HORNTAIL) != null) {
          cm.sendOk(
            "The #rHorntail Squad#k has been created, tell your members to sign up now.\r\n\r\nTalk to me again to view the current team, to close registrations, or start the fight!"
          );
          cm.dispose();
        } else {
          cm.sendOk(
            "There was an error, someone may have created a Horntail Squad before you, please inform a GameMaster if this is not the case."
          );
          cm.dispose();
        }
      } else if (selection == 2) {
        cm.sendOk(
          "Sure, not everyone's up to challenging the might of Horntail."
        );
        cm.dispose();
      }
    } else if (status == 10) {
      if (selection == 1) {
        ...
        cm.sendOk(
          "Sure, not everyone's up to challenging the might of #rHorntail.#k"
        );
        cm.dispose();
      }
    } else if (status == 20) {
      ...
      if (em == null) {
        ...
        // Begin the PQ.
        em.startInstance(
          cm.getSquad(MapleSquadType.HORNTAIL),
          cm.getChar().getMap()
        );
      }
      cm.dispose();
