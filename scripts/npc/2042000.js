    ...
    if (status == 0) {
      if (cm.getParty() == null) {
        ...
        for (var i = 0; i < party.size(); i++) {
          if (
            party.get(i).getLevel() >= minLvl &&
            party.get(i).getLevel() <= maxLvl
          ) {
            lvlOk++;
          }
        ...
        if (party >= 1) {
          cm.sendOk(
            "You don't have enough people in your party. You need a party of #b" +
              minAmt +
              "#k - #r" +
              maxAmt +
              "#k members and they must be in the map with you. There are #b" +
              inMap +
              "#k members here."
          );
          cm.dispose();
          ...
          cm.sendOk(
            "Make sure everyone in your party is here and is in the right level range of 30-50!"
          );
          cm.dispose();
        } else if (isInMap > 0) {
