...
function start() {
  ...
  if (eim == null) {
    ...
    if (eim.getProperty("leader").equals(cm.getPlayer().getName())) {
      if (
        cm.getPlayer().getMap().getReactorByName("watergate").getState() > 0
      ) {
        ...
        if (currentCombo == null || currentCombo.equals("reset")) {
          ...
          if (guess != null) {
            if (combo == guess) {
              ...
              var black = net.sf.odinms.server.life.MapleLifeFactory.getMonster(
                9300036
              );
              var myst = net.sf.odinms.server.life.MapleLifeFactory.getMonster(
                9300037
              );
              if (attempt < 7) {
                ...
                if (results[1] != 0) {
                  if (results[1] == 1) {
                    string +=
                      "1 vassal has recieved an incorrect offering.\r\n";
                  } else {
                    string +=
                      results[1] +
                      " vassals have recieved incorrect offerings.\r\n";
                  }
                }
                if (results[2] != 0) {
                  if (results[2] == 1) {
                    ...
                    string +=
                      results[2] +
                      " vassals have recieved unknown offerings.\r\n";
                  }
                }
                string += "This is your ";
                switch (attempt) {
                ...
                cm.getPlayer()
                  ...
                  .spawnMonsterOnGroundBelow(
                    black,
                    new java.awt.Point(-350, 150)
                  );
                cm.getPlayer()
                  .getMap()
                  .spawnMonsterOnGroundBelow(
                    myst,
                    new java.awt.Point(400, 150)
                  );
                ...
                for (var i = 0; i < 5; i++) {
                  ...
                  black = net.sf.odinms.server.life.MapleLifeFactory.getMonster(
                    9300036
                  );
                  myst = net.sf.odinms.server.life.MapleLifeFactory.getMonster(
                    9300037
                  );
                  cm.getPlayer()
                    .getMap()
