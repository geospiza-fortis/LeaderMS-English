  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      if (
        cm
          .getQuestStatus(6192)
          .equals(Packages.client.MapleQuestStatus.Status.STARTED)
      ) {
        if (cm.getParty() == null) {
          // no party
        ...
        if (!cm.isLeader()) {
          ...
          if (next) {
            ...
            cm.sendOk(
              "Your party is not a party of three to six.  Make sure all your members are present and qualified to participate in this quest.  I see #b" +
                levelValid.toString() +
      ...
      if (
        ...
        cm
          .getQuestStatus(6192)
          .equals(Packages.client.MapleQuestStatus.Status.STARTED) == false
      ) {
      ...
      if (cm.getJob().equals(MapleJob.PAGE)) {
        ...
        cm.getChar().gainAp(5);
        cm.sendOk(
          "You are now a #bDragon Knight#k.\r\n\r\nNow go, with pride!"
        );
        cm.dispose();
      }
    }
