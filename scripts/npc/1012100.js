  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      if (cm.getJob().equals(MapleJob.BEGINNER)) {
        ...
        cm.getPlayer().getJob().isA(MapleJob.BOWMAN)
      ) {
        cm.sendYesNo(
      ...
      cm.sendNextPrev(
        "You may be ready to take the next step as a #rHunter#k or #rCrossbowman#k."
      );
    } else if (status == 12) {
      ...
      if (cm.haveItem(4031010)) {
        ...
        cm.sendOk(
          "Go see the #bJob Instructor#k near Henesys. He will show you the way."
        );
      }
    } else if (status == 21) {
      cm.sendSimple(
