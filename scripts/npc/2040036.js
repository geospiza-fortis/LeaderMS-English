  ...
  if (mode == -1) {
    ...
    if (playerStatus) {
      ...
      if (status == 0) {
        ...
        if (preamble == null) {
          cm.sendNext(
            "Hello, and welcome to the first stage of Ludibrium PQ. There are #r25#k monsters in here, kill them to retrieve the passes, and give them to me, and I will open the portal."
          );
          ...
          if (complete != null) {
            ...
            if (cm.haveItem(4001022, 25) == false) {
              cm.sendNext(
                "I'm sorry, but you do not have all 25 passes needed to clear this stage."
              );
              cm.dispose();
            } else {
              cm.sendNext(
                "Congratulations on clearing the first stage! I will open the portal now."
              );
              clear(1, eim, cm);
