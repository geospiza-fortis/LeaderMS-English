  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      if (cm.getJob().equals(MapleJob.BEGINNER)) {
        ...
        cm.getPlayer().getJob().isA(MapleJob.WARRIOR)
      ) {
        cm.sendYesNo(
        ...
        if (cm.getLevel() >= 30 && cm.getJob().equals(MapleJob.WARRIOR)) {
          if (
            ...
            if (
              cm.getQuestStatus(100005) == MapleQuestStatus.Status.COMPLETED
            ) {
              status = 20;
              cm.sendNext(
      ...
      cm.sendNextPrev(
        "You may be ready to take the next step as a #rFighter#k, #rPage#k or #rSpearman#k."
      );
    } else if (status == 12) {
      ...
      if (cm.haveItem(4031008)) {
        ...
        cm.sendOk(
          "Go see the #bJob Instructor#k near Perion. He will show you the way."
        );
      }
    } else if (status == 21) {
      cm.sendSimple(
