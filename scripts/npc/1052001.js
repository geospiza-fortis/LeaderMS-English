  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      if (cm.getJob().equals(MapleJob.BEGINNER)) {
        ...
        cm.getPlayer().getJob().isA(MapleJob.THIEF)
      ) {
        cm.sendYesNo(
          "Wow, you already reached your maximum potential. Would you like to have an NPC player representing your power?"
        );
        pnpc = 1;
      } else {
        if (cm.getLevel() >= 30 && cm.getJob().equals(MapleJob.THIEF)) {
          if (
            cm.getQuestStatus(100009).getId() >=
            MapleQuestStatus.Status.STARTED.getId()
          ) {
            cm.completeQuest(100011);
            if (
              cm.getQuestStatus(100011) == MapleQuestStatus.Status.COMPLETED
            ) {
              status = 20;
              cm.sendNext(
      ...
      if (pnpc == 1) {
        ...
        if (success) {
          cm.sendOk(
            "Congratulations, your symbol of power will now be seen by all new players!"
          );
        } else {
          cm.sendOk(
            "There was a problem creating the NPC. You already have one or all NPC spawn points are taken."
          );
        }
        cm.dispose();
      } else {
      ...
      var p = cm.getPlayer();
      var totAp =
        p.getRemainingAp() + p.getStr() + p.getDex() + p.getInt() + p.getLuk();
      p.setStr(4);
      p.setDex(4);
      p.setInt(4);
      p.setLuk(4);
      ...
      cm.sendNextPrev(
        "You may be ready to take the next step as a #rAssassin#k or #rBandit#k."
      );
    } else if (status == 12) {
      ...
      if (cm.haveItem(4031011)) {
        ...
        cm.sendOk(
          "Go see the #bJob Instructor#k somewhere in the city. He will show you the way."
        );
      }
    } else if (status == 21) {
      cm.sendSimple(
