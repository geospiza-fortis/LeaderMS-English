...
function action(mode, type, selection) {
  if (mode == -1) {
    ...
    if (
      cm.getPlayer().getMapId() == 980010100 ||
      cm.getPlayer().getMapId() == 980010200 ||
      cm.getPlayer().getMapId() == 980010300
    ) {
      if (status == 0) {
        ...
        if (cm.checkSquadLeader(arena)) {
          ...
          for (var i = 1; i <= noOfChars; i++) {
            toSend +=
              ...
              i +
              "#" +
              cm.getSquadMember(arena, i - 1).getName() +
              "#l";
          }
        ...
        if (selection == 1) {
          ...
          if (cm.numSquadMembers(arena) < 2 && !cm.getChar().isGM()) {
            cm.sendOk(
              "I can only let you fight when you have two or more people."
            );
            cm.dispose();
          } else {
          ...
          cm.getPlayer()
            .getClient()
            .getChannelServer()
            .removeMapleSquad(squad, arena);
          cm.dispose();
        }
      }
    } else if (
      cm.getPlayer().getMapId() == 980010101 ||
      cm.getPlayer().getMapId() == 980010201 ||
      cm.getPlayer().getMapId() == 980010301
    ) {
      ...
      if (status == 0) {
        ...
        if (gotTheBombs != null) {
          cm.sendOk(
            "I already gave you the bombs. Please kill #eScorpions#n to get more in the next round!"
          );
