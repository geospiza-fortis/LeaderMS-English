...
var chatState;
var questions = Array(
  "This is your task: Collect the same number of coupons as the minimum level necessary to make the first job advancement to Warrior.",
    ...
    if (curMap == 1) {
      if (playerStatus) {
        if (status == 0) {
          ...
          if (preamble == null) {
            ...
            if (complete != null) {
              cm.sendNext(
                "Please hurry on to the next stage. The portal is open!"
              );
              cm.dispose();
            } else {
              ...
              if (!passes) {
                cm.sendNext(
                  "You need to collect the number of coupons that correctly answers the question. Neither more, nor less. Make sure you have the coupons in your inventory."
                );
                cm.dispose();
              } else {
                cm.sendNext(
                  "You collected " +
                    strpasses +
                    "! Congratulations on completing this stage. I will open the portal and send you to the next stage. There is a time limit, so please hurry. Good luck to all of you!"
                );
                clear(1, eim, cm);
        ...
        if (status == 0 && preamble == null) {
          ...
          if (complete != null) {
            ...
            if (enough && !toomany) {
              cm.sendNext(
                "Correct! You just won a #bpass#k. Please give it to your party leader."
              );
              cm.gainItem(4001007, -numcoupons);
              cm.gainItem(4001008, 1);
            } else {
              cm.sendNext(
                "Incorrect answer. I can give you the pass if you collect the number of #bcoupons#k that correctly answers the question."
              );
            }
            cm.dispose();
          }
      ...
      if (stage5done == null) {
        if (playerStatus) {
          ...
          if (passes) {
            cm.sendNext(
              "Here is the gate that leads to the ultimate bonus stage. This stage allows you to defeat common monsters a bit more easily. You will have a time limit to defeat as many as possible, but will be able to leave the stage when you want by talking to the NPC. Again, congratulations on completing the stages."
            );
            party = eim.getPlayers();
        ...
        if (status == 0) {
          cm.sendNext(
            "Incredible! You have completed all the stages in order to get here! Here is a small reward for a job well done. But before accepting, make sure you have available space in your use and etc. inventory tabs."
          );
        }
        if (status == 1) {
          getPrize(eim, cm);
        ...
        if (complete != null) {
          ...
          if (totplayers == 3 || debug) {
            ...
            if (debug) {
              ...
              cm.sendNext(
                "It seems that you still have not found the three barrels. Think of a different combination of the barrels. Don't forget that only 3 members can stand on top of the barrels, and they must be in the center for the response to count as correct. Keep trying!"
              );
            cm.dispose();
          }
        }
