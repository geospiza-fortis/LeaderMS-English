  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      if (cm.getSquadState(MapleSquadType.ZAKUM) == 0) {
        cm.sendSimple(
          "Would you like to assemble a team to take on the mighty #rZakum#k?\r\n#b#L1#Lets get this going!#l\r\n#L2#No, I think I'll wait a bit...#l"
        );
        ...
        if (cm.checkSquadLeader(MapleSquadType.ZAKUM)) {
          ...
          cm.sendSimple(
            "Would you like to join the Zakum squad?\r\n#b#L1#Yes, I'm ready to take him on!#l\r\n#L2#No, I think I'll wait a bit...#l"
          );
          status = 9;
        }
      } else {
        if (cm.checkSquadLeader(MapleSquadType.ZAKUM)) {
          ...
          cm.sendOk(
            "Sorry but the leader of the squad has closed registrations. Try again next time."
          );
          cm.dispose();
        }
      }
      ...
      if (selection == 1) {
        if (cm.createMapleSquad(MapleSquadType.ZAKUM) != null) {
          cm.sendOk(
            "The #rZakum Squad#k has been created, tell your members to sign up now.\r\n\r\nTalk to me again to view the current team, to close registrations, or start the fight!"
          );
          cm.dispose();
        } else {
          cm.sendOk(
            "There was an error, someone may have created a Zakum Squad before you, please inform a GameMaster if this is not the case."
          );
          cm.dispose();
        }
      } else if (selection == 2) {
      ...
      if (selection == 1) {
        if (cm.numSquadMembers(MapleSquadType.ZAKUM) > 29) {
          ...
          if (cm.canAddSquadMember(MapleSquadType.ZAKUM)) {
            ...
            cm.sendOk(
              "You have signed up, please wait for further instruction from your squad leader."
            );
            cm.dispose();
          } else {
            cm.sendOk(
              "Sorry, but the leader has requested you not to be allowed to join."
            );
            cm.dispose();
          }
        }
        ...
        cm.sendOk(
          "Sure, not everyone's up to challenging the might of #rZakum.#k"
        );
        cm.dispose();
      }
    } else if (status == 20) {
      if (selection == 1) {
        var noOfChars = cm.numSquadMembers(MapleSquadType.ZAKUM);
        var toSend =
          "The following members make up your squad (Click on them to Expell them):\r\n#b";
        for (var i = 1; i <= noOfChars; i++) {
          if (i == 1)
            toSend +=
              "\r\n" + cm.getSquadMember(MapleSquadType.ZAKUM, i - 1).getName();
        ...
        if (cm.getSquadState(MapleSquadType.ZAKUM) == 1) {
          ...
          cm.sendOk(
            "Registrations have been closed, please talk to me again to start the fight or re-open them."
          );
        } else {
          cm.setSquadState(MapleSquadType.ZAKUM, 1);
          cm.sendOk(
            "Registrations have been opened, please talk to me again to start the fight or close them."
          );
        }
        cm.dispose();
      } else if (selection == 3) {
      ...
      cm.warpSquadMembersClock(
        MapleSquadType.ZAKUM,
        280030000,
        10800,
        211042300
      ); //Should have a clock and exit now :D
      var squad = cm
        .getC()
        .getChannelServer()
        .getMapleSquad(MapleSquadType.ZAKUM);
