  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      ...
      if (checkLevelsAndMap(minLevel, maxLevel) == 2) {
        cm.sendOk(
          "Not all members of the party are present or are not within the allowed level range."
        );
        cm.dispose();
        return;
      } else {
        ...
        while (it.hasNext()) {
          var cPlayer = it.next();
          if (
            cPlayer.getLevel() >= minLevel &&
            cPlayer.getLevel() <= maxLevel
          ) {
            levelValid += 1;
          } else {
            next = false;
        ...
        if (next) {
          ...
          if (em == null) {
            ...
            if (prop.equals("0") || prop == null) {
              ...
              cm.sendOk(
                "Another party is already challenging the quest. Please try again later."
              );
              cm.dispose();
            }
          }
          ...
          cm.sendOk(
            "Someone in your party is not between levels 21-30. Please try again."
          );
          cm.dispose();
        }
      }
