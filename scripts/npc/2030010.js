...
function action(mode, type, selection) {
  if (mode == -1) {
    ...
    if (cm.getPlayer().getMapId() == 280030000) {
      if (status == 0) {
        cm.sendSimple(
          "Welcome to #rZakum's altar#k. I can not help with the fight, but can I do anything else for you?\r\n#L2#Take me to #bEl Nath#k!#l\r\n#L3#Take me to #bThe Door To Zakum#k!#l"
        );
      } else if (status == 1) {
        if (selection == 1) {
      ...
      cm.getPlayer().getMapId() == 280020000 ||
      cm.getPlayer().getMapId() == 280020001
    ) {
      if (status == 0) {
        cm.sendYesNo(
          "Would you like to quit this quest? You will have to start from scratch next time..."
        );
      } else if (status == 1) {
        cm.warp(211042300, 0);
        cm.dispose();
