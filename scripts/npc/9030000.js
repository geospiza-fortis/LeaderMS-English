...
function action(mode, type, selection) {
  ...
  if (status == 0)
    cm.sendNext(
      "#e<" +
        cm.getServerName() +
        " Merchant>#n\r\n\r\nHello #e#h ##n,\r\nMy name is  Fredrick, and I take care of storing items/mesos in #e" +
        cm.getServerName() +
        "#n.\r\n"
    );
  else if (status == 1)
      ...
      if (status == 3) {
        ...
        if (mesoEarnt > 0)
          cm.sendYesNo(
            "You have " +
              mesoEarnt +
              " mesos in this shop at the moment. Would you like to remove them?"
          );
        else {
