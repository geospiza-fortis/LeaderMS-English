  ...
  if (mode == -1) {
    ...
    if (cm.getPlayer().getMapId() == 680000000) {
      if (status == 0) {
        ...
        if (cm.getParty() == null) {
          cm.sendOk(
            "You want to get married? Create a party with your partner!"
          );
          cm.dispose();
          return;
        }
          ...
          if (next) {
            ...
            if (em == null) {
              ...
              var StringLine =
                ...
                " and " +
                wname +
                "'s wedding is starting in the Cathedral on Channel (" +
                cm.getC().getChannel() +
                ").";
            ...
            cm.sendOk(
              "Unable to start wedding. You must be on the same map with your partner. Only unmarried, opposite-sex partners can get married. The groom should be the party leader. Remember, you have to be level 10+ to get married."
            );
            cm.dispose();
          }
        }
        ...
        cm.sendGetText(
          "Please enter the name of one of the players getting married."
        );
        ...
        if (cm.getPlayer().getMarriageQuestLevel() == 50) {
          cm.sendNext(
            "Please go and visit Mom and Dad. They live in Henesys Hunting Ground II."
          );
        ...
        if (chrr != null) mapid = chrr.getMapId();
        if (mapid == 680000200) {
          ...
          cm.sendOk(
            "The wedding you would like to participate in has not started."
          );
          cm.dispose();
        }
        cm.dispose();
      ...
      if (status == 0) {
        if (
          cm.getPlayer().getId() != hiidii &&
          cm.getPlayer().getId() != wiidii
        ) {
