...
function action(mode, type, selection) {
  if (mode == -1) {
    ...
    if (status == 0) {
      cm.sendSimple(
        "Hi, I work for the #bmarket#k. What would you like to do today?\r\n#L0#Sell Items#l\r\n#L1#Buy Items#l\r\n#L2#Retrieve Items#l\r\n#L3#Nothing.#l\r\n"
      );
    } else {
      if (s == 0) {
        //Sell items
        if (status == 1) {
          cm.sendSimple(
            "You want to sell your items? Choose an inventory first:\r\n#L0#Equip#l\r\n#L1#Use#l\r\n#L2#Set up#l\r\n#L3#Etc#l\r\n#L4#Cash#l\r\n"
          );
        } else if (status == 2) {
          var inv = selection + 1;
          inventory = inv;
          ...
          if (quantity > item.getQuantity()) {
            cm.sendOk(
              "You're trying to sell more than you have. Sorry, but no can do."
            );
            cm.dispose();
          } else {
            cm.sendGetNumber(
          ...
          cm.sendOk(
            "Your item, #v" +
              item.getItemId() +
              "# was #bsuccessfully#k added to the market with " +
              quantity +
              " quantity and " +
              selection +
              " mesos."
        ...
        if (status == 1) {
          var m = cm.getMarket();
          if (!m.equals("")) cm.sendSimple("Heres the items:\r\n" + m);
          else {
        ...
        if (status == 1) {
          ...
          if (!r.equals("")) cm.sendSimple("Here are your items:\r\n" + r);
          else {
          ...
          cm.sendYesNo(
            "Are you sure you wish to retrieve #v" +
              cm.getMyMarketItems().get(selection).getId() +
              "#?"
          );
          retrieve = selection;
        } else if (status == 3) {
        ...
        //Nothing
        cm.sendOk(
          "Really? If you do want to access the market, don't hesitate to talk to me."
        );
        cm.dispose();
      } else {
        cm.dispose();
