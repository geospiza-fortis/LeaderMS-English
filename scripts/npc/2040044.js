        ...
        if (preamble == null) {
          ...
          if (complete != null) {
            ...
            if (cm.haveItem(4001023, 1) == false) {
              cm.sendNext(
                "I'm sorry, but you do not have the Key of Dimension needed to clear this stage."
              );
              cm.dispose();
            } else {
              cm.sendNext(
                "Congratulations on clearing the Boss stage! You have saved Ludibrium! Good Job and you will now be warped to Bonus!"
              );
              clear(1, eim, cm);
...
function clear(stage, eim, cm) {
  ...
  cm.mapMessage(
    "[" +
      cm.getServerName() +
      " Quest] Congratulations on successfully completing the quest!"
  );
}
