  ...
  if (mode == -1) {
    ...
    if (status == 0) {
      if (cm.getJob().equals(MapleJob.BEGINNER)) {
        ...
        cm.getPlayer().getJob().isA(MapleJob.MAGICIAN)
      ) {
        cm.sendYesNo(
        ...
        if (cm.getLevel() >= 30 && cm.getJob().equals(MapleJob.MAGICIAN)) {
          if (
            cm.getQuestStatus(100006).getId() >=
            MapleQuestStatus.Status.STARTED.getId()
          ) {
            cm.completeQuest(100008);
            if (
              cm.getQuestStatus(100008) == MapleQuestStatus.Status.COMPLETED
            ) {
              status = 20;
              cm.sendNext(
      ...
      if (cm.haveItem(4031009)) {
        ...
        cm.sendOk(
          "Go see the #bJob Instructor#k near Ellinia. He will show you the way."
        );
      }
    } else if (status == 21) {
      cm.sendSimple(
