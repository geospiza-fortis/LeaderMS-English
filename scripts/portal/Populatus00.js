...
function enter(pi) {
  var papuMap = ChannelServer.getInstance(
    pi.getPlayer().getClient().getChannel()
  )
    .getMapFactory()
    .getMap(220080001);
  if (papuMap.getCharacters().isEmpty()) {
    sendMessage(
      pi,
      "The room is empty.  A perfect opportunity to challenge the boss."
    );
...
function sendMessage(pi, message) {
  pi.getPlayer()
    .getClient()
    .getSession()
    .write(MaplePacketCreator.serverNotice(6, message));
}
