...
var bosshp = Array(
  ...
  10000000,
  20000000
); //-1 means original hp; mushmom is hardcoded into NPC as the NPC starts chain, so configure there
var mapId = 221000200;
...
function monsterValue(eim, mobId) {
  ...
  if (!contains(bossid, mobId) && mobId != 8500002)
    //boss summons and gm summons needn't count & we add an exception for papulatus cause he's cool
    return;
  ...
  map.broadcastMessage(
    MaplePacketCreator.serverNotice(
      6,
      "[Event] " + "Congratulations on killing " + bossname[stage] + "."
    )
  );
  stage++;
  if (stage > bossid.length - 1)
...
function playerRevive(eim, player) {
  ...
  if (
    (eim.isLeader(player) || party.size() <= minPlayers) &&
    eim.getProperty("enforceParty").equals("true")
  ) {
...
function playerDisconnected(eim, player) {
  ...
  if (
    ...
    for (var i = 0; i < party.size(); i++)
      if (party.get(i).equals(player)) removePlayer(eim, player);
      else playerExit(eim, party.get(i));
    dispose(eim);
  } else removePlayer(eim, player);
}
...
function playerExit(eim, player) {
  ...
  if (eim.getPlayerCount() == 0) dispose(eim);
}

function removePlayer(eim, player) {
  //for disconnected peeps / peeps who have left
  player.setallowedMapChange(true);
...
function timeOut(eim) {
  if (eim != null) {
    if (eim.getPlayerCount() > 0) {
      ...
      while (pIter.hasNext()) playerExit(eim, pIter.next());
    }
    stage = 0;
    dispose(eim);
  }
...
function spawnNextBoss(eim) {
  ...
  map.spawnMonsterOnGroudBelow(
    mob,
    new java.awt.Point(bossx[stage], bossy[stage])
  );
}
