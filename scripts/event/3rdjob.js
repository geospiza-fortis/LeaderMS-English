...
function setClassVars(player) {
  ...
  var mapId;
  if (
    player.getJob().equals(MapleJob.FP_WIZARD) ||
    ...
    player.getJob().equals(MapleJob.FIGHTER) ||
    ...
    player.getJob().equals(MapleJob.ASSASSIN) ||
    ...
    player.getJob().equals(MapleJob.HUNTER) ||
    ...
    player.getJob().equals(MapleJob.BRAWLER) ||
...
function allMonstersDead(eim) {
  ...
  em.getChannelServer().broadcastPacket(
    MaplePacketCreator.serverNotice(
      6,
      "[Event] " + winner.getName() + " defeated " + mob.getName() + "!"
    )
  );
}
