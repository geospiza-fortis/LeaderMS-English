...
function announce() {
  ...
  em.getChannelServer().broadcastPacket(
    MaplePacketCreator.serverNotice(
      6,
      "[Event] Field of Judgment will be opened in " + i + " minutes!"
    )
  );
  i--;
}
  ...
  while (iter.hasNext()) {
    ...
    if (eim.getPlayerCount() > 0) {
      ...
      map.broadcastMessage(
        MaplePacketCreator.serverNotice(
          6,
          "[Event] " + winner.getName() + " wins " + meso + " mesos!"
        )
      );
...
function start() {
  scheduleNew();
  em.getChannelServer().broadcastPacket(
    MaplePacketCreator.serverNotice(6, "[Event] Field of Judgment has opened.")
  );
  var iter = em.getInstances().iterator();
  while (iter.hasNext()) {
    var eim = iter.next();
...
function playerDead(eim, player) {
  ...
  player
    ...
    .write(
      MaplePacketCreator.serverNotice(
        1,
        "You lost with " + eim.getKillCount(player) + " points."
      )
    );
}
...
function allMonstersDead(eim, player) {
  ...
  if (priceRand <= 35) {
    ...
    var uberWeapons;
    if (
      eim.getName().equals("lolcastle1") ||
      eim.getName().equals("lolcastle2")
    ) {
      // not so uber (lv50 weapons)
      uberWeapons = new Array(
        1452008,
        1472021,
        1462007,
        1332054,
    ...
    price = new client.Item(
      starId,
      0,
      ii.getSlotMax(player.getClient(), starId)
    );
  } else if (priceRand > 75 && priceRand <= 95) {
    // 60% scroll
    var scrolls60 = new Array(
      2044701, // claw att
      2044501, // bow att
      2043301, // dagger att
    ...
    winner
      ...
      .write(
        MaplePacketCreator.serverNotice(
          1,
          "You win with " +
            eim.getKillCount(winner) +
            " points. You will be warped out in 2 minutes."
        )
      );
