    ...
    iter
      ...
      .write(
        MaplePacketCreator.serverNotice(
          6,
          "[" + eim.getServerName() + " Quest] The quest has begun!"
        )
      );
  }
}
  ...
  player
    ...
    .write(
      MaplePacketCreator.getClock(
        (Long.parseLong(eim.getProperty("entryTimestamp")) -
...
function playerRevive(eim, player) {
  ...
  player.setStance(0);
  player.changeMap(
    eim.getMapInstance(returnMap),
    eim.getMapInstance(returnMap).getPortal(0)
  );
  return false;
}
...
function playerDisconnected(eim, player) {
  ...
  if (player.getName().equals(eim.getProperty("leader"))) {
      ...
      pl.getClient()
        ...
        .write(
          MaplePacketCreator.serverNotice(
            ...
            "[" +
              eim.getServerName() +
              " Quest] The leader of the group was disconnected, and the remaining players will be warped out."
          )
        );
      if (pl.equals(player)) {
        removePlayer(eim, pl);
    ...
    if (party.size() < 6) {
        ...
        "[" +
          eim.getServerName() +
          " Quest] There are not enough players to continue. The remaining players will be warped out."
      );
    }
  }
}
...
function earringcheck(eim, player) {
  ...
  while (iter.hasNext()) {
    ...
    if (
      pl.getHp() > 0 &&
      pl.getMapId() > 990000200 &&
      pl.getInventory(MapleInventoryType.EQUIPPED).countById(1032033) == 0
    ) {
      pl.addHP(-30000);
      pl.getMap().broadcastMessage(
        MaplePacketCreator.serverNotice(
          6,
          "[" +
            eim.getServerName() +
            " Quest] " +
            pl.getName() +
            " died for not using the earrings!"
        )
      );
    }
  }
