  ...
  player
    ...
    .write(
      MaplePacketCreator.serverNotice(
        6,
        "[The Boss Quest] Your current points have been awarded, spend them as you wish. Better luck next time!"
      )
    );
...
function allMonstersDead(eim) {
  ...
  var monster_time = Math.round(
    (monster_end - parseInt(eim.getProperty("monster_start"))) / 1000
  );

  if (3600 - monster_time <= 0) points += monster_number * 10000;
  else
    points +=
      monster_number * 10000 + (3600 - monster_time) * (monster_number + 1);
  ...
  if (monster_number > 19) {
    map.broadcastMessage(
      MaplePacketCreator.serverNotice(
        6,
        "[The Boss Quest] Congratulations! Your team has defeated all the bosses with " +
          points +
          " points!"
      )
    );
    map.broadcastMessage(
      MaplePacketCreator.serverNotice(
        6,
        "[The Boss Quest] The points have been awarded, spend them as you wish."
      )
    );
    disbandParty();
  } else {
    map.broadcastMessage(
      MaplePacketCreator.serverNotice(
        6,
        "[The Boss Quest] Your team now has " +
          points +
          " points! The next boss will spawn in 10 seconds."
      )
    );
...
function monsterSpawn(eim) {
  var mob = MapleLifeFactory.getMonster(
    monster[parseInt(eim.getProperty("monster_number"))]
  );
...
function beginQuest(eim) {
  ...
  map.broadcastMessage(
    MaplePacketCreator.serverNotice(
      6,
      "[The Boss Quest] The creatures of the darkness are coming in 30 seconds. Prepare for the worst!"
    )
  );
