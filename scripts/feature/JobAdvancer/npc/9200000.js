...
function action(mode, type, selection) {
  ...
  if (cm.getJob().getId() % 10 == 2) {
    ...
    if (
      (secondJob && cm.getLevel() < 70) ||
      (!secondJob && cm.getLevel() < 120)
    ) {
      cm.sendOk("Hey, how's it going? I've been doing well here.");
      cm.dispose();
    } else {
      ...
      if (status == 0)
        cm.sendYesNo(
          "Great job getting to level " +
            cm.getLevel() +
            ". Would you like to become a #b" +
            MapleJob.getById(newJob) +
            "#k ?"
        );
      else if (status == 1) {
        cm.sendSimple(
          "Congratulations, you are now a #b" + MapleJob.getById(newJob) + "#k."
        );
      ...
      if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER)) {
        ...
        if (cm.getLevel() >= 10) {
          ...
          if (cm.getPlayer().getDex() >= 25) {
            possibleJobs.push(300);
            possibleJobs.push(400);
          }
        ...
        if (cm.getLevel() >= 30) {
          switch (cm.getJob().getId()) {
            case 100:
              possibleJobs = [110, 120, 130];
              break;
            case 200:
              possibleJobs = [210, 220, 230];
              break;
            case 300:
              possibleJobs = [310, 320];
              break;
            case 400:
              possibleJobs = [410, 420];
              break;
            case 500:
              possibleJobs = [510, 520];
              break;
          }
        }
      }
